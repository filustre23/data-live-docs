...
如要管理開放式資料表的中繼資料，Lakehouse for Apache Iceberg 會使用 [*Lakehouse 執行階段目錄*](https://docs.cloud.google.com/lakehouse/docs/about-lakehouse-catalogs?hl=zh-tw)。這項全代管的無伺服器中繼資料服務可為不同系統提供單一事實來源，集中探索資料，並免除在不同存放區之間同步處理中繼資料的需求。
...
* **無伺服器架構：**Google Cloud 的 Lakehouse 可免除伺服器或叢集管理作業，減少營運負擔，並根據需求自動調度資源。對於運算工作負載，無伺服器批次和互動式工作階段可消除工作之間的資源爭用，並自動維護基礎架構。
* **統一資料管理和治理：**與 [Knowledge Catalog](https://docs.cloud.google.com/dataplex/docs/introduction?hl=zh-tw) 整合，確保在多個引擎中集中定義及強制執行治理政策，並啟用語意搜尋、資料歷程和品質檢查。
...
* **跨雲端分析和 AI：**使用跨雲端 Lakehouse ([搶先版](https://docs.cloud.google.com/lakehouse/docs/about-cross-cloud-lakehouse?hl=zh-tw)) 同步處理其他雲端供應商的中繼資料，透過 Apache Iceberg REST 目錄端點，使用 BigQuery 或外部開放原始碼引擎查詢資料，完全不必遷移資料。
* **探索公開資料集：**使用 Apache Iceberg REST 目錄端點輕鬆查詢高品質公開資料集，不必管理基礎架構。
* **Hive Metastore：**使用 Hive 目錄 ([預覽版](https://docs.cloud.google.com/lakehouse/docs/about-spark-hive-metastore?hl=zh-tw))，將 Apache Spark 和 Apache Hive 等開放原始碼引擎連結至 Lakehouse 執行階段目錄。這樣一來，您就不必自行代管 Hive 中繼存放區 (HMS)，可減少作業負擔，並在 BigQuery 中順暢分享資料及直接查詢資料表。
...
* **IDE 和 Notebook：**使用互動式 Apache Spark Notebook 和 IDE 擴充功能 (例如 VS Code 適用的 Data Agent Kit (DAK) 擴充功能)，向 Google Cloud 進行驗證、以互動方式撰寫程式碼，以及直接在開發環境中管理 Notebook 工作階段。
...
* **支援 Apache Iceberg 資料表：**支援 Apache Iceberg V2 資料表 (正式版) 和 V3 資料表 (預先發布版)，但不支援 Iceberg V1 資料表。
