...
如要管理開放式資料表的中繼資料，Lakehouse for Apache Iceberg 會使用 [*Lakehouse 執行階段目錄*](https://docs.cloud.google.com/lakehouse/docs/about-lakehouse-catalogs?hl=zh-tw)。這項全代管的無伺服器中繼資料服務可為不同系統提供單一事實來源，集中管理探索作業，並免除在不同存放區之間同步中繼資料的需求。
