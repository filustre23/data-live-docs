...
如要停止強制執行存取權控管，您必須先刪除分類中與政策標記相關聯的所有資料政策。詳情請參閱[強制執行存取權控管](https://docs.cloud.google.com/bigquery/docs/column-level-security?hl=zh-tw#enforce_access_control)。
...
您也可以授予資料政策的精細存取權控管權限。

授予資料政策精細存取權控管權限，以及管理資料政策的權限不同。如要控管精細的存取權控管權限，請更新資料政策的 `grantees` 欄位。如要控管資料政策的存取權，請使用 [`setIamPolicy`](https://docs.cloud.google.com/bigquery/docs/reference/bigquerydatapolicy/rest/v2/projects.locations.dataPolicies/setIamPolicy?hl=zh-tw) 方法設定 IAM 角色。
...
如要從資料政策撤銷精細存取權控管存取權，請使用 `REVOKE FINE_GRAINED_READ` 陳述式移除 `grantees`。例如：
...
如要取得直接在資料欄上指派資料政策所需的權限，請要求管理員授予您資料表的「[BigQuery 資料政策管理員](https://docs.cloud.google.com/iam/docs/roles-permissions/bigquerydatapolicy?hl=zh-tw#bigquerydatapolicy.admin) 」(`roles/bigquerydatapolicy.admin`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
