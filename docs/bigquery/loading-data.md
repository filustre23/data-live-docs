...
在 BigQuery 頁面的「新增資料」[對話方塊](https://docs.cloud.google.com/bigquery/docs/bigquery-web-ui?hl=zh-tw#studio-overview)中，您可以查看所有可用的方法，將資料載入 BigQuery 或從 BigQuery 存取資料。根據您的用途和資料來源，選擇下列任一選項：

| 載入方法 | 說明 |
| --- | --- |
| **批次載入** | 這個方法適合從各種來源批次載入大量資料。   如要從 Cloud Storage 和其他支援的資料來源批次或增量載入資料，建議使用 [BigQuery 資料移轉服務](https://docs.cloud.google.com/bigquery/docs/dts-introduction?hl=zh-tw)。   使用 BigQuery 資料移轉服務，您可以排定載入工作，自動將資料載入管道移至 BigQuery。您可以安排一次性或批次資料移轉作業，並設定定期執行 (例如每日或每月)。為確保 BigQuery 資料一律為最新狀態，您可以監控及記錄移轉作業。   如要查看 BigQuery 資料移轉服務支援的資料來源清單，請參閱「[支援的資料來源](https://docs.cloud.google.com/bigquery/docs/dts-introduction?hl=zh-tw#supported_data_sources)」。 |
| **串流負荷** | 這種方法可從訊息系統近乎即時載入資料。   如要將資料串流至 BigQuery，您可以在 [Pub/Sub](https://docs.cloud.google.com/pubsub/docs/overview?hl=zh-tw) 中使用 BigQuery 訂閱項目。Pub/Sub 可處理高處理量資料，並載入至 BigQuery。這項服務支援即時資料串流，可載入產生的資料。詳情請參閱「[BigQuery 訂閱項目](https://docs.cloud.google.com/pubsub/docs/bigquery?hl=zh-tw)」。 |
...
| **串流負荷** | 如需支援自訂串流資料來源，或在將資料以高處理量串流至 BigQuery 前預先處理資料，請使用 [Dataflow](https://docs.cloud.google.com/dataflow/docs?hl=zh-tw)。  如要進一步瞭解如何從 Dataflow 載入至 BigQuery，請參閱「[從 Dataflow 寫入至 BigQuery](https://docs.cloud.google.com/dataflow/docs/guides/write-to-bigquery?hl=zh-tw)」。  您也可以直接使用 [BigQuery Storage Write API](https://docs.cloud.google.com/bigquery/docs/write-api?hl=zh-tw)。  如要最佳化資料表的串流作業，避免達到每日載入上限，請參閱「[最佳化載入工作](https://docs.cloud.google.com/bigquery/docs/optimize-load-jobs?hl=zh-tw)」。 |

[Cloud Data Fusion](https://docs.cloud.google.com/data-fusion/docs/concepts/overview?hl=zh-tw) 可協助您簡化 ETL 程序。此外，BigQuery 也與[第三方合作夥伴合作，將資料轉換並載入 BigQuery](https://docs.cloud.google.com/bigquery/docs/bigquery-ready-partners?hl=zh-tw#etl-data-integration)。

BigQuery 可讓您建立外部連線，查詢儲存在 BigQuery 以外的資料，例如 Cloud Storage 或 Spanner 等 Google Cloud 服務，或是 Amazon Web Services (AWS) 或 Microsoft Azure 等第三方來源。這些外部連線會使用 BigQuery Connection API。詳情請參閱[連線簡介](https://docs.cloud.google.com/bigquery/docs/connections-api-intro?hl=zh-tw)。

## 其他取得資料的方式

您可以查詢資料，不必自行將資料載入 BigQuery。以下各節將說明幾種替代方案。
...
如要查詢他人與您共用的 BigQuery 資料集，請參閱[BigQuery sharing (舊稱 Analytics Hub) 簡介](https://docs.cloud.google.com/bigquery/docs/analytics-hub-introduction?hl=zh-tw)。Sharing 是一個資料交換平台，可讓您共用資料。
...
* **Observability Analytics** 可讓您[執行查詢來分析記錄檔資料](https://docs.cloud.google.com/logging/docs/log-analytics?hl=zh-tw#analytics)。
...
* 進一步瞭解如何透過[工作探索器](https://docs.cloud.google.com/bigquery/docs/admin-jobs-explorer?hl=zh-tw)和 [BigQuery 指標](https://docs.cloud.google.com/monitoring/api/metrics_gcp_a_b?hl=zh-tw#gcp-bigquery)監控載入工作。
