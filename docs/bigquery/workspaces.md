...
使用 BigQuery [存放區](https://docs.cloud.google.com/bigquery/docs/repository-intro?hl=zh-tw)對檔案進行版本管控時，您可以使用[工作區](https://docs.cloud.google.com/bigquery/docs/workspaces-intro?hl=zh-tw)編輯這些存放區中的程式碼。本文說明如何在工作區中建立、刪除及管理檔案。
...
工作區的存取權取決於上層存放區授予的角色。詳情請參閱「[必要的角色](https://docs.cloud.google.com/bigquery/docs/repositories?hl=zh-tw#required-roles)」。

## 建立工作區

視您使用的是 BigQuery 存放區或第三方存放區而定，工作區的建立方式有所不同。
...
7. 指定要使用的分支版本：
   ...
   1. 如果您選擇「現有的遠端分支版本」圓形按鈕，請在「遠端分支版本」欄位中選擇現有的分支版本。
   2. 如果選擇「新分支」單選按鈕，請在「工作區 ID」欄位中輸入分支名稱。在第三方存放區中建立的分支名稱與 BigQuery 工作區 ID 相同。
...
BigQuery 會使用 Git 追蹤存放區內檔案的每項變更。在 BigQuery 存放區中，您可以直接與 Git 存放區互動。在連結的存放區中，您可以與[連結存放區](https://docs.cloud.google.com/bigquery/docs/repositories?hl=zh-tw#connect-third-party)時設定的遠端存放區預設分支互動。

BigQuery 會根據工作區的變更狀態顯示版本管控選項。舉例來說，只有當工作區有未提交的本機變更時，BigQuery 才會顯示提交選項。如果工作區中的檔案與預設或預設分支版本完全相同，BigQuery 會顯示「最新版本」狀態。
...
如果工作區與存放區不同步，BigQuery 會顯示「Pull」選項。

如要將存放區的變更內容拉取至工作區，請按照下列步驟操作：
...
在工作區中進行變更後，BigQuery 會顯示「Commit」(提交) 選項。您可以提交所有本機變更或選取的檔案。工作區窗格中，新增或修改的檔案旁會顯示藍點。

**注意：** BigQuery 預設會使用已驗證使用者的電子郵件地址做為提交作者，但 Dataform API 允許為提交作者設定自訂電子郵件地址。 Google Cloud 這項行為與 [`git commit --author` 指令](https://git-scm.com/docs/git-commit#Documentation/git-commit.txt---authorauthor) 類似，且只會影響 Git 修訂記錄。提交作者身分未經過加密驗證。
...
7. 在「Commit changes」(提交變更) 窗格中，執行下列操作：
   ...
   1. 選取要修訂的已變更檔案。
      ...
      如未選取任何檔案，BigQuery 會提交所有本機變更。您可以依檔案狀態、檔案名稱和路徑篩選變更的檔案。
...
6. 在工作區窗格中執行下列操作：
   ...
   2. 如果您位於[已連結至第三方 Git 存放區](https://docs.cloud.google.com/bigquery/docs/repositories?hl=zh-tw#connect-third-party)的存放區中，請執行下列任一操作：
      ...
      1. 按一下「推送至預設分支版本」，將內容推送至第三方存放區的預設分支版本。
      2. 按一下「推送至」**`your-branch-name`**，將變更推送至與目前工作區對應的第三方存放區分支。
...
如果工作區的本機變更與存放區預設分支版本的變更不相容，就可能發生合併衝突。通常是多位使用者同時編輯同一個檔案時，就會發生合併衝突。

通常在其他使用者將衝突變更推送至相同分支後，您從分支提取時會遇到合併衝突。您需要編輯受影響的檔案，手動解決合併衝突。
...
3. 在「Explorer」窗格中展開專案，然後按一下「Repositories」，在詳細資料窗格中開啟「Repositories」分頁。
4. 選取要刪除的工作區所在的存放區。
5. 找出工作區，然後按一下 more\_vert「開啟動作」**> 刪除**。
6. 點選「刪除」。
