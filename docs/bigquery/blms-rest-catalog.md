...
对于新工作流，我们建议在 *Lakehouse 运行时目录*中使用 *Apache Iceberg REST Catalog* 端点。
...
* 仅支持 Iceberg V2 表。
* 只有在 Compute Engine 上使用 Managed Service for Apache Spark 2.3 映像版本 2.3.16 及更高版本时，Trino 才支持 BigQuery 目录联合。
* 使用凭证分发模式时，您必须将 `io-impl` 属性设置为 `org.apache.iceberg.gcp.gcs.GCSFileIO`。不支持默认值 `org.apache.iceberg.hadoop.HadoopFileIO`。
