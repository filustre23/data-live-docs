...
* BigQuery 使用者 (`roles/bigquery.user`)：可執行或重複執行工作、列出工作、查看工作詳細資料，以及取消工作 (這個角色的權限比 BigQuery 工作使用者更寬鬆)。
* BigQuery 資源管理員 (`roles/bigquery.resourceAdmin`) - 列出所有工作，並擷取任何工作的中繼資料。
* BigQuery 管理員 (`roles/bigquery.admin`)：可列出所有工作、擷取任何工作的中繼資料，以及取消任何工作。
...
您可以使用 Google Cloud 主控台、bq 指令列工具、API 或用戶端程式庫，查看工作詳細資料。詳細資料包括資料和中繼資料，例如工作類型、工作狀態，以及建立工作的使用者。
...
發出 [`bq show`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_show) 指令並搭配使用 `--job=true` 標記和工作 ID。

當您提供工作 ID 時，可以使用完整 ID 或簡短格式。舉例來說， Google Cloud 控制台中列出的工作 ID 是完整名稱，也就是包含專案和位置：
...
* `LOCATION`：執行工作的位置名稱。舉例來說，如果您在東京地區使用 BigQuery，請將該旗標的值設定為 `asia-northeast1`。您可以使用 [`.bigqueryrc` 檔案](https://docs.cloud.google.com/bigquery/docs/bq-command-line-tool?hl=zh-tw#setting_default_values_for_command-line_flags)設定位置的預設值。如果未在工作 ID 中指定位置，或使用 `--location` 標記，系統會使用預設位置。
* `JOB_ID`：工作 ID
...
BigQuery 會儲存專案所有位置的所有工作，為期六個月的工作記錄。工作記錄包含處於「`RUNNING`」狀態的工作，以及已經「`DONE`」(從回報的「`SUCCESS`」或「`FAILURE`」狀態來判斷) 的工作。
...
* `--min_creation_time`：列出在已提供的時間戳記值之後的工作。這個值以毫秒為單位，表示為 [Unix 紀元](https://wikipedia.org/wiki/Unix_time)時間戳記。
...
* `MIN_TIME`：代表以毫秒為單位的 [Unix Epoch](https://wikipedia.org/wiki/Unix_time) 時間戳記。
* `MAX_TIME`：代表以毫秒為單位的 [Unix Epoch](https://wikipedia.org/wiki/Unix_time) 時間戳記。
...
如果您位於不同專案，但與要取消的工作位於相同區域，您也必須加入專案 ID：
...
提供 `JOB_ID` 引數時，可以使用完整 ID 或簡短格式。舉例來說， Google Cloud 控制台中列出的工作 ID 是完整名稱，包含專案和位置：
...
* `LOCATION` (選用)：執行工作的位置名稱。舉例來說，如果您在東京地區使用 BigQuery，請將該旗標的值設定為 `asia-northeast1`。您可以使用 [`.bigqueryrc` 檔案](https://docs.cloud.google.com/bigquery/docs/bq-command-line-tool?hl=zh-tw#setting_default_values_for_command-line_flags)設定位置的預設值。
* `JOB_ID`：要取消的工作 ID。
  如果您從 Google Cloud 控制台複製工作 ID，則專案 ID 和位置會併入工作 ID 中；例如：`my-project-1234:US.bquijob_123x456_123y123z123c`。
...
以下指令會取消在 `my-project-1234` 專案中於 `US` 多地區位置執行的 `bquijob_123x456_123y123z123c` 工作，並立即傳回。因為使用的是完整工作 ID，所以未提供 `--location` 標記。
...
您可以使用 bq 指令列工具和 Python 用戶端程式庫，刪除特定工作的中繼資料。BigQuery 會保留過去 6 個月內執行的工作記錄。您可以使用這個方法，移除查詢陳述式中可能存在的敏感資訊。工作完成後才能刪除工作的中繼資料。如果工作已建立子項工作，子項工作也會一併刪除。系統不允許刪除子項工作。只能刪除父項或頂層工作。
...
當您提供工作 ID 時，可以使用完整 ID 或簡短格式。舉例來說， Google Cloud 控制台中列出的工作 ID 是完全符合資格的 ID，也就是包含專案和位置：
...
系統無法重新執行工作 ID 相同的工作，因此您必須使用相同的設定建立新工作。您可以在Google Cloud console 或 bq 指令列工具中提交新工作，系統會指派新的工作 ID。如果您使用 API 或用戶端程式庫來提交工作，則必須產生新的工作 ID。
