...
如要安排從 Salesforce Sales Cloud 到 BigQuery 的週期性資料移轉作業，請建立移轉設定，指定要移轉的資料物件，以及安排資料移轉作業的頻率。您可以使用 Google Cloud 控制台、`bq` 指令列工具或 BigQuery 資料移轉服務 API，建立移轉設定。設定完移轉設定後，[BigQuery 資料移轉服務](https://docs.cloud.google.com/bigquery/docs/dts-introduction?hl=zh-tw)就會按照指定時間表，將最新資料移轉至 BigQuery 資料表。

如要瞭解 Salesforce 轉移作業的運作方式，請參閱「[Salesforce 轉移作業簡介](https://docs.cloud.google.com/bigquery/docs/salesforce-transfer-intro?hl=zh-tw)」。
...
* Salesforce 連接器僅支援 Salesforce Bulk API V1 64.0 版中包含的欄位。系統可能不支援舊版 Salesforce Bulk API 中的某些欄位。如要進一步瞭解 Salesforce 連接器的這些變更，請參閱「[Salesforce Bulk API](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#salesforce)」。
* Salesforce 連接器會使用 Salesforce Bulk API V1 連線至 Salesforce Sales Cloud 端點，以擷取資料。

  + Salesforce 連接器僅支援 Salesforce Bulk API V1，可連線至 Salesforce 執行個體，且僅支援轉移 Salesforce Bulk API 支援的實體。如要進一步瞭解支援的實體，請參閱「[Bulk API 不支援實體](https://help.salesforce.com/s/articleView?id=000383508&type=1)」。
...
* 如果透過網路連結進行資料移轉，請務必[設定 Public NAT](https://docs.cloud.google.com/nat/docs/set-up-manage-network-address-translation?hl=zh-tw)，並使用靜態 IP 位址。詳情請參閱「[為 Salesforce 移轉作業設定 IP 許可清單](https://docs.cloud.google.com/bigquery/docs/salesforce-transfer?hl=zh-tw#salesforce-allowlist)」一文。
...
如要瞭解增量轉移的運作方式，請參閱「[完整或增量轉移](https://docs.cloud.google.com/bigquery/docs/salesforce-transfer-intro?hl=zh-tw#full-incremental-transfers)」。
...
* [在連結的應用程式中設定基本資訊](https://help.salesforce.com/s/articleView?id=sf.connected_app_create_basics.htm&type=5)。**連結的應用程式名稱**和**聯絡人電子郵件地址**
  欄位是 Salesforce 轉移作業的必要欄位。
* [啟用 OAuth 設定](https://help.salesforce.com/s/articleView?id=sf.connected_app_create_api_integration.htm&type=5)
  ...
  + 確認未選取「Issue JSON Web Token (JWT)-based access tokens for named users」(為具名使用者核發以 JSON Web Token 為基礎的存取權杖) 核取方塊。
* 在「Selected OAuth Scopes」部分，選取「Manage user data via APIs (api)」。
...
* 選取「Enable Client Credentials Flow」，然後按一下顯示通知中的「OK」。

設定連結的應用程式並完成必要設定後，請按一下「儲存」。系統會將您重新導向至新建立的「已連結的應用程式」詳細資料頁面。
...
5. 「應用程式詳細資料」頁面隨即顯示。在「Client Credentials Flow」(用戶端憑證流程) 部分，於「Run As」(以使用者身分執行) 欄位中輸入使用者名稱。您可以使用這個欄位的尋找工具，確保選取的使用者正確無誤。
6. 按一下 [儲存]。
...
8. 在「消費者詳細資料」頁面中，「消費者金鑰」就是你的 `ClientId`
   值。「Customer Secret」是你的 `ClientSecret` 值。
...
### 為 Salesforce 轉移作業設定 IP 許可清單

您必須設定 Google Cloud 環境和 Salesforce 帳戶，才能將特定 IP 位址加入資料傳輸許可清單。這樣可確保 Salesforce 只會接受來自可信任的靜態 IP 位址的連線。如果您在資料傳輸時使用網路附件，就必須執行這個步驟。

如要這麼做，您必須先設定 Google Cloud 網路，使其使用靜態 IP 位址：

1. [設定公用網路位址轉譯 (NAT)](https://docs.cloud.google.com/nat/docs/set-up-manage-network-address-translation?hl=zh-tw)，並在虛擬私有雲網路中指定靜態 IP 位址。Cloud NAT 必須在與資料移轉目的地資料集相同的區域內設定。
...
**注意：** 如要更精細地控管，可以在設定檔層級或每次 API 呼叫時套用 IP 限制。詳情請參閱「[限制設定檔中的登入 IP 位址](https://help.salesforce.com/s/articleView?id=platform.login_ip_ranges.htm&type=5)」一文。

設定 IP 範圍後，您現在可以在[設定轉移設定](https://docs.cloud.google.com/bigquery/docs/salesforce-transfer?hl=zh-tw#sf-transfer-setup)時指定靜態 IP，方法是在「網路附件」欄位中選取網路附件。
...
如要將 Salesforce 資料新增至 BigQuery，請使用下列任一方法設定移轉作業：
...
4. 在「Data source details」(資料來源詳細資料) 部分執行下列操作：
   ...
   * 在「Ingestion type」(擷取類型) 部分，選取「Full」(完整) 或「Incremental」(增量)。
     + 如果為「Write mode」(寫入模式) 選取「Incremental」(增量)，請選取「Append」(附加) 或「Upsert」(新增或更新)。如要進一步瞭解不同的寫入模式，請參閱「[完整或增量轉移](https://docs.cloud.google.com/bigquery/docs/salesforce-transfer-intro?hl=zh-tw#full-incremental-transfers)」一節的說明。
...
* DATASET：移轉設定的目標資料集。
* PARAMETERS：已建立移轉設定的 JSON 格式參數。例如：`--params='{"param":"param_value"}'`。以下是 Salesforce 資料轉移的參數：
  ...
  + `connector.authentication.oauth.myDomain`：[Salesforce 我的網域](https://help.salesforce.com/s/articleView?id=sf.domain_name_overview.htm)。舉例來說，如果網域網址為 `example.my.salesforce.com`，則值為 `example`。
  + `ingestionType`：指定 `full` 或 `incremental`。詳情請參閱「[完整或增量轉移](https://docs.cloud.google.com/bigquery/docs/salesforce-transfer-intro?hl=zh-tw#full-incremental-transfers)」。
