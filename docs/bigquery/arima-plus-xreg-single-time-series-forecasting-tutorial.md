...
2. 在查詢編輯器中貼入下列查詢，然後點選「執行」：
   ...
   `has_holiday_effect`、`has_spikes_and_dips` 和 `has_step_changes` 欄提供輸入時間序列資料的相關資訊，與 ARIMA 模型建立功能無關。系統傳回這些資料欄，是因為 `CREATE MODEL` 陳述式中的 `decompose_time_series` 選項值為 `TRUE`。所有輸出資料列的這些資料欄值也相同。
