...
* [排定管道時間表](https://docs.cloud.google.com/bigquery/docs/schedule-pipelines?hl=zh-tw)，在指定時間和頻率自動執行。
...
* 如果排定的管道執行作業未在下一個排定作業開始前完成，系統會略過下一個排定作業，並標示為錯誤。
...
所有程式碼資產都會儲存在[程式碼資產的預設區域](#set_the_default_region_for_code_assets)中。更新預設區域後，之後建立的所有程式碼資產都會使用新的區域。
...
含有筆記本的管道會產生 Colab Enterprise 執行階段費用，費用依[預設機型](https://docs.cloud.google.com/colab/docs/runtimes?hl=zh-tw#default_runtime_specifications)計算。
如需定價詳細資料，請參閱 [Colab Enterprise 定價](https://cloud.google.com/colab/pricing?hl=zh-tw)。

系統會使用 [Cloud Logging](https://docs.cloud.google.com/logging/docs?hl=zh-tw) 記錄每次 BigQuery 管道執行作業。BigQuery 管道執行作業會自動啟用記錄功能，這可能會產生 Cloud Logging 帳單費用。詳情請參閱 [Cloud Logging 定價](https://cloud.google.com/logging/pricing?hl=zh-tw)。
