...
您可以在Google Cloud 控制台的「評估」專區中找到筆記本的下載連結。這些檔案會以 ZIP 封存檔的形式提供，內含下列項目：
...
您必須使用其中一種方法，才能繼續下一個章節。
如果可以，建議您使用服務主體，而非 PAT，因為 Databricks 認為 PAT 選項是舊版。

## 取得評估資料

完成所有設定步驟後，即可執行筆記本。啟用擷取筆記本後，請使用「全部執行」選項執行筆記本。

如果系統非常龐大，執行筆記本可能需要一天。以約 50,000 個資料表的典型系統為例，擷取作業耗時不到一小時。

## 選用：監控擷取筆記本進度

擷取筆記本會提供含有進度和錯誤資訊的記錄，以供監控。特定筆記本執行的最新記錄訊息包含所擷取內容的相關資訊。

由於筆記本執行作業不會在發生第一個錯誤時停止，因此請分兩步驟檢查錯誤：

1. 擷取完成後，請查看摘要中的錯誤計數或其他錯誤指標，例如找到的資料少於預期。如果結果可接受，可以跳過下一個步驟。
2. 複製記錄輸出內容，並儲存為文字檔。
...
`False` 結果表示驗證錯誤。請確認核對和與 ZIP 檔案是從相同發布版本下載，並放在同一個目錄中。
...
* `VERSION`：驅動程式版本
* `USER`：用於資料庫連線的使用者名稱
* `IAM_PROFILE_NAME`：[Amazon Redshift IAM 設定檔名稱](https://docs.aws.amazon.com/redshift/latest/mgmt/connecting-with-authentication-profiles.html)。這是 Amazon Redshift 驗證和 AWS API 存取權的必要條件。如要取得 Amazon Redshift 叢集的說明，請使用 AWS API。
...
使用 `dwh-migration-dumper` 工具從 Amazon Redshift Serverless 命名空間擷取用量記錄和中繼資料，並以兩個 ZIP 檔案的形式儲存。在可存取來源資料倉儲的機器上執行下列指令，產生檔案。
...
* 用於儲存結果的空白 BigQuery 資料集。
  或者，您也可以使用 Google Cloud 控制台 UI 建立評估工作時，一併建立 BigQuery 資料集。
...
使用 `dwh-migration-dumper` 工具從 Snowflake 資料倉儲中擷取記錄和中繼資料，並壓縮成兩個 zip 檔案。在可存取來源資料倉儲的機器上執行下列指令，產生檔案。
...
如果是多租戶資料庫，則必須在根容器中執行 `dwh-migration-dumper` 工具。在其中一個可插入式資料庫中執行，會導致其他可插入式資料庫的效能統計資料和中繼資料遺失。
...
4. 使用 `dwh-migration-dumper` 工具將中繼資料和成效統計資料擷取至 zip 檔案：
   ...
   * `SPARK_HISTORY_SERVICE_NAMES` (選用)：Spark 記錄伺服器的服務名稱 (以逗號分隔的清單)，用於透過 Apache Knox 查詢 Spark 事件記錄，以擷取應用程式中繼資料。如未提供，則預設值為 `sparkhistory,spark3history`。
...
BigQuery 遷移評估作業會使用資料表、分區和資料欄統計資料，進一步瞭解 Apache Hive 資料倉儲，並提供深入洞察資訊。如果來源 Apache Hive 資料倉儲中的 `hive.stats.autogather` 設定設為 `false`，建議您啟用這項設定或手動更新統計資料，再執行 `dwh-migration-dumper` 工具。
...
* Google Cloud 帳戶，並具備 Cloud Storage bucket 來儲存資料。
* 用於儲存結果的空白 BigQuery 資料集。
  或者，您也可以使用 Google Cloud 控制台建立評估工作時，一併建立 BigQuery 資料集。
...
BigQuery 遷移評估會將評估結果寫入 BigQuery 中的資料表。開始前，請[建立資料集](https://docs.cloud.google.com/bigquery/docs/datasets?hl=zh-tw)來存放這些資料表。共用數據分析報表時，您也必須授予使用者讀取這個資料集的權限。詳情請參閱「[為使用者提供報表](#share_the_data_studio_report)」。
...
4. 填寫評估設定對話方塊。
   ...
   1. 在「Display name」(顯示名稱) 部分，輸入名稱，名稱可包含英文字母、數字或底線。這個名稱僅供顯示之用，不必是專屬名稱。
   2. 在「評估資料來源」中，選擇資料倉儲。
...
* **現有系統。**這個部分是現有 Teradata 系統和用量的快照，包括資料庫、結構定義、資料表數量，以及以 TB 為單位的總大小。此外，這項服務還會依大小列出結構定義，並指出可能未充分利用的資源 (沒有寫入或讀取次數很少的資料表)。
* **BigQuery 穩定狀態轉換 (建議)。**
...
:   「分區和叢集」檢視畫面會顯示可透過分區、叢集或兩者獲益的資料表。
    ...
    報表中的建議僅適用於大於 1 GB 的資料表，因為叢集處理和分割功能對小型資料表沒有幫助。不過，所有資料表 (包括小於 1 GB 的資料表) 的 DDL 都會顯示在 `SchemaConversion` 資料表中。

沒有使用情形的資料表
:   「沒有用量的資料表」檢視畫面會顯示 BigQuery 遷移評估在分析記錄期間，未偵測到任何用量的資料表。如果使用量不足，可能表示您不需要在遷移期間將該資料表移轉至 BigQuery，或是將資料儲存在 BigQuery 的費用可能較低 (以[長期儲存空間](https://cloud.google.com/bigquery/pricing?hl=zh-tw#storage)計費)。建議您驗證未使用的資料表清單，因為這些資料表可能在記錄檔期間外有使用情形，例如每三或六個月只使用一次的資料表。
...
* 相容性：提供遷移作業本身的相關資訊。
  針對每個分析的資料庫，這項工具都會顯示預計遷移時間，以及可透過 Google 提供的工具自動遷移的資料庫物件數量。
...
* 資料庫連線：深入瞭解使用者或應用程式啟動的資料庫工作階段。分析這項資料有助於找出可能需要額外費時遷移的外部應用程式。
* 查詢類型：提供執行的 SQL 陳述式類型細目，以及使用統計資料。您可以利用「查詢執行」或「查詢 CPU 時間」的小時直方圖，找出系統使用率較低的時段，以及資料移轉的最佳時間。
* PL/SQL 原始碼：深入瞭解 PL/SQL 物件 (例如函式或程序) 及其大小 (適用於每個資料庫和結構定義)。此外，您也可以使用每小時執行次數直方圖，找出 PL/SQL 執行次數最多的尖峰時段。
...
* 總持有成本 (TCO)：可估算年度合約價值 (ACV)，也就是 BigQuery 的運算和儲存成本。您也可以使用計算機計算儲存空間費用。您也可以使用計算機計算儲存費用，這項費用會因*動態儲存*和*長期儲存*而異，取決於分析期間的表格修改次數。如要進一步瞭解儲存空間價格，請參閱[儲存空間價格](https://cloud.google.com/bigquery/pricing?hl=zh-tw#storage)。
...
* 資料庫物件遷移工作量：顯示各資料庫、結構定義或資料庫物件類型的預估遷移工作量 (以小時為單位)。此外，還會根據遷移工作量，顯示小型、中型和大型物件的百分比。
...
偏斜分區
:   「Skewed Partitions」檢視畫面會顯示以中繼資料分析為基礎的資料表，以及一或多個分區的資料偏斜。這些資料表很適合進行結構定義變更，因為查詢傾斜分區時，效能可能不佳。

BI Engine 和具體化檢視表
:   「低延遲查詢和具體化檢視表」檢視畫面會根據分析的記錄資料，顯示查詢執行時間的分布情形，並提供進一步的最佳化建議，以提升 BigQuery 的效能。如果查詢時間分配圖顯示大量查詢的執行時間少於 1 秒，建議啟用 BI Engine，加快 BI 和其他低延遲工作負載的執行速度。
