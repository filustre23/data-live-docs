...
* 資料集的讀取權限，可查看結果
* 建議：使用擷取工具存取系統資料表時，來源資料庫的管理員層級存取權
...
根據預設，公用程式記錄檔會從 `dbc.DBQLUtilityTbl` 表格中擷取。如要從其他位置擷取公用程式記錄，可以使用 `-Dteradata-logs.utility-logs-table` 旗標指定資料表名稱。
...
* 在資料庫上具有 `IMPORTED PRIVILEGES` 存取權的 Snowflake 使用者。`Snowflake`建議您建立採用金鑰配對驗證的使用者。[`SERVICE`](https://docs.snowflake.com/en/user-guide/admin-user-management#types-of-users)這樣一來，您就能安全地存取 Snowflake 資料平台，不必產生多重驗證權杖。
  ...
  + 除了金鑰組驗證，您也可以使用密碼驗證。不過，自 2025 年 8 月起，Snowflake 會對所有使用密碼登入的使用者強制執行多重驗證。使用擷取工具時，您必須核准多重驗證推播通知。
...
如要針對這項功能提供意見回饋或尋求支援，請傳送電子郵件至 [bq-edw-migration-support@google.com](mailto:bq-edw-migration-support@google.com)。
...
4. 使用 `dwh-migration-dumper` 工具將中繼資料和效能統計資料擷取至 zip 檔案：
   ...
   * `START_DATE` (選用)：歷史記錄傾印的開始日期，格式為 ISO 8601，例如 `2025-05-29`。預設值為目前日期前 90 天。
...
* 連線至來源 Apache Hive 資料倉儲的機器 (BigQuery 遷移評估支援 Hive on Tez 和 MapReduce，以及 Apache Hive 2.2 至 3.1 版)
* 具有 Cloud Storage bucket 的帳戶，用於儲存資料 Google Cloud
* 用於儲存結果的空白 BigQuery 資料集
* 資料集的讀取權限，可查看結果
...
將包含中繼資料和效能統計資料的 zip 檔案上傳至 Cloud Storage 值區。ZIP 檔案的預設名稱為 `dwh-migration-oracle-stats.zip`，但您可以在 `--output` 旗標中指定名稱，自訂檔案名稱。ZIP 檔案內所有檔案的未壓縮總大小上限為 50 GB。
...
將包含中繼資料和效能統計資料的 zip 檔案上傳至 Cloud Storage 值區。根據預設，ZIP 檔案的名稱為 `dwh-migration-cloudera-manager-RUN_DATE.zip` (例如 `dwh-migration-cloudera-manager-20250312T145808.zip`)，但你可以使用 `--output` 標記自訂名稱。ZIP 檔案內所有檔案的總未壓縮大小上限為 50 GB。
...
將含有 Informatica XML 存放區物件的 zip 檔案上傳至 Cloud Storage bucket。這個 ZIP 檔案也必須包含 `compilerworks-metadata.yaml` 檔案，其中含有下列內容：
...
**注意：** 資料集應與包含從來源資料庫擷取的中繼資料和記錄檔的 Cloud Storage 值區位於相同區域。不過，如果 Cloud Storage 值區位於多地區，則資料集必須位於該多地區內的任何地區。
...
4. 填寫評估設定對話方塊。
   ...
   4. 如要選擇評估結果的儲存方式，請執行下列任一步驟：
      ...
      * 取消勾選「自動建立新的 BigQuery 資料集」核取方塊，然後使用 `projectId.datasetId` 格式選擇現有的空白 BigQuery 資料集，或是建立新的資料集名稱。您可以選擇 BigQuery 資料集名稱。**重要事項：** Cloud Storage 值區位置和 BigQuery 資料集位置必須位於相同多地區，或位於該多地區內的位置。如要進一步瞭解位置限制，請參閱「[位置注意事項](https://docs.cloud.google.com/bigquery/docs/batch-loading-data?hl=zh-tw#data-locations)」。
...
6. 評估作業執行期間，您可以點選評估作業清單中的「查看報表」連結，在數據分析中查看部分資料的評估報表。評估作業執行期間，系統可能需要一些時間才會顯示「查看報表」連結。報表會在新的分頁中開啟。
...
系統會在新的分頁中開啟數據分析報表，並顯示預覽模式。您可以使用預覽模式，在進一步分享報表前先檢查內容。
...
應用程式
:   「應用程式使用情形」和「應用程式模式」檢視畫面會提供記錄處理期間發現的應用程式統計資料。使用者可透過這些檢視畫面瞭解特定應用程式的用量變化，以及對資源用量的影響。在遷移期間，請務必將資料的擷取和使用情形視覺化，進一步瞭解資料倉儲的依附元件，並分析一併遷移各種依附應用程式的影響。透過 JDBC 連線使用資料倉儲時，IP 位址表有助於找出確切的應用程式。

查詢
:   「查詢」檢視畫面會顯示執行的 SQL 陳述式類型，以及使用統計資料。您可以利用查詢類型和時間的直方圖，找出系統使用率偏低的時段，以及最適合傳輸資料的時段。您也可以使用這個檢視畫面，找出經常執行的查詢，以及叫用這些查詢的使用者。
...
唯一性限制
:   「唯一限制」檢視畫面會顯示來源資料倉儲中定義的 `SET` 資料表和唯一索引。在 BigQuery 中，建議使用暫存資料表和 `MERGE` 陳述式，只將不重複的記錄插入目標資料表。您可根據這個檢視區塊的內容，判斷在遷移期間可能需要調整 ETL 的資料表。
...
SQL 翻譯
:   「SQL 翻譯」檢視畫面會列出 BigQuery 遷移評估自動轉換的查詢數量和詳細資料，這些查詢不需要手動介入。如果提供中繼資料，自動 SQL 翻譯通常能達到高翻譯率。這個檢視畫面是互動式，可分析常見查詢和這些查詢的翻譯方式。
...
1. 「現有系統」面板會提供現有 Redshift 系統的資料庫、結構定義、資料表數量，以及總大小等資訊。此外，這項工具也會依大小和潛在的次佳資源使用率列出結構定義。您可以運用這項資訊移除、分區或叢集處理資料表，進而最佳化資料。
...
依類型和排程查詢
:   「依類型和時間表查詢」檢視畫面會將查詢分為 ETL/寫入和報表/彙整。查看一段時間內的查詢組合，有助於瞭解現有的使用模式，並找出可能影響成本和效能的突發狀況和潛在過度佈建。

查詢佇列
:   「查詢佇列」檢視畫面提供系統負載的額外詳細資料，包括查詢量、組合，以及因佇列而造成的任何效能影響，例如資源不足。

查詢和 WLM 資源調度
:   「Queries and WLM Scaling」(查詢和 WLM 擴充) 檢視畫面會將並行擴充視為額外成本和設定複雜度。顯示 Redshift 系統如何根據您指定的規則，以及佇列、並行調度和遭逐出查詢造成的效能影響，來傳送查詢。
...
:   「分區和叢集」檢視畫面會顯示可透過分區、叢集或兩者提升效能的資料表。
    ...
    系統會分析來源查詢記錄，以提供工作負載建議。系統會分析工作負載，尤其是所分析查詢記錄中的 `WHERE` 或 `JOIN` 子句，藉此判斷建議。
...
結構定義耦合
:   「結構定義耦合」檢視畫面提供資料庫、結構定義和資料表的高階檢視畫面，這些項目會透過單一查詢一併存取。這個檢視畫面可顯示經常參照的資料表、結構定義和資料庫，以及可用於遷移規劃的項目。
...
系統使用率
:   提供系統使用率的歷史記錄相關一般資訊。這個檢視畫面會顯示 RPU (Amazon Redshift 處理單元) 的過往用量，以及每日儲存空間用量。這個檢視畫面可協助您瞭解系統容量預留量。
...
這份報告包含不同部分，可單獨或一併使用。下圖將這些章節歸類為三種常見的使用者目標，協助您評估遷移需求：
...
:   列出不同等級/版本的定價。此外，這份文件還附上插圖，說明與 Snowflake 相比，BigQuery 自動調度資源功能如何協助您節省更多費用。
...
沒有寫入權的資料表
:   「沒有寫入作業的資料表」檢視畫面會顯示 BigQuery 遷移評估在所分析的記錄期間內，找不到任何更新的資料表。這可能表示在 BigQuery 中儲存資料的成本較低。
...
* 資料庫連線：深入瞭解使用者或應用程式啟動的資料庫工作階段。分析這項資料有助於找出可能需要額外費力的外部應用程式。
* 查詢類型：提供執行的 SQL 陳述式類型細目，以及使用統計資料。您可以利用「查詢執行作業」或「查詢 CPU 時間」的小時直方圖，找出系統使用率較低的時段，以及資料移轉的最佳時間。
...
* Exadata 與 BigQuery 定價比較：一般而言，比較 Exadata 和 BigQuery 的定價模式，有助於您瞭解遷移至 BigQuery 後的優點和潛在成本節省。
* BigQuery 資料庫讀取/寫入：提供資料庫實體磁碟作業的洞察資訊。分析這項資料有助於找出從 Oracle 遷移至 BigQuery 的最佳時機。
* BigQuery 運算費用：可預估 BigQuery 的運算費用。計算機中有四個手動輸入項目：**BigQuery 版本**、**區域**、**承諾期**和**基準**。根據預設，計算機提供最佳的經濟實惠基準承諾，您可以手動覆寫。「年度自動調度運算單元時數」值會顯示承諾使用量以外的運算單元時數用量。這個值是根據系統使用率計算得出。頁面結尾會以視覺化方式說明基準、自動調度資源和使用率之間的關係。每項預估值都會顯示可能的數量和預估範圍。
...
* 資料庫物件遷移工作量：顯示各資料庫、結構定義或資料庫物件類型的預估遷移工作量 (以小時為單位)。此外，還會根據遷移作業量，顯示小型、中型和大型物件的百分比。
...
佇列使用率
:   這個檢視畫面會提供處理記錄時發現的 YARN 佇列使用情況統計資料。使用者可透過這些檢視畫面，瞭解特定佇列和應用程式的使用情況，以及對資源用量的影響。這些檢視畫面也有助於找出並優先處理要遷移的工作負載。在遷移期間，請務必將資料的擷取和使用情形視覺化，進一步瞭解資料倉儲的依附元件，並分析一併遷移各種依附應用程式的影響。透過 JDBC 連線使用資料倉儲時，IP 位址表有助於找出確切的應用程式。
...
資料庫
:   這個檢視畫面會提供來源資料倉儲系統中定義的大小、資料表、檢視表和程序指標。這個檢視畫面可深入瞭解需要遷移的物件數量。
...
沒有使用情形的資料表
:   「沒有用量的資料表」檢視畫面會顯示 BigQuery 遷移評估在分析記錄期間，找不到任何用量的資料表。如果使用量偏低，可能表示您不需要在遷移期間將該資料表移轉至 BigQuery，或是將資料儲存在 BigQuery 的費用可能較低。您必須驗證未使用的資料表清單，因為這些資料表可能在記錄檔時間範圍外有使用情形，例如每三或六個月只使用一次的資料表。
...
這份數據分析報表是遷移評估的前端資訊主頁，這項功能取決於基礎資料集的存取權限。如要共用報表，收件者必須同時擁有 Google 數據分析報表本身，以及包含評估結果的 BigQuery 資料集存取權。
...
**注意：** 管理共用資料中的[資料交換](https://docs.cloud.google.com/bigquery/docs/analytics-hub-manage-exchanges?hl=zh-tw)或項目，不會產生額外費用。
