...
| `refresh_watermark` | `TIMESTAMP` | 具體化檢視表的重新整理浮水印。實體化檢視表快取會包含實體化檢視表基礎資料表在此時間點之前所含的資料。 |
...
下列範例會擷取預設專案 (`myproject`) 中 `mydataset` 內具體化檢視表 `myview` 的 `last_refresh_time` 和 `refresh_watermark`。結果會顯示上次重新整理具體化檢視表的時間，以及將基礎資料表的資料收集到具體化檢視表快取的時間。
...
**注意：** 如果基礎資料表近期沒有變更，BigQuery 會定期增加 `refresh_watermark`，表示具體化檢視區塊為最新狀態，但不會實際重新整理。因此，`last_refresh_time` 可能會早於 `refresh_watermark`。
