...
本文將概述 BigQuery 代管災難復原機制，以及如何為資料和工作負載實作這項機制。
...
復原時間目標 (RTO) 是指發生災難時，BigQuery 允許復原的目標時間。如要進一步瞭解 RTO，請參閱「[災難復原規劃基礎知識](https://docs.cloud.google.com/architecture/dr-scenarios-planning-guide?hl=zh-tw)」。啟動容錯移轉後，受管理的災難復原功能會在五分鐘內完成 RTO。由於 RTO 的關係，啟動容錯移轉程序後，次要區域的容量會在五分鐘內可用。
...
**注意：** 建立容錯移轉預留資源前，請先確認次要區域中沒有相同名稱的預留資源。同樣地，請確保指派給容錯移轉預留位置的新指派項目，不會在次要位置重新指派相同資源和相同工作類型。這類衝突可能會導致複寫失敗，進而造成主要和次要位置不一致。這可能會導致日後無法順利執行容錯移轉作業。
