...
BigQuery 支援災難復原，即使整個區域的服務全面中斷，也能正常營運。BigQuery 災難復原機制會透過[跨區域資料集複製](https://docs.cloud.google.com/bigquery/docs/data-replication?hl=zh-tw)功能，管理儲存空間容錯移轉。在次要區域建立資料集副本後，您可以控管運算和儲存空間的容錯移轉行為，在服務中斷期間維持業務持續性。容錯移轉後，您可以在升級的區域存取運算容量 (時段) 和複製的資料集。災難復原功能僅支援 [Enterprise Plus 版](https://docs.cloud.google.com/bigquery/docs/editions-intro?hl=zh-tw)。

受管理災難復原功能提供兩種容錯移轉選項：硬體容錯移轉和軟體容錯移轉。硬體容錯移轉會立即將次要區域的預留空間和資料集副本升級為主要區域。即使目前的主要區域處於離線狀態，這項動作也會繼續進行，且不會等待任何未複製的資料完成複製。因此，硬體容錯移轉可能會導致資料遺失。[`replication_time`](https://docs.cloud.google.com/bigquery/docs/information-schema-schemata-replicas?hl=zh-tw#schema) 值為副本後，來源區域中提交資料的任何工作，都可能需要在硬體容錯移轉後於目的地區域重新執行。相較於硬體容錯移轉，軟體容錯移轉會等待主要區域中提交的所有預留空間和資料集變更複製到次要區域，再完成容錯移轉程序。軟體容錯移轉需要主要和次要區域都處於可用狀態。啟動軟體容錯移轉會為預留空間設定 [`softFailoverStartTime`](https://docs.cloud.google.com/bigquery/docs/reference/reservations/rest/v1/projects.locations.reservations?hl=zh-tw#replicationstatus)。軟體容錯移轉完成後，[`softFailoverStartTime`](https://docs.cloud.google.com/bigquery/docs/reference/reservations/rest/v1/projects.locations.reservations?hl=zh-tw#replicationstatus) 會清除。

如要啟用災難復原功能，您必須在主要區域建立 Enterprise Plus 版本預留項目，也就是資料集在容錯移轉前的所在區域。配對區域的待命運算容量會納入 Enterprise Plus 預留項目。接著，將資料集附加至這個預留項目，即可為該資料集啟用容錯移轉功能。只有在資料集回填完畢，且與預留項目具有相同的配對主要和次要位置時，您才能將資料集附加至預留項目。資料集附加至容錯移轉預留項目後，只有 Enterprise Plus 預留項目可以寫入這些資料集，且您無法對資料集執行[跨區域複製](https://docs.cloud.google.com/bigquery/docs/data-replication?hl=zh-tw)升級作業。您可以使用任何容量模型，從附加至容錯移轉預留項目的資料集讀取資料。如要進一步瞭解預留項目，請參閱「[工作負載管理簡介](https://docs.cloud.google.com/bigquery/docs/reservations-intro?hl=zh-tw)」。

容錯移轉後，次要區域會立即提供主要區域的運算容量。無論是否使用，這項可用性都適用於預留項目的基準。

您必須主動選擇容錯移轉，才能進行測試或因應實際災難。在 10 分鐘內，您不應容錯移轉超過一次。在資料複製情境中，資料補充是指在建立或啟用副本之前，以現有歷來資料填入資料集副本的程序。資料集必須先完成回填，才能容錯移轉至該資料集。
...
* 容錯移轉後的自動調度資源取決於次要區域的運算容量可用性。次要區域僅提供預訂基準。
...
* [`INFORMATION_SCHEMA.JOBS`檢視畫面](https://docs.cloud.google.com/bigquery/docs/information-schema-jobs?hl=zh-tw)中的主要區域資料不會複製到次要區域。這個檢視畫面只會顯示工作執行所在特定區域的工作記錄。如果發生容錯移轉，使用 `INFORMATION_SCHEMA.JOBS` 檢視畫面時，次要區域不會顯示主要區域的工作記錄。
...
* 如果預留項目設定變更未成功複製到次要區域，就無法觸發軟性容錯移轉。預留項目複製期間發生任何錯誤 (例如次要區域的配額不足，或其他暫時性問題)，都會導致無法啟動軟性容錯移轉。
* 在作用中的軟體容錯移轉期間，無法更新預訂和附加的資料集，但仍可從中讀取資料。
* 在進行軟性容錯移轉時，由於資料集和預留項目路徑會暫時變更，因此在容錯移轉預留項目上執行的工作可能無法在預留項目上執行。不過，這些工作會在啟動軟性容錯移轉前和完成後使用預留項目配額。
* 如果資料集使用 BigQuery 災難復原功能，載入和擷取工作就無法使用免費共用運算單元集區。您必須建立 `PIPELINE` 類型的預訂指派，因為只有 Enterprise Plus 版支援寫入已設定的 MDR 資料集。這項規定可確保所有資料擷取作業，都由支援 MDR 跨區域複製和復原點目標 (RPO) 的專用基礎架構處理。
* [預定查詢](https://docs.cloud.google.com/bigquery/docs/scheduling-queries?hl=zh-tw)不會在容錯移轉後自動重新導向至新的主要位置，因為這類查詢會繫結至建立時指定的位置。如要在新的主要位置繼續執行預定查詢，您必須在該位置手動重新建立查詢。
...
您必須在 `ASIA`、`AU`、`CA`、`DE`、`EU`、`IN` 或 `US` 中選取區域配對。舉例來說，`US` 中的區域無法與 `EU` 中的區域配對。

如果 BigQuery 資料集位於多區域位置，您就無法使用下列區域配對。這項限制是為了確保在複寫後，容錯移轉預留項目和資料會位於不同地理位置。如要進一步瞭解多區域包含的區域，請參閱「[多區域](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#multi-regions)」一文。
...
復原時間目標 (RTO) 是指發生災難時，BigQuery 允許復原的最長目標時間。如要進一步瞭解 RTO，請參閱「[災難復原規劃基礎知識](https://docs.cloud.google.com/architecture/dr-scenarios-planning-guide?hl=zh-tw)」。啟動容錯移轉後，受管理的災難復原程序會在五分鐘內完成 RTO。由於 RTO 的關係，啟動容錯移轉程序後，次要區域的容量會在五分鐘內可用。

### 復原點目標

復原點目標 (RPO) 是指資料必須能夠還原到的最近時間點。如要進一步瞭解 RPO，請參閱[災害復原規劃基本概念](https://docs.cloud.google.com/architecture/dr-scenarios-planning-guide?hl=zh-tw)。代管的災難復原服務
會為每個資料集定義 RPO。RPO 的目標是讓次要副本與主要副本的差距在 15 分鐘內。為達到這個 RPO，您不得超過[頻寬配額](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#bandwidth_limits)，且不得發生任何使用者錯誤。

## 配額

設定容錯移轉預留項目前，次要區域必須有您選擇的運算容量。如果次要區域沒有可用配額，您就無法設定或更新預留空間。詳情請參閱「[配額與限制](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#reservations)」。

強化型複製功能頻寬有配額限制。詳情請參閱「[配額與限制](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#bandwidth_limits)」。
...
* 強化型複製：災難復原功能在複製期間會依賴強化型複製功能。系統會根據實體位元組數，以及每個複製的實體 GiB 向您收費。詳情請參閱「[強化型複製功能的資料複製資料移轉價格](https://cloud.google.com/bigquery/pricing?hl=zh-tw#data_replication)」。
* 儲存空間：次要區域的儲存空間位元組費用與主要區域相同。詳情請參閱[儲存空間定價](https://cloud.google.com/bigquery/pricing?hl=zh-tw#storage)。
...
代管災難復原機制支援 [BigQuery 彈性擴縮](https://docs.cloud.google.com/bigquery/docs/slots?hl=zh-tw#fluid-scaling)。如要確保容錯移轉後以秒計費，且沒有最短計費時間，您必須在主要和次要區域啟用 BigQuery 彈性擴縮。

## 建立或變更 Enterprise Plus 預留項目

**注意：** 建立容錯移轉預留空間前，請先確認次要區域中沒有相同名稱的預留空間。同樣地，請確保指派給容錯移轉預留位置的新指派項目，不會在次要位置重新指派相同資源和相同工作類型。這類衝突可能會導致複寫失敗，進而造成主要和次要位置不一致。這可能會導致日後無法順利執行容錯移轉作業。
...
7. 選用：在「Baseline slots」(基準運算單元) 欄位中，輸入保留項目的基準運算單元數量。
   ...
   可用自動調度資源運算單元數量的計算方式，是從「預留項目大小上限」值減去「基準運算單元」值。舉例來說，如果您建立的預留項目有 100 個基準運算單元，且預留項目大小上限為 400，則預留項目有 300 個自動調度資源運算單元。如要進一步瞭解基準運算單元，請參閱「[使用設有基準和自動調度資源運算單元的預留項目](https://docs.cloud.google.com/bigquery/docs/slots-autoscaling-intro?hl=zh-tw#using_reservations_with_baseline_and_autoscaling_slots)」。
8. 在「次要位置」清單中，選取次要位置。
...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `SECONDARY_LOCATION`：預留項目的次要[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。如果發生中斷情形，附加至這個預留項目的任何資料集都會容錯移轉至這個位置。
...
5. 依序點按
   more\_vert
   「預訂動作」和「編輯」。
6. 在「Secondary location」(次要位置) 欄位中，輸入次要位置。
7. 按一下 [儲存]。

### SQL

如要為預訂項目新增或變更次要位置，請使用 [`ALTER RESERVATION SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_reservation_set_options_statement)。
...
3. 按一下要從中卸離資料集的預留項目。
4. 按一下「Disaster recovery」(災難復原) 分頁標籤。
5. 展開資料集主要副本的「動作」選項。more\_vert
6. 按一下 [移除]。

### SQL

如要將資料集從預留位置分離，請使用 [`ALTER SCHEMA SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_schema_set_options_statement)。
...
如果發生區域服務中斷情形，您必須手動將預留項目容錯移轉至副本使用的位置。預留項目容錯移轉時，也會一併容錯移轉任何相關聯的資料集。如要手動容錯移轉預留項目，請執行下列步驟：
...
如要為預訂項目新增或變更次要位置，請使用 [`ALTER RESERVATION SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_reservation_set_options_statement)，並將 `is_primary` 設為 `TRUE`。
...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `FAILOVER_MODE`：選用參數，用於說明容錯移轉模式。可設為 `HARD` 或 `SOFT`。如未指定此參數，系統預設會使用 `HARD`。
3. 按一下「執行」play\_circle。
