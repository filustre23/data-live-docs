...
在本教學課程的這一節中，您將執行 GoogleSQL 查詢，找出 2017 年瑪麗亞颶風的移動路徑。如果要繪製該颶風的路徑，您可以查詢該颶風在不同時間點的位置。
...
2. 在「資料欄」部分，選取 `usa_wind`。
   ...
   地圖會顯示颶風在不同時間的位置，並以風速的顏色漸層標示。
3. 選用：如要調整點的顯示程度，請將「最小值」設為 0，然後從「顏色」清單中選取其他色階。
...
「樣式」部分會提供視覺化樣式清單以供您自訂。如要進一步瞭解樣式屬性和值，請參閱「[設定視覺化資料的格式](https://docs.cloud.google.com/bigquery/docs/geospatial-get-started?hl=zh-tw#format_your_visualization)」。
