...
[standard tables](/bigquery/docs/tables-intro#standard-tables), and also
...
* If you are analyzing a video, the maximum supported length is two minutes.
  If the video is longer than two minutes, `AI.GENERATE_TABLE` only returns
...
* `QUERY_STATEMENT`: the GoogleSQL query that
  ...
  + Specify a `STRUCT` value that contains one or more fields.
    ...
    **Note:** To minimize Agent Platform charges, write query
    results to a table and then reference that table in the
    `AI.GENERATE_TABLE` function. This can help you ensure that
    you are sending as few rows as possible to the model.
...
* `MAX_OUTPUT_TOKENS`: an `INT64` value that sets
  ...
  approximately 60-80 words.This value must be in the range `[1,8192]`.
  Specify a lower value for shorter responses and a higher value for
  longer responses. If you don't specify a value, the model determines an
  appropriate value.
* `TOP_P`: a `FLOAT64` value in the range `[0.0,1.0]` that
  ...
  consider C.
* `TEMPERATURE`: a `FLOAT64` value in the range `[0.0,2.0]`
  ...
  probability response is always selected. If you don't specify a value, the
  model determines an appropriate value.
...
* `SAFETY_SETTINGS`: an `ARRAY<STRUCT<STRING AS category,
  ...
  [Harm categories](/vertex-ai/generative-ai/docs/multimodal/configure-safety-filters#harm_categories)
  and
  [How to configure content filters](/vertex-ai/generative-ai/docs/multimodal/configure-safety-filters#how-to-configure-content-filters).
* `REQUEST_TYPE`: a `STRING` value that specifies the
  ...
  follows:
  + `DEDICATED`: The `AI.GENERATE_TABLE` function only uses
    Provisioned Throughput quota. The `AI.GENERATE_TABLE` function
  ...
  + `SHARED`: The `AI.GENERATE_TABLE` function only uses
  ...
  + `UNSPECIFIED`: The `AI.GENERATE_TABLE` function uses quota as follows:
    - If you haven't purchased Provisioned Throughput quota,
      the `AI.GENERATE_TABLE` function uses DSQ quota.
    - If you have purchased Provisioned Throughput quota,
      the `AI.GENERATE_TABLE` function uses the
...
[region or multi-region](/bigquery/docs/locations) as the remote model that the
function references.

You can create remote models over Gemini models in the
[supported regions](/vertex-ai/generative-ai/docs/learn/locations#available-regions)
