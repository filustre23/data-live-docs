...
* `INPUT`: a `STRING` or `STRUCT` value that specifies
  the input to classify. The input must be the first argument that you specify.
  You can provide the input value in the following ways:
...
* `CATEGORIES`: the categories by which to classify
  ...
  [define a variable](/bigquery/docs/reference/standard-sql/procedural-language)
...
* `OUTPUT_MODE`: a `STRING` value that
  ...
  + `multi`: Each value is classified into zero or more categories. In this
    case, the function returns an array that contains each relevant
    category, or an empty array if no category applies.
...
* `OPTIMIZATION_MODE`: a `STRING` value that specifies the
  ...
  + `MINIMIZE_COST` (default): uses a local, distilled model to process the
    ...
    rows to ensure enough data for model training. Note that `output_mode =>
