...
| `data_governance_tags` | `ARRAY<STRUCT<STRING, STRING>>` | Applies [data governance tags](/bigquery/docs/tags#data-governance-tags) to a column in a table.  Example: `data_governance_tags = [("myproject/tag_key", "tag_value")]` |
...
| `uris` | For external tables, including object tables, that aren't Bigtable tables:  `ARRAY<STRING>`  An array of fully qualified URIs for the external data locations. Each URI can contain one asterisk (`*`) [wildcard character](/bigquery/docs/loading-data-cloud-storage#load-wildcards) |