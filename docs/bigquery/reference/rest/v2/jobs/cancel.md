...
# Method: jobs.cancel Stay organized with collections Save and categorize content based on your preferences.
...
| JSON representation |
| --- |
| ``` {   "kind": string,   "job": {     object (Job)   } } ``` |

| Fields | |
| --- | --- |
| `kind` | `string`  The resource type of the response. |
| `job` | `object (Job)`  The final state of the job. |
