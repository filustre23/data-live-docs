...
# Method: projects.locations.reservations.list Stay organized with collections Save and categorize content based on your preferences.
...
| JSON representation |
| --- |
| ``` {   "reservations": [     {       object (Reservation)     }   ],   "nextPageToken": string } ``` |

| Fields | |
| --- | --- |
| `reservations[]` | `object (Reservation)`  List of reservations visible to the user. |
