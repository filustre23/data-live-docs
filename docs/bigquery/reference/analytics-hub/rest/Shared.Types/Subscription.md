...
# Subscription Stay organized with collections Save and categorize content based on your preferences.
...
| `linkedPubsubSubscription` | `string`  Output only. Name of the Pub/Sub subscription, e.g. projects/subscriberproject/subscriptions/subscriptions/sub\_id |
...
| Fields | |
| --- | --- |
| `cloudMarketplace` | `object (GoogleCloudMarketplaceInfo)`  Output only. This is set when the subscription is commercialised via Cloud Marketplace. |
