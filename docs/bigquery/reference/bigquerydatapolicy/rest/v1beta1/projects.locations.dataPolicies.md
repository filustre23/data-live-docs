...
| JSON representation |
| --- |
| ``` {   "name": string,   "dataPolicyType": enum (DataPolicyType),   "dataPolicyId": string,    // Union field matching_label can be only one of the following:   "policyTag": string   // End of list of possible types for union field matching_label.    // Union field policy can be only one of the following:   "dataMaskingPolicy": {     object (DataMaskingPolicy)   }   // End of list of possible types for union field policy. } ``` |
...
| `dataPolicyType` | `enum (DataPolicyType)`  Required. Data policy type. Type of data policy. |
...
| `dataMaskingPolicy` | `object (DataMaskingPolicy)`  The data masking policy that specifies the data masking rule to use. |
...
| JSON representation |
| --- |
| ``` {    // Union field masking_expression can be only one of the following:   "predefinedExpression": enum (PredefinedExpression)   // End of list of possible types for union field masking_expression. } ``` |
...
| `predefinedExpression` | `enum (PredefinedExpression)`  A predefined masking expression. |
...
| Methods | |
| --- | --- |
| `create` | Creates a new data policy under a project with the given `dataPolicyId` (used as the display name), policy tag, and data policy type. |
