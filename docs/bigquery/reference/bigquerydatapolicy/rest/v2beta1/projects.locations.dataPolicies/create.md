...
# Method: projects.locations.dataPolicies.create Stay organized with collections Save and categorize content based on your preferences.
...
| JSON representation |
| --- |
| ``` {   "dataPolicyId": string,   "dataPolicy": {     object (DataPolicy)   } } ``` |
...
| `dataPolicy` | `object (DataPolicy)`  Required. The data policy to create. The `name` field does not need to be provided for the data policy creation. |
