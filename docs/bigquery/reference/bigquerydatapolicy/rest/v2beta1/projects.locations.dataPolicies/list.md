...
# Method: projects.locations.dataPolicies.list Stay organized with collections Save and categorize content based on your preferences.
...
| JSON representation |
| --- |
| ``` {   "dataPolicies": [     {       object (DataPolicy)     }   ],   "nextPageToken": string } ``` |

| Fields | |
| --- | --- |
| `dataPolicies[]` | `object (DataPolicy)`  Data policies that belong to the requested project. |
