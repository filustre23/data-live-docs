...
8. 儲存結果後，類似下列的訊息會顯示在 Google Cloud 控制台的查詢結果下方：`Saved to Sheets as
   ...
   **附註：** bq 指令列工具或 API 均不支援將結果儲存至 Google 試算表。詳情請參閱「[使用連結試算表](https://docs.cloud.google.com/bigquery/docs/connected-sheets?hl=zh-tw)」。

### 排解無法將結果儲存到 Google 試算表的問題

將資料從 BigQuery 儲存到 Google 試算表時，您可能會發現試算表中的某些儲存格是空白的。這是因為您寫入儲存格的資料超過 Google 試算表的 50,000 個字元限制。如要解決這個問題，請在 GoogleSQL 查詢中使用[字串函式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/string_functions?hl=zh-tw#split)，將含有長資料的資料欄分割成兩或多個資料欄，然後再次將結果儲存到試算表。
...
* 設定 [VPC Service Controls](https://docs.cloud.google.com/vpc-service-controls/docs/overview?hl=zh-tw) perimeter，防止資料竊取。這項設定會禁止使用者在已建立的 perimeter 邊界外下載及匯出資料。
* 請與 [Cloud Customer Care](https://docs.cloud.google.com/support?hl=zh-tw)聯絡，要求將專案或機構加入限制清單。這樣一來，您就無法直接在控制台中下載及匯出資料。 Google Cloud Google Cloud
