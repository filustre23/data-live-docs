...
BigQuery 不支援將巢狀或重複資料 (`RECORD` / `STRUCT` 或 `ARRAY` 資料類型) 匯出為 CSV 格式。即使 `RECORD` 或 `STRUCT` 欄位看起來並非深層巢狀結構，BigQuery 仍會將其視為巢狀結構定義欄位，並傳回錯誤，指出結構定義無法巢狀化。

如要將查詢結果匯出為結構化 CSV 檔案，查詢必須正規化為平面結構，每個查詢欄位對應一個欄位。您可以使用點記號選取個別欄位，藉此正規化巢狀結構體，也可以使用 `UNNEST` 運算子正規化重複陣列。

舉例來說，如果資料表包含結構體 `customer` 和陣列 `items`，請先正規化查詢，再執行查詢並將結果儲存為 CSV：
...
4. 選用：如要變更處理位置，請依序點選「編輯」>「查詢設定」。針對「Data location」(資料位置)，選擇資料的[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。
5. 按一下「執行」。
...
嘗試將 BigQuery 結果儲存至 Google 雲端硬碟時，可能會發生錯誤。這個錯誤是因為 Drive SDK API 無法存取 Google Workspace 所致。如要解決這個問題，您必須啟用使用者帳戶，允許透過 Drive SDK API [存取 Google 雲端硬碟](https://support.google.com/a/answer/6105699?hl=zh-tw)。
...
將資料從 BigQuery 儲存到 Google 試算表時，您可能會發現試算表中的某些儲存格是空白的。當您寫入儲存格的資料超過 Google 試算表的 50,000 個字元限制時，就會發生這種情況。如要解決這個問題，請在 GoogleSQL 查詢中使用[字串函式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/string_functions?hl=zh-tw#split)，將含有長資料的資料欄分割成兩或多個資料欄，然後將結果再次儲存至試算表。
...
* 設定 [VPC Service Controls](https://docs.cloud.google.com/vpc-service-controls/docs/overview?hl=zh-tw) perimeter，防止資料遭竊。這樣一來，使用者就無法在已建立的範圍邊界外下載及匯出資料。
* 請與 [Cloud Customer Care](https://docs.cloud.google.com/support?hl=zh-tw) 團隊聯絡，要求將您的專案或機構加入限制清單。 Google Cloud 這會直接在 Google Cloud 控制台中停用資料下載和匯出選項。
