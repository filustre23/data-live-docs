...
您可以使用 BigQuery 資料移轉服務中的 Apache Hive Metastore 移轉連接器，大規模地將 Hive Metastore 資料表無縫移轉至 Google Cloud 。這個連接器支援來自地端安裝和雲端環境 (包括 Cloudera 設定) 的 Hive 和 Iceberg 資料表。Hive Metastore 移轉連接器支援儲存在下列資料來源中的檔案：
...
* [Lakehouse 執行階段目錄 Hive 目錄](https://docs.cloud.google.com/lakehouse/docs/about-spark-hive-metastore?hl=zh-tw) ([預先發布版](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))

  建議您為所有 Hive 資料表使用 Lakehouse 執行階段目錄 Hive 目錄。

  您可以使用 Lakehouse 執行階段目錄 Hive 目錄，透過 Hive 目錄向 [Lakehouse 執行階段目錄](https://docs.cloud.google.com/lakehouse/docs/about-spark-hive-metastore?hl=zh-tw)註冊已遷移的 Hive 資料表。這項功能提供 Apache Hive 資料表的無伺服器 metastore。除了 Apache Spark 和其他 OSS 引擎外，您也可以使用 BigQuery 查詢資料 (受格式限制)。
...
這個連結器支援完整移轉和僅移轉中繼資料。完整移轉會將來源資料表中的資料和中繼資料移轉至目標 Metastore。如果您已將資料儲存在 Cloud Storage 中，且只想將資料註冊至目的地 Metastore，可以建立僅移轉中繼資料的作業。
...
* 如果資料在移轉期間於來源端變更，Storage 移轉服務會有特定行為。我們不建議在資料表移轉期間寫入資料表。如要查看其他 Storage 移轉服務限制，請參閱[已知限制](https://docs.cloud.google.com/storage-transfer/docs/known-limitations-transfer?hl=zh-tw)。
...
* 開放原始碼中繼資料格式和資料類型的所有 Lakehouse 執行階段目錄 Hive 目錄限制都適用。
...
如果移轉設定採用週期性時間表，後續每次移轉都會更新 Google Cloud 中的資料表，反映來源資料表的最新變更。舉例來說，所有資料更新，以及所有插入、刪除或更新作業 (含結構定義變更) 都會反映在 Google Cloud 中，每次轉移都會更新。
...
* **`IN`**：指定要納入的確切分割區路徑。`partitions` 陣列包含代表分割區確切目錄結構的字串，相對於表格基本路徑 (例如 `["partition_key1=value1/partition_key2=value2"]`)。您可以在陣列中指定多個路徑。
...
* **包含的值：**`GREATER_THAN`、`LESS_THAN` 和 `RANGE` 的篩選條件包含提供的值。舉例來說，如果篩選器值為 `2023`，則會包含 `2023` 之前 (含 `2023`) 的所有分區。`LESS_THAN`
* **刪除分區：**如果現有的目的地分區符合分區篩選器，但來源中已不再存在，系統就會從目的地中繼存放區捨棄該分區。不過，該資料分割區的基礎資料檔案不會從 Cloud Storage 目的地 bucket 刪除。
* **單一表格限制：**
  + 同一個表格中不得包含多個篩選器。
  + 您無法在同一張表格中混用不同類型的條件 (例如：`GREATER_THAN` 和 `IN`)。
* **目標分區資料欄：**`GREATER_THAN`、`LESS_THAN` 和 `RANGE` 等篩選條件必須以主要分區資料欄為目標。
* **前置字元限制：**指定的篩選條件組合不得解析為每個資料表超過 1000 個前置字元。舉例來說，在以 `year/month/day` 分割的資料表上使用 `year>2020` 等篩選條件時，產生的不重複 `year=` 前置字元不得超過 1000 個。
...
2. 啟用 Data Transfer API 時，系統會建立[服務代理程式](https://docs.cloud.google.com/bigquery/docs/enable-transfer-service?hl=zh-tw#service_agent) (P4SA)。
   ...
   **重要事項：**您必須將這些角色授予服務代理程式，*而非*使用者帳戶。如果未將角色授予正確的主體，可能會導致權限錯誤。
...
1. 為 Microsoft Azure 儲存體帳戶[產生共用存取簽章 (SAS) 權杖](https://docs.cloud.google.com/storage-transfer/docs/source-microsoft-azure?hl=zh-tw#sas-token)。
2. 產生 SAS 權杖後，請記下該權杖。
...
7. 在「Data source details」(資料來源詳細資料) 部分執行下列操作：
   ...
   1. 在「轉移策略」中，選取下列任一選項：
      ...
      * `METADATA_ONLY`：僅註冊中繼資料。您必須已在正確的 Cloud Storage 位置中提供資料，中繼資料會參照該位置。
   ...
   3. **在「BQMS discovery dump gcs path」(BQMS 探索傾印 GCS 路徑) 欄位中，輸入您在[為 Apache Hive 建立中繼資料檔案](#generate-metadata-dump-for-apache-hive)時產生的 `hive-dumper-output.zip` 檔案路徑。如果您使用 [dumper 輸出自動化功能搭配 `cron`](#automate-dumper)，請提供在 `--gcs-base-path` 中設定的 Cloud Storage 資料夾路徑，其中包含 dumper 輸出 ZIP 檔案。**- 在「儲存空間類型」部分，選取下列其中一個選項。只有在「轉移策略」設為「`FULL_TRANSFER`」時，這個欄位才會顯示：
      - `HDFS`：如果檔案儲存空間是 `HDFS`，請選取這個選項。在「STS 代理程式集區名稱」欄位中，您必須提供[設定 Storage Transfer Service 代理程式](#configure-sts)時建立的代理程式集區名稱。
      - `S3`：如果檔案儲存空間是 `Amazon S3`，請選取這個選項。在「Access key ID」(存取金鑰 ID) 和「Secret access key」(存取密鑰) 欄位中，您必須提供在[設定存取憑證](#configure-sts)時建立的存取金鑰 ID 和存取密鑰。
      - `AZURE`：如果檔案儲存空間是 `Azure Blob Storage`，請選取這個選項。在「SAS token」(SAS 權杖) 欄位中，您必須提供[設定存取憑證](#configure-sts)時建立的 SAS 權杖。
8. 選用：在「Partition Filter gcs path」(分區篩選器 GCS 路徑) 欄位中，輸入自訂篩選器 JSON 檔案的完整 Cloud Storage 路徑，以[篩選來源資料表中的分區](#filter-partitions)。
9. 在**「Destination gcs path」(目的地 GCS 路徑)** 中，輸入 Cloud Storage bucket 的路徑，以儲存遷移的資料。
10. 從下拉式清單中選擇目的地 Metastore 類型：
    * `DATAPROC_METASTORE`：選取這個選項，將中繼資料儲存在 [Dataproc Metastore](https://docs.cloud.google.com/dataproc-metastore/docs/overview?hl=zh-tw) 中。您必須在「Dataproc metastore url」中提供 Dataproc Metastore 的網址。
    * `BIGLAKE_REST_CATALOG`：選取這個選項，將中繼資料儲存在 Lakehouse 執行階段目錄 Iceberg REST 目錄中。系統會根據目的地 Cloud Storage bucket 建立目錄。
    * `BIGLAKE_HIVE_CATALOG` ([預覽版](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))：選取這個選項，即可將中繼資料儲存在 Lakehouse 執行階段目錄 Hive 目錄中。您必須在 **BigLake Metastore Hive Catalog ID** 中提供目錄名稱。如果目錄不存在，系統會自動建立。
11. 選用：針對「服務帳戶」，輸入要用於這項資料移轉作業的服務帳戶。服務帳戶應屬於移轉設定和目的地資料集建立所在的Google Cloud 專案。
...
* `SERVICE_ACCOUNT`：用於建立轉移作業的服務帳戶名稱。服務帳戶應屬於同一個Google Cloud 專案，該專案中已建立轉移設定和目的地資料集。
...
* `TRANSFER_STRATEGY`：(選用) 指定下列其中一個值：
  ...
  + `METADATA_ONLY`：僅註冊中繼資料。您必須已在正確的 Cloud Storage 位置中提供資料，並在中繼資料中參照該位置。
* `LIST_OF_TABLES`：要轉移的實體清單。請使用階層式命名規格 - `database.table`。這個欄位支援 RE2 規則運算式，可指定資料表。例如：
  + `db1..*`：指定資料庫中的所有資料表
  + `db1.table1;db2.table2`：資料表清單
* `DUMPER_BUCKET`：包含 `hive-dumper-output.zip` 檔案的 Cloud Storage 值區。如果您使用 `cron` 搭配 [dumper 輸出自動化功能，請將 `table_metadata_path` 變更為使用 cron 設定 `--gcs-base-path` 時設定的 Cloud Storage 資料夾路徑，例如：`"table_metadata_path":"<var>GCS_PATH_TO_UPLOAD_DUMPER_OUTPUT</var>"`。](#automate-dumper)
...
* `METASTORE`：要遷移的 Metastore 類型。請將此值設為下列其中一個值：
  ...
  + `BIGLAKE_HIVE_CATALOG`：將中繼資料轉移至 Lakehouse 執行階段目錄
    Hive 目錄 (建議用於 Apache Hive 表格) ([預先發布版](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))。
* `DATAPROC_METASTORE_URL`：Dataproc Metastore 的網址。如果 `metastore` 為 `DATAPROC_METASTORE`，則為必填欄位。
* `HIVE_CATALOG_ID`：Lakehouse 執行階段目錄 Hive 目錄的 ID。如果 `metastore` 為 `BIGLAKE_HIVE_CATALOG`，則為必填。如果目錄不存在，系統會自動建立。
* `STORAGE_TYPE`：指定資料表的基礎檔案儲存空間。支援的類型為 `HDFS`、`S3` 和 `AZURE`。如果 `transfer_strategy` 為 `FULL_TRANSFER`，則為必填。
* `AGENT_POOL_NAME`：用於建立代理程式的代理程式集區名稱。如果 `storage_type` 為 `HDFS`，則為必填。
...
* `AZURE_SAS_TOKEN`：來自[存取憑證](#configure-sts)的 SAS 權杖。如果 `storage_type` 為 `AZURE`，則為必要。
...
3. 如要透過 Google Cloud 驗證，允許 `gsutil` 將檔案上傳至 Cloud Storage，請執行下列指令：
...
1. 將下列指令碼儲存至本機檔案。這個指令碼的設計用途是透過 `cron` Daemon 進行設定和執行，自動擷取及上傳傾印輸出內容。
   ...
   if [[ "$DUMPER_EXECUTABLE" == "DUMPER_PATH"* || "$GCS_BASE_PATH" == "gs://PATH_TO_DUMPER_OUTPUT" || "$LOCAL_BASE_DIR" == "LOCAL_BASE_DIRECTORY_PATH"
   ```