...
* [呼叫 Spark 預存程序](#call-spark-procedure)：
  + 資料集的 [BigQuery 中繼資料檢視者](https://docs.cloud.google.com/iam/docs/roles-permissions/bigquery?hl=zh-tw#bigquery.metadataViewer)  (`roles/bigquery.metadataViewer`)
    預存程序儲存位置
...
如有需要，您可以授予連線的服務帳戶存取 Dataproc Metastore 和 Managed Service for Apache Spark 持續性記錄伺服器的權限 (如果您已在連線中指定這些服務)。詳情請參閱「[授予服務帳戶存取權](https://docs.cloud.google.com/bigquery/docs/connect-to-spark?hl=zh-tw#grant-access)」。
...
* 如要讀取及寫入做為預設 Spark 應用程式檔案系統的暫存 bucket，請按照下列步驟操作：
  ...
  + 權限，或指定暫存值區的 `roles/storage.objectAdmin` IAM 角色。`storage.objects.*`
