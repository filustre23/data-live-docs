...
下列 Python 程式碼範例會實作遠端函式的所有整數引數加法。這個函式會處理要求，其中包含批次呼叫的引數，並在回應中傳回所有結果。
...
下列 Python 程式碼範例會實作網路服務，可建構及部署至 Cloud Run，以提供相同功能。
...
1. 啟用必要 API，並確認您已獲派必要角色，詳情請參閱「[遠端函式](https://docs.cloud.google.com/bigquery/docs/dataframes-custom-python-functions?hl=zh-tw#remote-function-requirements)」的「**必要條件**」一節。
...
無論是單一區域或多區域位置，您都只能在與所用連線相同的位置建立遠端函式。舉例來說，如要在 `US` 多區域中建立遠端函式，請使用位於 `US` 多區域的連線。
