...
* 「區域」是特定的地理位置，例如倫敦。
* 「多區域」是指包含許多獨立區域的大型地理區域，例如美國或歐洲。多區域位置提供的配額比單一區域大，但多區域位置不會提供區域備援。資料會儲存在單一區域，且運算資源僅在該區域內提供。如要跨區域備援，BigQuery 提供[代管災難復原](https://docs.cloud.google.com/bigquery/docs/managed-disaster-recovery?hl=zh-tw)功能。

無論是哪種位置類型，BigQuery 都會自動將資料副本儲存在所選位置的單一區域內，兩個不同的可用區。多區域與其他區域視為不同，即使位於同一可用區也是如此。如要進一步瞭解資料可用性和耐久性，請參閱[災害復原規劃](https://docs.cloud.google.com/bigquery/docs/reliability-intro?hl=zh-tw#disaster_planning)。
...
1 位於 `EU` 多區域的資料只會儲存在下列其中一個位置：`europe-west1` (比利時) 或 `europe-west4` (荷蘭)。資料的儲存和處理位置由 BigQuery 自動決定。

2 位於 `US` 多區域的資料只會儲存在下列其中一個位置：`us-central1` (愛荷華州)、`us-west1` (奧勒岡州) 或 `us-central2` (奧克拉荷馬州)。資料的儲存和處理位置由 BigQuery 自動決定。

## BigQuery Studio 程式碼資產位置
...
下表列出透過 Cloud AI 服務支援遠端模型的地區，以及部署至 Agent Platform 的自訂模型。資料欄名稱會指出遠端模型的類型。

|  | 區域說明 | 區域名稱 | Vertex AI 部署模型 | Cloud Natural Language API | Cloud Translation API | Cloud Vision API | Document AI API | Speech-to-Text API |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **美洲** | | | | | | | | |
|  | 俄亥俄州哥倫布 | `us-east5` |  |  |  |  |  |  |
|  | 達拉斯 | `us-south1` | ● |  |  |  |  |  |
...
* `US` 和 `EU` 多區域支援 Gemini 模型。
* `US` 多區域的 Claude、Llama 和 Mistral AI 模型可使用 Agent Platform 端點，存取 `US` 多區域內的任何單一區域。`EU` 多區域中的 Claude、Llama 和 Mistral AI 模型可使用 Agent Platform 端點，適用於 `EU` 多區域內的任何單一區域，但 `eu-west2` 和 `eu-west6` 除外。
...
如果您要建立遠端模型的資料集位於多區域，則 Agent Platform 模型端點必須位於該多區域內的區域。舉例來說，如果資料集位於 `eu` 多區域，則可以指定 `europe-west1` 區域端點的網址 `https://europe-west1-aiplatform.googleapis.com/v1/projects/myproject/locations/europe-west1/publishers/google/models/<target_model>`。如果您指定模型名稱而非端點網址，BigQuery ML 會預設使用 `europe-west4` 端點處理 `eu` 多區域的資料集，並使用 `us-central1` 端點處理 `us` 多區域的資料集。
...
全域端點涵蓋全球，與單一區域相比，可用性和可靠性更高。使用全域端點發出要求可提升整體可用性，同時減少資源耗盡 (429) 錯誤，這類錯誤會在您超出區域端點配額時發生。如要在 Gemini 2.0 以上版本未開放的區域使用，可以改用全域端點，避免將資料遷移至其他區域。您只能透過 `AI.GENERATE_TEXT` 函式使用部署至全域端點的模型。

如果您對資料處理位置有要求，請勿使用全域端點，因為使用全域端點時，您無法控管或瞭解處理要求的區域。

#### Google 模型和合作夥伴模型的處理位置

如要瞭解 Agent Platform 代管的 Google 模型所用的處理位置，請參閱「[模型適用的機器學習處理 Google Cloud 」。這項資訊涵蓋部署至區域或多區域的模型。使用全域端點的模型無法保證任何特定處理位置。](https://docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/data-residency?hl=zh-tw#ml-processing-google-models)
...
本節說明支援的[模型](https://docs.cloud.google.com/bigquery/docs/bqml-introduction?hl=zh-tw#supported_models)位置 (遠端模型除外)，以及模型處理作業的發生位置。  

#### 地區位置

下表列出所有模型類型 (遠端模型除外) 支援的位置：

|  | 區域說明 | 區域名稱 | 匯入的模型 | 內建 模型 訓練 | DNN/自動編碼器/ 增強型樹狀結構/ 廣而深模型 訓練 | AutoML 模型 訓練 | 超參數 調整 | 整合 Vertex AI Model Registry |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **美洲** | | | | | | | | |
|  | 俄亥俄州哥倫布 | `us-east5` | ● | ● |  |  |  |  |
|  | 達拉斯 | `us-south1` | ● | ● |  |  |  |  |  |
...
`US` 和 `EU` 多重區域支援所有支援的模型 (遠端模型除外)。

位於 `EU` 多區域的資料不會儲存在 `europe-west2` (倫敦) 或 `europe-west6` (蘇黎世) 資料中心。

Vertex AI Model Registry 整合功能僅支援單一區域整合。如果您將多區域 BigQuery ML 模型傳送至 Model Registry，該模型會在 Vertex AI 中轉換為區域模型。BigQuery ML 美國多區域模型會同步至 Agent Platform `us-central1`，而 BigQuery ML 歐盟多區域模型會同步至 Agent Platform `europe-west4`。單一區域模型則不會有任何變更。

#### 處理位置

如果是遠端模型以外的模型，BigQuery ML 會在與包含資料的資料集相同的位置中處理及暫存資料。
...
如果未指定位置，查詢用於判斷 BigQuery 中的處理位置時，可能會暫時儲存在 BigQuery 路由器記錄中。

如果[專案](https://docs.cloud.google.com/bigquery/docs/resource-hierarchy?hl=zh-tw#projects)在 `US` 以外的區域有以容量為準的預訂方案，且查詢未參考資料集內的任何資料表或其他資源，提交工作時就必須明確指定以容量為準的預訂方案位置。以容量為準的承諾與位置 (例如 `US` 或 `EU`) 相關聯。如果工作在容量位置以外的位置執行，該工作的價格會自動改為隨選價格。
...
* 使用 bq 指令列工具時，請提供 `--location`
  [通用旗標](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#global_flags)，然後將值設為您的位置。
...
單一區域位置與多區域位置不符，即使單一區域位置包含在多區域位置中也是如此。因此，如果位置同時包含單一區域位置和多區域位置，查詢就會以[全域查詢](https://docs.cloud.google.com/bigquery/docs/global-queries?hl=zh-tw)的形式執行。舉例來說，如果工作位置設為 `US`，但工作參照 `us-central1` 中的資料集，則工作會是全域查詢。同樣地，如果工作參照 `US` 中的一個資料集，以及 `us-central1` 中的另一個資料集，就會成為全域查詢。如果 `JOIN` 陳述式中的資料表同時位於區域和多區域，也會發生這種情況。
...
* 要求中參考的資料集位置。舉例來說，如果查詢參考了儲存在 `asia-northeast1` 區域資料集中的資料表或檢視區塊，查詢工作就會在 `asia-northeast1` 中執行。
* 要求中參照的連線所指定的區域。
* 目的地資料表的位置。
...
容量承諾是區域性資源。購買運算單元時，這些運算單元僅限用於特定區域或多區域。如果您的容量承諾僅位於 `EU`，就無法在 `US` 中建立預留項目。建立預留項目時，您會指定位置 (區域) 和運算單元數量。這些運算單元會從該區域的容量承諾中提取。

同樣地，在某個區域執行工作時，只有在工作位置與預留項目位置相符時，才會使用預留項目，除非工作是[全域查詢](https://docs.cloud.google.com/bigquery/docs/global-queries?hl=zh-tw)。舉例來說，如果您將預留項目指派給 `EU` 中的專案，並在該專案中對位於 `US` 的資料集執行查詢，則該查詢不會在 `EU` 預留項目中執行。如果沒有任何 `US` 預留項目，工作就會以隨選模式執行。
...
* [單一地區值區](https://docs.cloud.google.com/storage/docs/locations?hl=zh-tw#location-r)：如果您的 BigQuery 資料集位於華沙 (`europe-central2`) 地區，對應的 Cloud Storage 值區也必須位於華沙地區，或是包含華沙的任何 Cloud Storage 雙重地區。如果您的 BigQuery 資料集位於`US`多區域`us-central1`，則 Cloud Storage bucket 可以位於愛荷華州單一區域，或任何包含愛荷華州的雙區域。即使值區位於資料集多區域內的位置，從任何其他單一區域發出的查詢仍會產生資料移轉費用。舉例來說，如果外部資料表位於 `US` 多區域，而 Cloud Storage bucket 位於奧勒岡 (`us-west1`)，則這項工作會產生資料移轉費用。

  如果您的 BigQuery 資料集位於`EU`多區域`europe-west4`，則 Cloud Storage bucket 可以位於荷蘭 (`europe-west4`) 單一區域，或任何包含荷蘭 (`europe-west4`) 的雙區域。即使 bucket 位於資料集多區域內的位置，從任何其他單一區域發出的查詢仍會產生資料移轉費用。舉例來說，如果外部資料表位於 `EU` 多區域，而 Cloud Storage bucket 位於華沙 (`europe-central2`)，則這項工作會產生資料移轉費用。
* [雙地區值區](https://docs.cloud.google.com/storage/docs/locations?hl=zh-tw#location-dr)：如果您的 BigQuery 資料集位於東京 (`asia-northeast1`) 地區，對應的 Cloud Storage 值區必須位於東京地區，或位於包含東京的雙地區，例如 `ASIA1` 雙地區。
  ...
  如果 Cloud Storage 值區位於`EUR4`雙地區，或包含荷蘭 (`europe-west4`) 地區的任何雙地區，對應的 BigQuery 資料集可以位於`EU`多地區或荷蘭 (`europe-west4`)。
* [多區域 bucket](https://docs.cloud.google.com/storage/docs/locations?hl=zh-tw#location-mr)：使用多區域資料集位置和多區域 Cloud Storage bucket 時，**不**建議搭配外部資料表，因為外部查詢效能取決於最低延遲和最佳網路頻寬。

  如果 BigQuery 資料集位於`US`多區域，對應的 Cloud Storage bucket 必須位於包含愛荷華州 (`us-central1`) 的雙區域，例如 `NAM4`雙區域，或是包含愛荷華州 (`us-central1`) 的自訂雙區域。

  如果您的 BigQuery 資料集位於`EU`多區域，對應的 Cloud Storage bucket 必須位於包含荷蘭 (`europe-west4`) 的雙重區域，例如`EUR4`雙重區域，或是包含荷蘭 (`europe-west4`) 的自訂雙重區域。
...
如要進一步瞭解載入資料的傳輸費用，請參閱「[查詢 Cloud Storage 資料](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#query-storage-data-location)」一節，因為這兩者適用相同的指南。
...
* 單一區域：如果 BigQuery 資料集位於比利時 (`europe-west1`) 區域位置，對應的 Bigtable 執行個體必須位於比利時區域。
...
* 多區域：如果 BigQuery 資料集位於 `US` 多區域，對應的 Cloud SQL 執行個體必須位於美國地理區域的單一區域。
...
您可以使用[機構政策服務](https://docs.cloud.google.com/resource-manager/docs/organization-policy/overview?hl=zh-tw)，限制可建立資料集的地區。詳情請參閱「[限制資源位置](https://docs.cloud.google.com/resource-manager/docs/organization-policy/defining-locations?hl=zh-tw)」和「[支援資源位置的服務](https://docs.cloud.google.com/resource-manager/docs/organization-policy/defining-locations-supported-services?hl=zh-tw#bigquery)」。

## 資料集安全性

如要控管 BigQuery 資料集的存取權，請參閱「[控管資料集存取權](https://docs.cloud.google.com/bigquery/docs/control-access-to-resources-iam?hl=zh-tw)」。如要瞭解資料加密，請參閱「[靜態資料加密](https://docs.cloud.google.com/bigquery/docs/encryption-at-rest?hl=zh-tw)」。
...
* [查看我們在世界各地提供的所有服務](https://docs.cloud.google.com/about/locations?hl=zh-tw#region)。 Google Cloud
