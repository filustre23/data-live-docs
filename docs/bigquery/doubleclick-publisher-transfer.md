...
* **確認貴機構可存取 Google Ad Manager 資料移轉 (Google Ad Manager DT) 檔案。**這些檔案會由 Google Ad Manager 小組傳送至 Cloud Storage 值區。如要取得 Google Ad Manager DT 檔案的存取權，請參閱「[Ad Manager 資料移轉報表](https://support.google.com/admanager/answer/1733124?hl=zh-tw)」。Google Ad Manager 小組可能會向您收取額外費用。
