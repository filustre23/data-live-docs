...
# WRITE\_API\_TIMELINE\_BY\_ORGANIZATION 檢視畫面

「`INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION`」檢視畫面會顯示與目前專案相關聯的整個機構，每分鐘的匯總串流統計資料。
...
如要查詢 `INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION` 檢視畫面，您需要具備該機構的 `bigquery.tables.list` Identity and Access Management (IAM) 權限。
...
| 檢視表名稱 | 資源範圍 | 區域範圍 |
| --- | --- | --- |
| `` [PROJECT_ID.]`region-REGION`.INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION `` | 包含指定專案的機構 | REGION |
...
* 如要查詢歐盟多區域的資料，請使用 `` `region-eu`.INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_ORGANIZATION ``
...
以下範例會計算專案機構中，每分鐘成功和失敗的附加要求，並依錯誤代碼類別分類。這項查詢可用於填入資訊主頁。
...
以下範例會傳回專案機構中，傳入流量最多的 10 個資料表的 BigQuery Storage Write API 擷取統計資料：
