...
BigQuery 會根據可用的運算資源，動態決定可同時執行的查詢數量。系統會根據隨選專案或[預訂](https://docs.cloud.google.com/bigquery/docs/reservations-intro?hl=zh-tw)，計算可同時執行的查詢數量。其他查詢會排入佇列，直到有足夠容量可供執行為止。每個專案在每個區域的佇列長度上限為 1,000 個互動式查詢和 20,000 個批次查詢，無論專案是隨選還是使用預訂皆適用。以下範例顯示當計算出的查詢並行數為 202 時，隨選專案的行為：
...
系統會優先將並行量最低專案的查詢從佇列中移除。執行期間，系統會先將運算單元平均分配給各專案，然後再分配給專案中的工作。

舉例來說，假設您有一個指派給兩個專案的預訂：A 和 B。BigQuery 會計算預留項目的並行數，結果為 5。專案 A 有四項查詢正在同步執行，專案 B 有一項查詢正在執行，其他查詢則在佇列中等待。即使專案 B 的查詢是在專案 A 的查詢之後提交，系統仍會先將專案 B 的查詢從佇列中移除。查詢開始執行後，會獲得共用預留位置的公平分配。

除了並行查詢總數之外，BigQuery 還會動態決定每個隨選專案或預訂可執行的並行批次查詢數量上限。如果同時執行的批次查詢數量達到上限，即使互動式查詢是稍後提交，也會優先執行。

刪除預留項目後，所有已排入佇列的查詢都會逾時。如果指派給預留項目的專案重新指派給其他預留項目，所有已加入佇列或正在執行的要求會繼續在舊的預留項目中執行，而所有新要求則會前往新的預留項目。從保留項目移除指派給保留項目的專案後，執行中的查詢會繼續使用保留項目，而新的要求和佇列要求則會使用隨選模式執行。您可以視需要[取消](https://docs.cloud.google.com/bigquery/docs/managing-jobs?hl=zh-tw#cancel_jobs)個別執行中或已加入佇列的查詢作業。
...
8. 如要設定目標工作並行，請將「覆寫自動目標工作並行」切換鈕設為開啟，然後輸入「目標工作並行」。
9. 按一下 [儲存]。
...
您可以使用 [BigQuery 管理資源圖表](https://docs.cloud.google.com/bigquery/docs/admin-resource-charts?hl=zh-tw#view-resource-utilization)監控預留項目的查詢佇列長度，並選取「待處理」指標的「工作並行」圖表。
...
查詢作業可能看起來很慢，因為在開始執行前，作業會在佇列中等待相當長的時間。這段時間就是*佇列時間*。
...
* **`INFORMATION_SCHEMA`**：
  ...
  可提供查詢佇列時間的深入分析。舉例來說，您可以從 `start_time` 減去 `creation_time`，計算特定工作的佇列時間，或是找出 `state="PENDING"` 狀態持續時間較長的工作。您可以從[範例查詢](#monitoring)著手，觀察查詢負載，並找出何時達到動態並行門檻。
* **監控**：
...
* **並行限制**：專案或預留項目已達到允許的並行查詢數上限，該上限是動態決定或[手動設定](#set_the_maximum_concurrency_target)。
  達到上限後，新查詢就必須等待。
* **運算單元爭用**：可用運算單元不足，無法處理工作負載。這會導致執行中的查詢速度變慢，佔用並行查詢運算單元的時間變長，其他查詢的佇列時間也會增加。
* **工作負載尖峰**：提交的查詢數量突然增加。
* **查詢效率不佳**：個別查詢未經過最佳化。即使查詢量偏低，這些效率不彰的查詢仍可能長時間耗用過多時段，導致有效查詢的周轉率降低。這樣一來，系統就不會啟動新的查詢，而是強制查詢在佇列中等待。
...
* 每個隨選專案一次最多可將 1,000 個互動式查詢和 20,000 個批次查詢加入佇列。如果查詢超出這項限制，系統會傳回配額錯誤。這些限制無法提高。
* 在預留項目中，指派給該預留項目的每個專案一次最多可將 1,000 個互動式查詢和 20,000 個批次查詢排入佇列。如果查詢超出這項限制，系統會傳回配額錯誤。這些限制無法提高。
* 根據預設，如果查詢工作未開始執行，互動式查詢會在 6 小時後逾時，批次查詢則會在 24 小時後逾時。
* 您無法為在隨需專案中執行的查詢設定並行數上限目標。
* 您無法為使用 Standard 版預訂執行的查詢設定並行上限目標。如要進一步瞭解版本，請參閱「[BigQuery 版本簡介](https://docs.cloud.google.com/bigquery/docs/editions-intro?hl=zh-tw)」。
