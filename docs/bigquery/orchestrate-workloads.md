...
[Vertex AI Pipelines](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/pipelines/introduction?hl=zh-tw) 是以 Kubeflow Pipelines 為基礎的無伺服器工具，專為排定機器學習工作負載而設計。從訓練資料到程式碼，這項服務會自動執行並連結模型開發和部署的所有工作，讓您全面瞭解模型的運作方式。如要進一步瞭解如何搭配使用 Vertex AI Pipelines 與 BigQuery，請參閱「[匯出及部署 BigQuery 機器學習模型，以進行預測](https://codelabs.developers.google.com/codelabs/bqml-vertex-prediction?hl=zh-tw#0)」。
...
* 開始使用 [Vertex AI Pipelines](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/pipelines/introduction?hl=zh-tw)。
