...
您可以管理 BigQuery 資料表，控管資料表的生命週期、盡量降低儲存空間費用，以及確保資料治理。資料表管理包括更新屬性、重新命名、複製及刪除資料表等工作。更新 BigQuery 資料表屬性時，您可以修改資料表的到期時間、說明、結構定義、標籤和預設捨入模式。

如要還原已刪除的資料表，請參閱「[還原已刪除的資料表](https://docs.cloud.google.com/bigquery/docs/restore-deleted-tables?hl=zh-tw)」。
如要進一步瞭解如何建立及使用資料表，包括取得資料表資訊、列出資料表，以及控管資料表資料的存取權等，請參閱「[建立及使用資料表](https://docs.cloud.google.com/bigquery/docs/tables?hl=zh-tw)」。
...
4. 點選「詳細資料」分頁標籤，然後點選「編輯詳細資料」。
5. 在「說明」部分新增或編輯說明。
6. 按一下 [儲存]。

### SQL

請使用 [`ALTER TABLE SET OPTIONS` 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_table_set_options_statement)。以下範例會更新名為 `mytable` 的資料表說明：
...
2. 發出含有 `--description` 旗標的 `bq update` 指令。如果您要更新非預設專案中的資料表，請使用下列格式將專案 ID 新增至資料集名稱：`project_id:dataset`。
   ...
   * `table`：要更新的資料表名稱
   ...
   如要將 `mydataset` 資料集中的 `mytable` 資料表說明變更為「mytable 的說明」，請輸入下列指令。`mydataset` 資料集位於 `myotherproject` 專案，而非預設專案。
...
如要進一步瞭解資料洞察，包括設定步驟、必要的身分與存取權管理角色，以及提升所產生洞察準確度的最佳做法，請參閱「[在 BigQuery 中產生資料洞察](https://docs.cloud.google.com/bigquery/docs/data-insights?hl=zh-tw)」一文。
...
5. 點按「生成」。
   ...
   **注意：** 如果沒有看到「產生」按鈕，請點選「描述資料」。
   你可能需要捲動畫面才能看到這個按鈕。

   Gemini 會生成資料表說明和資料表洞察結果。系統需要幾分鐘的時間才能填入資訊。您可以在表格的「洞察」分頁中查看生成的洞察資料。
...
資料表過期時，系統會一併刪除資料表和當中的所有資料。如有需要，您可以在資料集指定的時間旅行視窗內取消刪除過期的資料表，詳情請參閱「[還原已刪除的資料表](https://docs.cloud.google.com/bigquery/docs/restore-deleted-tables?hl=zh-tw)」。
...
請使用 [`ALTER TABLE SET OPTIONS` 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_table_set_options_statement)。以下範例會更新名為 `mytable` 的資料表到期時間：
...
2. 發出含有 `--expiration` 旗標的 `bq update` 指令。如果您要更新非預設專案中的資料表，請使用下列格式將專案 ID 新增至資料集名稱：`project_id:dataset`。
   ...
   * `table`：要更新的資料表名稱。
   ...
   如要將 `mydataset` 資料集中的 `mytable` 資料表到期時間更新為 5 天 (432000 秒)，請輸入下列指令。`mydataset` 資料集位於 `myotherproject` 專案中，而非預設專案。
...
您可以使用 [`ALTER TABLE SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_table_set_options_statement)，更新資料表的[預設捨入模式](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tables?hl=zh-tw#Table.FIELDS.default_rounding_mode)。以下範例會將 `mytable` 的預設捨入模式更新為 `ROUND_HALF_EVEN`：
...
將 `NUMERIC` 或 `BIGNUMERIC` 欄位新增至資料表時，如果未指定[捨去模式](https://docs.cloud.google.com/bigquery/docs/schemas?hl=zh-tw#rounding_mode)，系統會自動將捨去模式設為資料表的預設捨去模式。變更資料表的預設捨去模式不會影響現有欄位的捨去模式。
...
資料表建立後，您可以使用 [`ALTER TABLE RENAME TO` 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_table_rename_to_statement)重新命名資料表。以下範例會將 `mytable` 重新命名為 `mynewtable`：
...
* 如要重新命名正在串流資料的資料表，請停止串流、提交所有待處理的串流，並等待 BigQuery 指出串流未在使用中。
...
* 重新命名資料表時，系統會捨棄該資料表上建立的任何搜尋索引或向量索引。
* 您無法重新命名外部資料表。
...
* Google Cloud 控制台一次只能複製一個資料表，且無法覆寫目的地資料集中的現有資料表。目的地資料集中的資料表名稱不得重複。
* Google Cloud 控制台不支援將多個來源資料表複製到目的地資料表。
* 使用 API、bq 指令列工具或用戶端程式庫複製多個來源資料表到目的地資料表時，所有來源資料表都必須具有相同的結構定義，包括任何分割或叢集。

  某些資料表結構定義更新 (例如捨棄或重新命名資料欄)，可能會導致資料表具有看似相同的結構定義，但內部表示法不同。這可能會導致資料表複製工作失敗，並顯示 `Maximum limit on diverging physical schemas reached` 錯誤。在這種情況下，您可以使用 [`CREATE TABLE LIKE` 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#create_table_like)，確保來源資料表的結構定義與目的地資料表的結構定義完全一致。
...
* 如果複製作業會建立新資料表，新資料表的資料表層級存取權，取決於新資料表所在資料集的存取權政策。此外，[標記](https://docs.cloud.google.com/bigquery/docs/tags?hl=zh-tw)也會從來源資料表複製到新資料表。
...
2. 請發出 `bq cp` 指令。選用標記可用來控管目的地資料表的寫入配置：
   ...
   * `destination_table`：目的地資料集中的資料表名稱。
   ...
   如要複製 `mydataset.mytable` 資料表，並在目的地資料集有相同名稱的資料表時傳回錯誤，請輸入下列指令。來源資料集位於預設專案中。目的地資料集位於 `myotherproject` 專案中。`-n` 捷徑是用來防止有相同名稱的資料表遭到覆寫。
