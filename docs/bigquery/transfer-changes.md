...
Google Ads 適用的 BigQuery 資料移轉服務會定期更新，以支援新資料欄，並配合新版 [Google Ads API](https://developers.google.com/google-ads/api/docs/release-notes?hl=zh-tw) 進行調整。Google Ads 適用的 BigQuery 資料移轉服務連接器會使用 Google Ads 連接器中[支援的 API 版本](https://docs.cloud.google.com/bigquery/docs/google-ads-transfer?hl=zh-tw#connector_overview)。

下列各節說明更新至新版 Google Ads API 時的變更。變更內容會依發布日期排序，每筆項目都會提供您需要進行的變更資訊，確保能繼續接收 Google Ads 的資料。
...
Google Ads 連接器的這項更新於 2024 年 6 月 17 日開始，並於 2024 年 6 月 23 日完成。
...
Google Analytics 適用的 BigQuery 資料移轉服務連接器會定期更新，以支援新資料欄，並配合新版 [Google Analytics Data API](https://developers.google.com/analytics/devguides/reporting/data/v1/changelog?hl=zh-tw) 的異動進行調整。Google Analytics 專用的 BigQuery 資料移轉服務連接器會使用最新支援的 API 版本，擷取報表資料。

以下各節概述更新至新版 Google Analytics Data API 時的變更。變更會依發布日期排序，每筆項目都會提供相關資訊，說明您需要進行哪些變更，才能繼續接收 Google Analytics 資料。
...
MySQL 連接器的 BigQuery 資料移轉服務會定期更新，以因應 MySQL 導入的新變更。
...
PostgreSQL 連接器的 BigQuery 資料移轉服務會定期更新，以因應 PostgreSQL 推出的新變更。
...
以下各節將說明 YouTube Reporting API 推出新報表時的異動。變更會依發布日期排序，每筆項目都會提供相關資訊，說明您需要進行哪些變更，才能繼續接收 YouTube 資料。
