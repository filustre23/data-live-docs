...
# WRITE\_API\_TIMELINE\_BY\_FOLDER 檢視畫面

`INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_FOLDER` 檢視畫面會顯示目前專案父項資料夾 (包括子資料夾) 的 BigQuery Storage Write API 擷取統計資料，並以每分鐘為單位匯總。
...
如要查詢 `INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_FOLDER` 檢視畫面，您需要專案父項資料夾的 `bigquery.tables.list` Identity and Access Management (IAM) 權限。
...
* 如要查詢歐盟多區域的資料，請使用 `region-eu.INFORMATION_SCHEMA.WRITE_API_TIMELINE_BY_FOLDER`
...
以下範例會計算專案資料夾中所有資料表在過去 30 分鐘內，每分鐘的失敗要求總數，並依資料流類型和錯誤代碼細分：
...
以下範例會計算專案資料夾中每分鐘成功和失敗的附加要求，並依錯誤代碼類別細分。這項查詢可用於填入資訊主頁。
...
以下範例會傳回專案資料夾中，傳入流量最多的 10 個資料表的 BigQuery Storage Write API 擷取統計資料：
