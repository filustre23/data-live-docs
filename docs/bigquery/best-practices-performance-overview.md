...
本文提供最佳化技巧總覽，可提升 BigQuery 的查詢效能。一般來說，查詢時需要完成的工作越少，成效就會越好。這類模型執行速度更快，耗用的資源也較少，因此可降低成本並減少失敗次數。
...
如要評估特定查詢或資源爭用情形，可以使用 [Cloud Monitoring](https://docs.cloud.google.com/bigquery/docs/monitoring?hl=zh-tw) 或 [BigQuery 管理資源圖表](https://docs.cloud.google.com/bigquery/docs/admin-resource-charts?hl=zh-tw)，監控 BigQuery 工作長期資源使用情形。您也可以使用 Gemini Cloud Assist [分析工作](https://docs.cloud.google.com/bigquery/docs/use-cloud-assist?hl=zh-tw#analyze_jobs)。如果發現查詢速度緩慢或耗用大量資源，可以針對該查詢進行效能最佳化。

某些查詢模式 (尤其是商業智慧工具產生的模式) 可使用 [BigQuery BI Engine](https://docs.cloud.google.com/bigquery/docs/bi-engine-query?hl=zh-tw) 加速。BI Engine 是一種快速的記憶體內分析服務，可智慧快取最常使用的資料，藉此加快 BigQuery 中許多 SQL 查詢的速度。BI Engine 內建於 BigQuery，因此您通常不需要修改查詢，就能獲得更優異的效能。
...
如果您想設定每月支出預算，或需要比隨選模式提供的容量更多，建議採用[以容量計費](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw)模式。使用以容量計費模式時，您會分配專屬的查詢處理容量，並以[時段](https://docs.cloud.google.com/bigquery/docs/slots?hl=zh-tw)為單位計算。以容量計費模式的價格包含所有處理的位元組費用。除了固定的[時段承諾](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#slot_commitments)，您也可以使用[自動調度時段](https://docs.cloud.google.com/bigquery/docs/slots-autoscaling-intro?hl=zh-tw)，根據查詢工作負載動態調整容量。

對相同資料重複執行的查詢，效能可能會有所不同，而且使用隨選配額的查詢，效能差異通常會比使用配額預留的查詢更大。

在處理 SQL 查詢時，BigQuery 會將執行查詢每個階段所需的運算能力細分為運算單元。BigQuery 會自動決定可同時執行的查詢數量，方法如下：
...
每次執行查詢時，BigQuery 都會產生[查詢計畫](https://docs.cloud.google.com/bigquery/query-plan-explanation?hl=zh-tw)。瞭解這項計畫對於有效最佳化查詢至關重要。查詢計畫包含執行統計資料，例如讀取的位元組數和消耗的時段時間。查詢計畫也包含執行作業各階段的詳細資料，有助於診斷及改善查詢效能。[查詢執行圖表](https://docs.cloud.google.com/bigquery/docs/query-insights?hl=zh-tw)提供圖形介面，可查看查詢計畫及診斷查詢效能問題。

您也可以使用 [`jobs.get` API 方法](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/jobs/get?hl=zh-tw)或 [`INFORMATION_SCHEMA.JOBS` 檢視畫面](https://docs.cloud.google.com/bigquery/docs/information-schema-jobs?hl=zh-tw)，擷取查詢計畫和時間軸資訊。[BigQuery Visualizer](https://github.com/GoogleCloudPlatform/professional-services/tree/master/tools/bq-visualizer) 會使用這項資訊，以視覺化方式呈現 BigQuery 工作中的執行階段流程。

BigQuery 執行查詢工作時，會將宣告式 SQL 陳述式轉換成執行圖。這個圖會再細分成一連串的查詢階段，而這些階段本身是由更精細的執行步驟集所組成。BigQuery 使用大量分散式平行架構來執行這些查詢。BigQuery 的階段則是模擬許多潛在工作站可能平行執行的工作單元。各階段之間是使用[快速的分散式重組基礎架構](https://cloud.google.com/blog/products/gcp/in-memory-query-execution-in-google-bigquery?hl=zh-tw)通訊。
