...
如要授予 `allAuthenticatedUsers` 主體已儲存的 BigQuery Studio 查詢的程式碼檢視者 ([roles/dataform.codeViewer](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#dataform.codeViewer)) 角色，即可公開存取該查詢。
...
**注意：** 將管理員、編輯或執行層級的權限授予 `allAuthenticatedUsers`，可能會導致惡意行為人存取您的資料。只授予最低必要權限。
...
4. 按一下已儲存的查詢旁的 more\_vert「開啟動作」，然後依序點選「開啟方式」「連結試算表」。
   ...
   或者，按一下已儲存的查詢名稱，在詳細資料窗格中開啟查詢，然後按一下「在以下項目中開啟」**> 已連結的試算表**。
...
公開的傳統版已儲存查詢
:   擁有查詢連結的任何人都可看到公開的傳統版已儲存查詢。這類項目會標示 share 圖示。只有擁有者才能遷移傳統版已儲存的公開查詢。
...
1. 前往 Google Cloud 控制台的「BigQuery」頁面。
   ...
   [前往「BigQuery」](https://console.cloud.google.com/bigquery?hl=zh-tw)
2. 點選左側窗格中的「類別」「傳統版 Explorer」：
3. 在「傳統版 Explorer」窗格中，展開專案並依序點選「(傳統版) 查詢」旁的 more\_vert「查看動作」，然後點選「遷移傳統版已儲存的查詢」。
