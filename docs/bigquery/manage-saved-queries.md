...
本文說明如何管理[已儲存的查詢和傳統已儲存的查詢](https://docs.cloud.google.com/bigquery/docs/saved-queries-introduction?hl=zh-tw)，以及如何在 [Knowledge Catalog](https://docs.cloud.google.com/dataplex/docs/introduction?hl=zh-tw) 中管理已儲存查詢的中繼資料。
...
您可以在[遷移](#migrate_classic_saved_queries)、[建立](https://docs.cloud.google.com/bigquery/docs/work-with-saved-queries?hl=zh-tw#create_saved_queries)或選取 BigQuery Studio 已儲存查詢時，選擇為遷移的公開或專案傳統已儲存查詢設定 IAM 權限，並[授予所選身分與存取權管理 (IAM) 權限](https://docs.cloud.google.com/bigquery/docs/manage-saved-queries?hl=zh-tw#share-saved-query)給該已儲存查詢。

將公開或專案的傳統已儲存的查詢遷移至 BigQuery Studio 已儲存的查詢時，請選取 BigQuery Studio 已儲存的查詢，將授予該查詢的權限複製到遷移的已儲存的查詢。

### 必要的角色

如要取得管理已儲存查詢所需的權限，請要求管理員在您要管理已儲存查詢的專案中，授予下列 IAM 角色：

* 如要在 Google Cloud 控制台中管理 BigQuery Studio 已儲存的查詢，請按照下列步驟操作：
...
與您共用查詢的使用者只會看到查詢的最新版本。您自動儲存但未明確儲存的變更，不會顯示在共用的查詢中。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。

   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「檔案」窗格中展開專案和子資料夾，找出要共用的已儲存的查詢。
4. 依序點選已儲存的查詢旁的 more\_vert「查看動作」、「共用」>「管理權限」。
5. 在「管理權限」窗格中，按一下「新增使用者/群組」。
6. 在「New principals」(新增主體) 欄位中輸入主體。
7. 在「Role」(角色) 清單中，選取下列其中一個角色：
...
8. 選用：如要查看完整的角色清單和進階共用設定，請按一下「進階共用設定」。
9. 按一下 [儲存]。
10. 如要返回已儲存的查詢詳細資料，請按一下「關閉」。
11. 如要產生已儲存的查詢連結，請依序點選「查看動作」more\_vert>「分享」>「複製連結」。

    連結即會複製到剪貼簿。

## 授予已儲存查詢的公開存取權

如要授予 `allAuthenticatedUsers` 主體 BigQuery Studio 已儲存查詢的公開存取權，請授予該主體已儲存查詢的程式碼檢視者 ([roles/dataform.codeViewer](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#dataform.codeViewer)) 角色。
...
舉例來說，如果您在`sales`已儲存的查詢中，將「程式碼檢視者」角色授予 `allAuthenticatedUsers`，那麼所有透過 Google 帳戶完成驗證的服務帳戶和使用者，都將擁有`sales`已儲存查詢的唯讀存取權。

**注意：** 將管理員、編輯或執行層級的權限授予 `allAuthenticatedUsers`，可能會讓惡意行為者存取您的資料。只授予最低必要權限。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Files」窗格中展開專案和子資料夾，找出要授予公開存取權的已儲存查詢。
4. 依序點選已儲存的查詢旁的 more\_vert「查看動作」、「共用」>「管理權限」。
5. 在「管理權限」窗格中，按一下「新增使用者/群組」。
6. 在「New principals」(新增主體) 欄位中輸入 `allAuthenticatedUsers`。
7. 在「Role」(角色) 清單中，選取「Code Viewer」(程式碼檢視者) 角色。
8. 按一下 [儲存]。
9. 如要返回已儲存的查詢詳細資料，請按一下「關閉」。
...
在`iam.allowedPolicyMemberDomains`政策中限制`allAuthenticatedUsers`後，專案中的任何 IAM 政策都無法使用`allAuthenticatedUsers`主體，因此無法授予所有資源公開存取權，包括 BigQuery Studio 已儲存的查詢。

如要進一步瞭解這項 `iam.allowedPolicyMemberDomains` 政策，以及如何設定這項政策，請參閱「[依照網域設定身分限制](https://docs.cloud.google.com/resource-manager/docs/organization-policy/restricting-domains?hl=zh-tw)」。

## 設定程式碼資產的預設區域

Google Cloud 專案中的所有新程式碼資產都會使用預設區域。資產建立後，就無法變更區域。

**重要事項：** 如果您在建立程式碼資產時變更區域，該區域會成為後續所有程式碼資產的預設區域。現有程式碼資產不受影響。
...
3. 在專案名稱旁，按一下 more\_vert「查看檔案面板動作」>「切換程式碼區域」。
4. 選取要設為預設的程式碼區域。
5. 按一下 [儲存]。
...
您可以透過 Dataform API 的預設專案層級 CMEK 金鑰，使用客戶自行管理的加密金鑰 (CMEK) 加密已儲存的查詢。您無法在 Google Cloud 控制台中，將個別 CMEK 金鑰套用至已儲存的查詢。新設定只會套用至新建立的已儲存查詢。

如要進一步瞭解如何設定預設金鑰，請參閱[使用 CMEK 保護 BigQuery Studio 程式碼資產](https://docs.cloud.google.com/bigquery/docs/customer-managed-encryption?hl=zh-tw#cmek-studio)。
...
3. 在「Explorer」窗格中，依序點選「Queries」旁邊的 more\_vert「View actions」，然後執行下列其中一項操作：
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Files」窗格中展開專案和子資料夾，找出要查看中繼資料的已儲存的查詢。
4. 依序點選已儲存的查詢和「資訊」，即可查看已儲存查詢的相關資訊，例如使用的[區域](https://docs.cloud.google.com/bigquery/docs/saved-queries-introduction?hl=zh-tw#supported_regions)和上次修改日期。

## 使用已儲存的查詢版本

您可以選擇在[存放區](https://docs.cloud.google.com/bigquery/docs/repository-intro?hl=zh-tw)內或外部建立已儲存的查詢。系統會根據已儲存的查詢位置，以不同方式處理已儲存的查詢版本管理。

### 存放區中已儲存查詢的版本控管

存放區是位於 BigQuery 或第三方供應商的 Git 存放區。您可以在存放區中使用[工作區](https://docs.cloud.google.com/bigquery/docs/workspaces-intro?hl=zh-tw)，對已儲存的查詢執行版本控管。詳情請參閱「[使用檔案進行版本控管](https://docs.cloud.google.com/bigquery/docs/workspaces?hl=zh-tw#use_version_control_with_a_file)」。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「檔案」窗格中，展開專案和子資料夾，找出要查看版本記錄的已儲存的查詢。
4. 按一下已儲存的查詢，然後點選「版本記錄」history，即可查看已儲存的查詢版本清單，清單會依日期降序排列。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「檔案」窗格中，展開專案和子資料夾，找出要比較版本記錄的已儲存的查詢。
4. 按一下已儲存的查詢，然後點選「版本記錄」history。
5. 按一下已儲存的查詢版本旁的 more\_vert「查看動作」，然後點選「比較」。比較窗格隨即開啟，比較您選取的已儲存查詢版本與目前的查詢。
6. 選用：目前的查詢也會顯示自動儲存的變更。如要明確儲存這些變更，請按一下「覆寫」。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Files」窗格中，展開專案和子資料夾，找出要還原先前版本的已儲存的查詢。
4. 按一下已儲存的查詢，然後點選「版本記錄」history。
5. 按一下要還原的已儲存的查詢版本旁邊的「查看動作」more\_vert，然後按一下「比較」。比較窗格隨即開啟，比較您選取的已儲存的查詢版本與最新查詢版本，包括任何自動儲存的變更。
6. 比較後如要還原先前儲存的查詢版本，請按一下「還原」。
7. 按一下「確認」。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「檔案」窗格中，展開您的專案和子資料夾，找出要透過連結試算表開啟的已儲存的查詢。
4. 按一下已儲存的查詢旁的「開啟動作」more\_vert，然後依序點選「開啟方式」>「連結試算表」。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Files」窗格中，展開專案和子資料夾，找出要下載的已儲存的查詢。
4. 按一下已儲存的查詢名稱，然後按一下 **下載**。
...
2. 在左側窗格中，按一下「檔案」folder，開啟檔案瀏覽器。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Files」窗格中，展開專案和子資料夾，找出要刪除的已儲存的查詢。
4. 按一下已儲存的查詢旁的 more\_vert「開啟動作」，然後按一下「刪除」。
...
**注意：** 如果尚未啟用 BigQuery Studio，傳統儲存的查詢會顯示在「傳統探索器」窗格的「已儲存的查詢」資料夾中，而不是「(傳統) 查詢」資料夾。
NUMBER
...
如果您打算共用已儲存的傳統查詢，請考慮在查詢中加入說明用途的註解。
...
1. 點選左側窗格中的 explore「Explorer」。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
2. 在「Explorer」窗格中展開專案，然後按一下「(傳統版) 查詢」。
3. 找出並點選要授予存取權的傳統已儲存查詢。
   您可以使用搜尋功能或篩選器尋找查詢。
4. 按一下查詢旁的 more\_vert「View actions」(查看動作)，然後按一下「Copy link」(複製連結)。
5. 將連結分享給要授予查詢存取權的使用者。
...
2. 點選左側窗格中的 explore「Explorer」。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Explorer」窗格中展開專案，然後按一下「(傳統版) 查詢」。
4. 按一下已儲存的傳統查詢名稱即可開啟。
5. 依序點按 「儲存查詢 (傳統版)」**>「將查詢另存為...」**。
6. 在「儲存查詢」對話方塊中輸入名稱，然後選擇查詢位置。
7. 按一下 [儲存]。

### 遷移傳統版已儲存的查詢

如要批次遷移傳統版已儲存的查詢，您必須獲得[必要角色](https://docs.cloud.google.com/bigquery/docs/manage-saved-queries?hl=zh-tw#required_roles)。這些角色授予的權限會決定您可以遷移的傳統版已儲存查詢類型。

您可以批次遷移下列傳統版已儲存查詢：

個人傳統版已儲存查詢
:   只有建立個人傳統版已儲存查詢的使用者才能看到這類查詢。這類執行個體會以 person 圖示表示。個人儲存的傳統查詢只能由擁有者遷移。

公開的傳統版已儲存查詢
:   擁有查詢連結的任何人都可看到公開的傳統版已儲存查詢。這類查詢會以 share 圖示標示。只有擁有者可以遷移公開的傳統版已儲存查詢。

    公開傳統已儲存查詢的 IAM 權限，不會對應至 BigQuery Studio 已儲存查詢的權限。也就是說，從公開傳統儲存查詢遷移的 BigQuery Studio 儲存查詢，預設不會公開。您必須在遷移期間或之後，為遷移的 BigQuery Studio 已儲存查詢設定 IAM 權限。

    如要在遷移期間為遷移的 BigQuery Studio 已儲存的查詢設定 IAM 權限，您可以選取現有的 BigQuery Studio 已儲存的查詢，並將其權限套用至遷移的已儲存的查詢。BigQuery 會複製所選 BigQuery Studio 已儲存的查詢的權限，並套用至遷移的已儲存的查詢。您也可以手動新增要與之共用遷移已儲存的查詢的使用者或群組。

    如果在遷移期間未設定 IAM 權限，只有您能存取遷移的 BigQuery Studio 已儲存查詢。

專案傳統版已儲存查詢
:   凡是擁有[必要權限](https://docs.cloud.google.com/bigquery/docs/work-with-saved-queries?hl=zh-tw#required_permissions_for_classic_saved_queries)的主體，都能看到專案層級的儲存查詢。這類檔案會以 people 圖示表示。
    您可以批次遷移專案中的所有傳統版已儲存查詢。
...
* 將所有要遷移的傳統版已儲存查詢儲存為 BigQuery Studio 已儲存的查詢，並儲存在所選區域中。
...
批次遷移後，您將無法修改遷移的傳統版已儲存查詢。遷移的個人、公開和專案傳統版已儲存查詢會變成唯讀。

BigQuery 會使用 Dataform API，將遷移的 BigQuery Studio 已儲存查詢新增至 Google Cloud 專案。如要還原變更，您必須手動清理。
...
2. 點選左側窗格中的 explore「Explorer」。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Explorer」窗格中展開專案，然後依序點選「(Classic) Queries」旁的「View actions」more\_vert 和「Migrate classic saved queries」。
4. 在「傳統版已儲存查詢遷移作業」窗格的「檢查遷移準備狀態」部分，按一下「下一步」，確認您具備[必要角色](https://docs.cloud.google.com/bigquery/docs/manage-saved-queries?hl=zh-tw#required_roles)。
   ...
   您的 IAM 權限決定了可遷移的傳統版已儲存查詢類型，以及**傳統版已儲存查詢遷移作業**窗格中可見的部分。
5. 在「區域」部分，從「區域」下拉式選單中，選取 BigQuery 儲存已遷移已儲存查詢的區域。
...
7. 如要遷移專案中的所有傳統版公開查詢，請在「遷移公開查詢」部分中執行下列操作：
   ...
   3. 選用：如要新增要與之共用已遷移儲存查詢的使用者或群組，請按一下「新增使用者/群組」。
      ...
      如要公開分享已遷移的儲存查詢，請將 `allAuthenticatedUsers` 設為主體，並授予程式碼檢視者角色。詳情請參閱「[授予公開存取權](#grant-public-access)」。
   4. 點選「下一步」。
8. 如要遷移專案層級的傳統版儲存查詢，請在「遷移專案查詢」部分中執行下列操作：
   ...
   3. 選用：如要新增要與之共用已遷移儲存查詢的使用者或群組，請按一下「新增使用者/群組」。
   4. 點選「下一步」。
9. 如要確認您瞭解[遷移作業的風險](#migration-risks)，並要批次遷移傳統版已儲存查詢，請在「Confirm」(確認) 區段的「Confirm」(確認) 欄位中輸入 `confirm`，然後按一下「Next」(下一步)。
...
您可以透過 Knowledge Catalog 儲存及管理已儲存查詢的中繼資料。根據預設，Knowledge Catalog 會提供已儲存的查詢，不需額外設定。

您可以使用 Knowledge Catalog 管理所有[已儲存查詢位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)的已儲存的查詢。在 Knowledge Catalog 中管理已儲存的查詢時，須遵守 [Knowledge Catalog 配額和限制](https://docs.cloud.google.com/dataplex/docs/quotas?hl=zh-tw)，並適用 [Knowledge Catalog 定價](https://cloud.google.com/dataplex/pricing?hl=zh-tw)。
...
系統項目群組
:   已儲存的查詢的[系統項目群組](https://docs.cloud.google.com/dataplex/docs/ingest-custom-sources?hl=zh-tw#entry-groups)為 `@dataform`。如要在 Knowledge Catalog 中查看已儲存的查詢項目的詳細資料，您需要查看 `dataform` 系統項目群組。如要瞭解如何查看項目群組中的所有項目清單，請參閱 Knowledge Catalog 說明文件中的「[查看項目群組的詳細資料](https://docs.cloud.google.com/dataplex/docs/ingest-custom-sources?hl=zh-tw#entry-group-details)」。�

系統項目類型
:   已儲存的查詢的[系統項目類型](https://docs.cloud.google.com/dataplex/docs/ingest-custom-sources?hl=zh-tw#entry-types)為 `dataform-code-asset`。如要查看已儲存的查詢的詳細資料，請查看 `dataform-code-asset` 系統項目類型、使用切面篩選器篩選結果，並[將 `dataform-code-asset` 切面內的 `type` 欄位設為 `SQL_QUERY`](https://docs.cloud.google.com/dataplex/docs/search-syntax?hl=zh-tw#aspect-search)。接著，選取所選已儲存的查詢的項目。如要瞭解如何查看所選項目類型的詳細資料，請參閱 Knowledge Catalog 文件中的「[查看項目類型的詳細資料](https://docs.cloud.google.com/dataplex/docs/ingest-custom-sources?hl=zh-tw#entry-type-details)」。如要瞭解如何查看所選項目的詳細資料，請參閱 Knowledge Catalog 文件中的「[查看項目的詳細資料](https://docs.cloud.google.com/dataplex/docs/search-assets?hl=zh-tw#view-entry-details)」。

系統切面類型
:   已儲存查詢的[系統切面類型](https://docs.cloud.google.com/dataplex/docs/enrich-entries-metadata?hl=zh-tw#aspect-types)為 `dataform-code-asset`。如要透過註解資料已儲存的查詢項目[切面](https://docs.cloud.google.com/dataplex/docs/enrich-entries-metadata?hl=zh-tw#aspects)，在 Knowledge Catalog 中為已儲存的查詢提供額外背景資訊，請查看 `dataform-code-asset` 切面類型、使用切面篩選器篩選結果，並[將 `dataform-code-asset` 切面內的 `type` 欄位設為 `SQL_QUERY`](https://docs.cloud.google.com/dataplex/docs/search-syntax?hl=zh-tw#aspect-search)。如需如何使用切面註解項目的操作說明，請參閱 Knowledge Catalog 說明文件中的「[管理切面及豐富中繼資料](https://docs.cloud.google.com/dataplex/docs/enrich-entries-metadata?hl=zh-tw)」一文。

類型
:   已儲存查詢的類型為 `SQL_QUERY`。您可以使用`aspect:dataplex-types.global.dataform-code-asset.type=SQL_QUERY`[以切面為準的篩選器](https://docs.cloud.google.com/dataplex/docs/search-syntax?hl=zh-tw#aspect-search)，依`dataform-code-asset`系統項目類型和`dataform-code-asset`切面類型篩選已儲存的查詢。
