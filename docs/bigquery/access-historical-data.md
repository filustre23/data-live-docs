...
您可以使用 [`FOR SYSTEM_TIME AS OF`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/query-syntax?hl=zh-tw#for_system_time_as_of) 子句，在時空旅行時間範圍內，查詢資料表在任何時間點的歷來資料。這個子句會採用常數時間戳記運算式，並參照該時間戳記當前的資料表版本。資料表必須儲存在 BigQuery 中，不得為外部資料表。使用 `SYSTEM_TIME AS OF` 時，資料表大小沒有限制。
...
在舊版 SQL 中，[時間修飾符](https://docs.cloud.google.com/bigquery/docs/table-decorators?hl=zh-tw#time_decorators)提供同等功能。

如果時間戳記指定的時間早於時間回溯期，或早於資料表建立時間，查詢就會失敗，並傳回類似下列的錯誤：
...
如果資料表已刪除，您就無法使用 `FOR SYSTEM_TIME AS OF` 直接查詢其歷來資料。查詢失敗，並傳回類似下列內容的錯誤：

```
Not found: Table myproject:mydataset.table was not found in location LOCATION
```

如要存取已刪除資料表的歷來資料，請改為從[特定時間點](https://docs.cloud.google.com/bigquery/docs/access-historical-data?hl=zh-tw#restore-a-table)還原資料表。
...
* 進一步瞭解[時間旅行和容錯移轉的資料保留功能](https://docs.cloud.google.com/bigquery/docs/time-travel?hl=zh-tw)。
