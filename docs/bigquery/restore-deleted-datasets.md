...
您可以恢复数据集，以将其恢复到删除时的状态。您只能恢复处于[时间旅行窗口](https://docs.cloud.google.com/bigquery/docs/time-travel?hl=zh-cn#time_travel)内的数据集。这种恢复包括数据集中包含的所有对象、数据集属性和安全设置。对于未
恢复的资源，请参阅
[限制](#limitations)。
...
* 您无法直接恢复逻辑视图。不过，您可以恢复删除数据集或重新创建视图来恢复逻辑视图。如需详细了解这些解决方法，请参阅[恢复视图](https://docs.cloud.google.com/bigquery/docs/managing-views?hl=zh-cn#restore_a_view)。
...
* 数据集恢复删除后，在接下来的 7 天内无法删除。
  数据集的实体（例如表和例程）可以删除。
  如果您需要更短的时间，请与
  [Google Cloud 支持团队](https://cloud.google.com/support-hub?hl=zh-cn)联系。
...
* `bigquery.datasets.create`项目的
* 数据集的 `bigquery.datasets.get`
...
**注意** ：您只能恢复给定数据集 ID 的最新数据集。
如果您删除了某个数据集，然后创建了具有相同 ID 的新数据集，则无法恢复删除的原始数据集。[不过，您或许仍然可以从已删除的数据集中恢复特定表。](https://docs.cloud.google.com/bigquery/docs/managing-datasets?hl=zh-cn#restore-delete-tables)
