...
本文說明如何查詢儲存在 [Amazon Simple Storage Service (Amazon S3) BigLake 資料表](https://docs.cloud.google.com/bigquery/docs/omni-aws-create-external-table?hl=zh-tw)中的資料。

## 事前準備

確認您有 [Amazon S3 BigLake 資料表](https://docs.cloud.google.com/bigquery/docs/omni-aws-create-external-table?hl=zh-tw)。
...
呼叫者可以是您的帳戶或 [Amazon S3 連線服務帳戶](https://docs.cloud.google.com/bigquery/docs/omni-aws-create-connection?hl=zh-tw#creating-aws-connection)。視權限而定，您可以將這些角色授予自己，或請管理員授予您這些角色。如要進一步瞭解如何授予角色，請參閱「[查看可針對資源授予的角色](https://docs.cloud.google.com/iam/docs/viewing-grantable-roles?hl=zh-tw)」。

如要查看查詢 Amazon S3 BigLake 資料表所需的確切權限，請展開「Required permissions」(必要權限) 部分：
...
建立 Amazon S3 BigLake 資料表後，您可以使用 [GoogleSQL 語法查詢該資料表](https://docs.cloud.google.com/bigquery/docs/running-queries?hl=zh-tw)，就像查詢標準 BigQuery 資料表一樣。
...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `TABLE_NAME`：[您建立的資料表](#create-biglake-table)名稱
   * 按一下「執行」play\_circle。
...
BigQuery 會建立臨時資料表來儲存查詢結果。如要從暫時性資料表擷取查詢結果，可以使用 Google Cloud 主控台或 [BigQuery API](https://docs.cloud.google.com/bigquery/docs/reliability-read?hl=zh-tw#read_with_api)。

選取下列選項之一：

### 控制台

[查詢參照外部 Cloud 資料的 BigLake 資料表](#query-biglake-table)時，您可以在 Google Cloud 控制台中查看查詢結果。
...
3. 傳遞指定的工作參考資料，使用 [`jobs.getQueryResults`](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/jobs/getQueryResults?hl=zh-tw) 讀取資料列，並傳遞查詢結果的指定資料表參考資料，使用 [`tabledata.list`](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tabledata/list?hl=zh-tw) 方法。

## 查詢 `_FILE_NAME` 虛擬資料欄

以外部資料來源為基礎的資料表可提供名為 `_FILE_NAME` 的虛擬資料欄。這個資料欄含有該列所屬檔案的完整路徑。此資料欄僅適用於參照儲存在 **Cloud Storage**、**Google 雲端硬碟**、**Amazon S3** 和 **Azure Blob Storage** 中的外部資料的資料表。
...
如果查詢在 `_FILE_NAME` 虛擬資料欄上設有篩選述詞，BigQuery 會嘗試略過不符合篩選條件的檔案。使用 `_FILE_NAME` 虛擬資料欄建構查詢述詞時，請套用[使用虛擬資料欄查詢擷取時間分區資料表](https://docs.cloud.google.com/bigquery/docs/querying-partitioned-tables?hl=zh-tw#query_an_ingestion-time_partitioned_table)的類似建議。
