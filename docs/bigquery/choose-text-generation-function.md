...
本文比較 BigQuery ML [`AI.GENERATE_TEXT`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-generate-text?hl=zh-tw) 和 [`AI.GENERATE`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-ai-generate?hl=zh-tw) 文字生成函式，協助您在函式功能重疊時，決定要使用哪一個。
...
* **帳單**：系統會針對處理的資料收取 BigQuery ML 費用。詳情請參閱「[BigQuery ML 定價](https://cloud.google.com/bigquery/pricing?hl=zh-tw#bigquery-ml-pricing)」一文。呼叫 LLM 時，會產生 Gemini Enterprise Agent Platform 費用。如果您使用 Gemini 2.0 以上版本模型，系統會依批次 API 費率計費。詳情請參閱「[在 Agent Platform 中建構及部署 AI 模型的成本](https://docs.cloud.google.com/vertex-ai/generative-ai/pricing?hl=zh-tw)」。
* **擴充性**：每個 6 小時的查詢工作可處理 100 萬到 1,000 萬列資料。實際處理量取決於輸入列中的平均權杖長度等因素。詳情請參閱「[生成式 AI 函式](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#generative_ai_functions)」。
...
| 函式輸出內容 | Gemini 模型的函式輸出內容：   * 生成的文字 * 負責任的 AI 技術 (RAI) 結果 * 如果啟用，系統會顯示 Google 搜尋基準建立結果 * LLM call status   其他類型模型的函式輸出內容：   * 生成的文字 * LLM call status | * 生成的文字 * JSON 格式的完整模型回覆 * LLM call status |
| 函式輸出格式 | 系統會根據 `flatten_json_output` 引數值，在單一 JSON 資料欄或個別資料表資料欄中傳回生成的值。 | 產生的值會以 `STRUCT` 物件中的欄位形式傳回。 |
...
| 設定權限 | 您必須手動建立 BigQuery 連線，並將 Agent Platform User 角色權限授予連線的服務帳戶。如果您使用 BigQuery [預設連線](https://docs.cloud.google.com/bigquery/docs/default-connections?hl=zh-tw#example-remote-model)，可以略過這個步驟。 | 您可以使用[使用者憑證](https://docs.cloud.google.com/bigquery/docs/permissions-for-ai-functions?hl=zh-tw)呼叫此函式。 |
