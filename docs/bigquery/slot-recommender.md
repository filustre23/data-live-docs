...
運算單元建議工具會根據過去 30 天的運算單元用量，預估自動調度資源用量。如要進一步瞭解自動調度資源功能，請參閱「[自動調度資源簡介](https://docs.cloud.google.com/bigquery/docs/slots-autoscaling-intro?hl=zh-tw)」。建議工具可以產生多個約期選項，並計算每個選項的總費用。建議工具也可以使用自訂價格，建議總費用最低的選項。建議的承諾和自動調度運算單元數量，是為了涵蓋整個 30 天觀察期內 P99 的運算單元用量。
...
2. 系統會選擇設定中的自動調度配額，讓可用容量符合歷史尖峰用量，確保效能不受影響。您也可以將自動調度配額調整為低於上限的值，提高自動調度使用率。不過請注意，如果配額用量無法完全涵蓋，可能會影響查詢效能。
3. 如果工作負載出現尖峰，暫時超出最大容量，時段建議工具可能會過高。在這種情況下，如果您對目前的成效水準感到滿意，可以考慮維持目前的設定。

即使有時超出設定上限，您也可能會看到 `Slot Estimator doesn't have any recommendations
that would be more effective than your current settings` 訊息。這是因為 BigQuery 有時會暫時超額佈建運算單元，藉此提升查詢速度，且不會額外收費。運算單元建議工具的目標是維持您近期的效能，包括這些爆發。如果這個 P99 用量高於目前的上限，任何等於或低於目前設定的建議都無法達到這些成效水準，因此系統不會建議變更。
...
選取特定預留項目工作負載時，運算單元建議工具會建議可提升工作效能的預留項目大小上限。接著，運算單元估算工具會分析[運算單元建模](https://docs.cloud.google.com/bigquery/docs/slot-estimator?hl=zh-tw#model_slot_performance)資料，找出可將工作效能提升至少 5% 的預留項目大小上限，以及該上限的最小增量值。如果目前的預留項目大小上限符合您過去的需求，系統就不會提供建議。

**附註：** 建議是根據歷來資料提供，工作效能可能因實際使用情形而異。
...
如要取得預留項目效能改善建議的運算單元建議工具，您必須在管理專案中具備下列 IAM 權限：
