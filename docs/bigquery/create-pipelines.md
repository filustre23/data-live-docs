...
- Enable the BigQuery, Dataform, and Vertex AI APIs.
  ...
  **Roles required to enable APIs**
  ...
  [Enable the APIs](https://console.cloud.google.com/apis/enableflow?apiid=bigquery.googleapis.com%2Cdataform.googleapis.com%2Caiplatform.googleapis.com&hl=zh-tw)
...
如果您使用 VPC Service Controls 保護管道，請注意管道執行作業是由 Dataform 支援。為排定執行的作業設定 VPC Service Controls 時，請確保符合下列規定：
...
1. 前往「BigQuery」頁面
   ...
   [前往「BigQuery」](https://console.cloud.google.com/bigquery?hl=zh-tw)
2. 在編輯器窗格的分頁列中，按一下「+」符號旁的arrow\_drop\_down箭頭，然後點選「Pipeline」(管道)。
3. 選用：如要重新命名管道，請按一下管道名稱，然後輸入新名稱。
4. 按一下「立即開始」，然後前往「設定」分頁標籤。
5. 在「Authentication」(驗證) 部分，選擇使用 Google 帳戶或服務帳戶的使用者憑證，授權管道。

   * 如要使用 Google 帳戶的使用者憑證 ([預覽](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))，請選取「以我的使用者憑證執行」。

     選用：在「Extended access options」(擴充存取權選項) 部分，選取管道需要的其他服務：
     ...
     + **Bigtable**：可讀取 Google Bigtable 資料 (唯讀)。
       ...
       **注意：** 系統不支援使用使用者憑證驗證以 API 為基礎的執行作業。如要[使用 Dataform API 執行管道中的所有工作](#run-pipeline-all-tasks)，必須將管道設為使用服務帳戶。
   * 如要使用服務帳戶，請選取「以所選服務帳戶執行」，然後選取服務帳戶。如要建立服務帳戶，請按一下「新增服務帳戶」。
6. 在「Processing location」(處理位置) 部分，選取管道的處理位置。
   ...
   * 如要啟用自動選取位置功能，請選取「自動選取位置」。這個選項會根據要求中參照的資料集選取位置。選取程序如下：
     ...
     + 如果查詢參照來自兩個以上不同位置的資料集，就會發生錯誤。如要進一步瞭解這項限制，請參閱「[跨區域資料集複製](https://docs.cloud.google.com/bigquery/docs/data-replication?hl=zh-tw)」一文。
   ...
   * 如要選擇多區域，請選取「多區域」，然後在「多區域」選單中選擇多區域。
...
1. 在「Default project」(預設專案) 欄位中，輸入現有Google Cloud 專案的名稱。這個值用於 `workflow_settings.yaml` 檔案中的 `defaultProject`，以及 `dataform.json` 檔案中的 `defaultDatabase`。管道工作在執行期間會使用預設專案。
...
2. 選用：在「預設資料集」欄位中，搜尋並選取現有資料集。系統會根據所選專案和處理位置，篩選可用資料集清單。這個值用於 `workflow_settings.yaml` 檔案中的 `defaultDataset`。管道工作在執行期間會使用預設資料集。
   ...
   **注意：** 設定預設資料集後，如果變更管道的區域，系統會使資料集選取失效。變更專案也可能導致資料集選取失效。如果所選專案中沒有指定資料集，系統會建立該資料集。
...
1. 在「執行階段範本」欄位中，接受預設的筆記本執行階段，或搜尋並選取現有的執行階段。
   ...
   * 如要建立新的執行階段，請參閱「[建立執行階段範本](https://docs.cloud.google.com/colab/docs/create-runtime-template?hl=zh-tw)」。**注意：**筆記本執行階段範本必須與指定該範本的管道位於相同區域。**注意：**在 BigQuery 管道中加入筆記本時，您無法變更 Gemini Enterprise Agent Platform 執行階段例項的網路。執行階段僅限使用預設網路，不支援選取其他網路。
2. 在「Cloud Storage bucket」欄位中，按一下「Browse」(瀏覽)，然後選取或建立 Cloud Storage bucket，用於儲存管道中筆記本的輸出內容。
3. 按照「[將主體新增至值區層級政策](https://docs.cloud.google.com/storage/docs/access-control/using-iam-permissions?hl=zh-tw#bucket-add)」一文中的步驟，將自訂 Dataform 服務帳戶新增為主體，加入您打算用來儲存排定管線執行作業輸出的 Cloud Storage 值區，並將[儲存空間管理員角色](https://docs.cloud.google.com/iam/docs/roles-permissions/storage?hl=zh-tw#storage.admin) (`roles/storage.admin`) 授予這個主體。
   ...
   您必須為所選值區授予所選自訂 Dataform 服務帳戶 Storage 管理員 IAM 角色。

## 新增管道工作

您可以將多個工作新增至管道，並以特定順序執行程式碼資產。

### 工作命名慣例

為管道工作命名時，請遵循下列命名慣例：

* 工作名稱只能包含英文字母 (a 至 z、A 至 Z)、數字 (0 至 9)、底線 (`_`) 和連字號 (`-`)。
* 工作名稱開頭須為英文字母、數字或底線。
* 工作名稱不得包含半形句號 (`.`)。命名或重新命名工作時，請勿加入 `.sql` 或 `.py` 等副檔名。如果工作名稱含有半形句號，系統會停用「執行」按鈕。
* 工作名稱最多可包含 1,024 個 UTF-8 位元組的字元。
* 管道容器中的工作名稱不得重複。
* 工作名稱會區分大小寫。
...
2. 點選左側窗格中的 explore「Explorer」：
   ...
   如果沒有看到左側窗格，請按一下
   last\_page「Expand left pane」(展開左側窗格)
   開啟窗格。
...
4. 如要新增程式碼素材資源，請選取下列其中一個選項：
   ...
   **匯入現有查詢**
   ...
   1. 按一下「新增工作」，然後選取「記事本」。
      您可以建立新筆記本，或匯入現有筆記本。如要變更筆記本執行階段範本的設定，請參閱[筆記本選項](#notebook_options)。
   ...
   **建立新筆記本**
   ...
   4. 按一下 [儲存]。
   5. 選用：如要重新命名筆記本，請按一下管線窗格中的筆記本名稱，然後按一下「編輯筆記本」，按一下畫面頂端的現有筆記本名稱，然後輸入新名稱。
   ...
   **注意：** 如要提供意見回饋或尋求支援，請傳送電子郵件至
   ...
   7. 使用「詳細資料」**> 設定」**中的設定，或檢視畫面程式碼編輯器的 `config` 區塊，設定工作。
...
3. 在「Explorer」窗格中展開專案，按一下「Pipelines」，然後選取管道。
4. 按一下選取的工作。
5. 如要變更前一個工作，請在「Run after」(在後執行) 選單中，選取要排在工作之前的任務。
...
**重要事項：** 如果您在 [`projects.locations.updateConfig` Dataform API 方法](https://docs.cloud.google.com/dataform/reference/rest/v1beta1/projects.locations/updateConfig?hl=zh-tw)中，將 `enable_private_workspace` 欄位設為 `true`，即可提升安全性。[(預覽版)](https://cloud.google.com/products?hl=zh-tw#product-launch-stages)
這麼做之後，只有管道建立者才能讀取及編寫該管道中的程式碼。
詳情請參閱「[啟用私人工作區](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#enable-private-workspaces)」。**注意：** 您可以共用管道，但無法共用管道中的工作。
...
2. 點選左側窗格中的 explore「Explorer」：
   ...
   開啟窗格。
3. 在「Explorer」窗格中展開專案，按一下「Pipelines」，然後選取管道。
4. 按一下「Run」play\_circle\_filled
   >「Select tasks to run」。
...
8. 選用：設定下列執行選項：
   ...
   * **包含依附元件**：選取這個選項，即可執行所選工作及其依附元件。
   * **納入附屬元件**：選取這個選項，即可執行所選工作及其下游轉換附屬元件。
...
9. 按一下「執行」。如果驗證方法選取「使用使用者憑證執行」，您必須[授權 Google 帳戶](#authorize-google-account) ([預覽版](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))。
