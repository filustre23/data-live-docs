...
您可以使用資訊結構定義和記錄監控等工具，監控[materialized view](https://docs.cloud.google.com/bigquery/docs/materialized-views-intro?hl=zh-tw)。
...
如果基礎資料表會在預先定義的時間點擷取資料 (例如使用夜間擷取、轉換和載入 (ETL) 管道)，請考慮控管具體化檢視維護時間表，方法如下：

1. [停用自動重新整理功能](https://docs.cloud.google.com/bigquery/docs/materialized-views-manage?hl=zh-tw#automatic-refresh)。
2. [手動重新整理](https://docs.cloud.google.com/bigquery/docs/materialized-views-manage?hl=zh-tw#manual-refresh)，可做為 ETL 管道的一環，或在一天中的特定時段設定排程查詢。
...
請完成下列步驟，建立快訊政策來指定條件，並在十分鐘內有三項具體化檢視表重新整理作業失敗時傳送電子郵件。設定警告政策時，這個選項可提供更多彈性。如果直接建立記錄指標，系統會在記錄檔中出現具體化檢視重新整理失敗錯誤時，傳送警告。
...
4. 在「新增篩選器」中，根據「規則運算式」欄位中定義的具體化檢視命名慣例，為快訊新增篩選條件。
...
6. 指定「門檻值」，例如 `3`，並視需要設定「警報觸發條件」和「門檻位置」欄位。按一下「繼續」。
7. 選擇接收快訊的通知管道。
8. 點選「建立政策」。
...
您可以使用 Terraform 建立自訂指標、快訊政策、通知管道和記錄範圍。下列 Terraform 範例會使用查詢，監控並記錄每個失敗的具體化檢視重新整理作業。
...
如要查看查詢作業的具體化檢視用量，您可以呼叫 [`jobs.get` 方法](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/jobs/get?hl=zh-tw)或查詢 [`INFORMATION_SCHEMA.JOBS_BY_*` 檢視](https://docs.cloud.google.com/bigquery/docs/information-schema-jobs?hl=zh-tw)，然後查看 `materialized_view_statistics` 欄位，其中提供查詢使用具體化檢視的詳細資料，包括：

* 是否使用具體化檢視表。
* 如果未使用具體化檢視區塊，請說明[遭拒原因](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/Job?hl=zh-tw#rejectedreason)。
...
如要查看具體化檢視表的使用量變化，請查詢 [`INFORMATION_SCHEMA.JOBS_BY_*` 檢視區塊](https://docs.cloud.google.com/bigquery/docs/information-schema-jobs?hl=zh-tw)。

舉例來說，下列查詢會傳回使用目標具體化檢視的近期查詢工作摘要：
...
2. [檢查具體化檢視的更新間隔](#view_last_refresh_status)。
