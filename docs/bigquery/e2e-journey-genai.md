...
* [部署至 Gemini Enterprise Agent Platform 的自訂模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-https?hl=zh-tw)
...
3 建立 BigQuery ML 遠端模型時，您可以指定模型的 Hugging Face 或 Agent Platform Model Garden ID，自動部署開放模型。BigQuery 會管理以這種方式部署的開放模型 Agent Platform 資源，並讓您使用 BigQuery ML `ALTER MODEL` 和 `DROP MODEL` 陳述式與這些 Agent Platform 資源互動。您也可以設定自動取消部署模型。詳情請參閱「[自動部署模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-open?hl=zh-tw#automatically_deployed_models)」。
