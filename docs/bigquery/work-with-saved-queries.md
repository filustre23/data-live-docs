...
您可以為 Dataform 支援的已儲存查詢，使用客戶自行管理的加密金鑰 (CMEK) 進行加密，方法是為 Dataform API 使用預設專案層級的 CMEK 金鑰。您無法在 Google Cloud 控制台中，對已儲存的查詢套用個別 CMEK 金鑰。新設定只會套用至新建立的已儲存查詢。
...
4. 按一下
   **依序點選「儲存」>「儲存查詢」**，或按下 `Ctrl + S` 鍵 (在 macOS 上為 `Command + S` 鍵)。
