...
* 檢視區塊內的參照必須符合資料集資格。預設資料集不會影響檢視區塊主體。
...
* 您無法在[萬用字元資料表](https://docs.cloud.google.com/bigquery/docs/querying-wildcard-tables?hl=zh-tw)查詢中參照檢視表。
...
* 部分檢視區塊名稱和檢視區塊名稱前置字元已保留，如果收到錯誤訊息，指出檢視區塊名稱或前置字元已保留，請選取其他名稱，然後再試一次。
...
檢視查詢不允許使用其他陳述式類型 (例如 DML 陳述式) 和[多陳述式查詢](https://docs.cloud.google.com/bigquery/docs/multi-statement-queries?hl=zh-tw)，但 `@@session_id`
...
建立檢視表之後，便可像查詢資料表一樣[查詢](https://docs.cloud.google.com/bigquery/docs/running-queries?hl=zh-tw)該檢視表。
