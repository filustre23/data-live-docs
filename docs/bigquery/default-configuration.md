...
以下各節說明如何指定預設設定。預設設定是在機構或專案層級設定，但可以在工作階段或工作層級覆寫。
...
* 修改`default_location`全域設定後，系統最多可能需要 10 分鐘才能套用變更。在設定套用完成前，符合資格的查詢可能會轉送至先前的預設位置。
...
您可以使用 [`ALTER ORGANIZATION SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_organization_set_options_statement)，在機構組織層級設定全域設定。預設位置是唯一的全域機構設定。如果無法從要求推斷位置，系統會使用預設位置執行工作。
...
3. 如要設定 `default_location`，請在「查詢編輯器」中輸入下列 DDL 陳述式：
   ...
   請將 `LOCATION` 替換為區域或多區域[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。如果系統無法從要求推斷位置，就會使用這個值來執行工作。舉例來說，如果無法判斷查詢中資料集的位置，系統就會使用預設位置。
...
1. 如要在機構層級設定 `default_location`，請輸入 [`bq query`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_query) 指令，並提供下列 DDL 陳述式做為查詢參數。將 `use_legacy_sql` 旗標設為 `false`。
   ...
   請將 `LOCATION` 替換為區域或多區域[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。如果系統無法從要求推斷位置，就會使用這個值來執行作業。舉例來說，如果無法判斷查詢中資料集的位置，系統就會使用預設位置。
2. 如要在機構層級清除 `default_location`，請輸入 [`bq query`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_query) 指令，並提供下列 DDL 陳述式做為查詢參數。將 `use_legacy_sql` 標記設為 `false`。
...
您可以使用 [`ALTER PROJECT SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_project_set_options_statement)，在專案層級設定全域設定。`ALTER PROJECT SET OPTIONS` DDL 陳述式可選擇性接受 `PROJECT_ID` 變數。如未指定 `PROJECT_ID`，系統會預設為您執行 `ALTER PROJECT` DDL 陳述式的目前專案。

預設位置是唯一的全域專案設定。設定預設位置時，您不需要指定設定適用的區域。您無法在同一個 DDL 陳述式中混用全域和區域設定。

專案層級設定會覆寫組織層級設定。
專案層級設定可由[工作階段層級設定](https://docs.cloud.google.com/bigquery/docs/sessions-write-queries?hl=zh-tw)覆寫，而工作階段層級設定可由[作業層級設定](https://docs.cloud.google.com/bigquery/docs/running-queries?hl=zh-tw)覆寫。
...
4. 或者，如要清除 `default_location` 設定，請在**查詢編輯器**中輸入下列 DDL 陳述式。如果您在專案層級清除 `default_location`，系統會使用組織層級的預設設定 (如有)。否則，系統會使用預設設定。
...
1. 如要在專案層級設定 `default_location`，請輸入 [`bq query`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_query) 指令，並提供下列 DDL 陳述式做為查詢參數。將 `use_legacy_sql` 旗標設為 `false`。
   ...
   * `PROJECT_ID`：專案 ID。
   * `LOCATION`：區域或多區域[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。如果無法從要求推斷位置，系統就會使用這個值執行工作。舉例來說，如果無法判斷查詢中資料集的位置，系統就會使用預設位置。
2. 或者，如要在專案層級清除 `default_location`，請輸入 [`bq query`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_query) 指令，並提供下列 DDL 陳述式做為查詢參數。將 `use_legacy_sql` 旗標設為 `false`。如果您在專案層級清除 `default_location`，系統會使用組織層級的預設設定 (如有)。否則，系統會使用預設設定。
...
您可以控管查詢如何使用預留項目或隨選計費。這些設定可允許彈性使用預留項目，或要求查詢使用預留項目配額，確保費用可預測。
...
您可以使用 [`ALTER PROJECT SET OPTIONS` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_project_set_options_statement)，在專案層級設定區域設定。指定設定時，您必須指定套用設定的區域。每個陳述式只能使用一個區域。
...
如要取得擷取設定所需的權限，請要求管理員授予您指定專案的 [BigQuery 工作使用者](https://docs.cloud.google.com/iam/docs/roles-permissions/bigquery?hl=zh-tw#bigquery.jobUser)  (`roles/bigquery.jobUser`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
...
* `default_query_job_timeout_ms`：查詢工作逾時的預設時間，包括工作排隊和執行所花費的時間。逾時時間必須介於 5 分鐘和 48 小時之間。這項逾時設定只適用於個別查詢工作和指令碼的子項工作。如要為指令碼工作設定逾時時間，請使用 [jobs.insert](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/jobs/insert?hl=zh-tw) API 方法並設定 `jobTimeoutMs` 欄位。

  **注意：** `default_query_job_timeout_ms` 設定也適用於[連續查詢](https://docs.cloud.google.com/bigquery/docs/continuous-queries-introduction?hl=zh-tw)作業。如要為個別持續查詢覆寫這項專案層級設定，請為相關持續查詢指派[工作逾時](https://docs.cloud.google.com/bigquery/docs/continuous-queries?hl=zh-tw#run_a_continuous_query_by_using_a_service_account)。持續查詢仍會遵守[最長執行時間](https://docs.cloud.google.com/bigquery/docs/continuous-queries-introduction?hl=zh-tw#authorization)。
* `default_location`：如果[未設定或無法判斷](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#specify_locations)位置，系統會使用 [`default_location` 設定](#global-settings)執行工作。如果未設定 `default_location`，工作會在 `US` 多區域執行。
* `enable_reservation_based_fairness`：決定如何共用閒置運算單元的選項。預設值為 false，表示閒置運算單元會平均分配給所有查詢專案。如果啟用此選項，系統會先將閒置運算單元平均分配給所有保留項目，然後再平均分配給保留項目中的專案。詳情請參閱[以保留項目為準的公平性](https://docs.cloud.google.com/bigquery/docs/slots?hl=zh-tw#fairness)。這個選項僅支援專案層級，無法在機構或工作層級指定。
* `default_time_zone`：在時區相關的 GoogleSQL 函式中使用的預設時區，但時區未指定為引數時除外。這項設定不適用於[時間單位資料欄分區資料表](https://docs.cloud.google.com/bigquery/docs/partitioned-tables?hl=zh-tw#date_timestamp_partitioned_tables) (使用世界標準時間做為時區)、[Storage 移轉服務排定移轉作業](https://docs.cloud.google.com/storage-transfer/docs/schedule-transfer-jobs?hl=zh-tw)，或是[使用 bq 指令列工具載入資料](https://docs.cloud.google.com/bigquery/docs/bq-command-line-tool?hl=zh-tw#loading_data)。詳情請參閱[時區](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-types?hl=zh-tw#time_zones)。

  **注意：** 如要設定預設時區，請確保使用 `DATETIME` 常值的現有查詢不受影響。包括含有明確 `DATETIME` 關鍵字的查詢、以參數形式傳遞至時間函式 (例如 `DATETIME_DIFF('2022-10-01', ...)`) 的隱含轉換字串常值、`PARSE_DATETIME()` 函式等。因此，建議只在新專案中設定 `default_time_zone` 參數。
...
* `default_sql_dialect_option`：使用 bq 指令列工具或 BigQuery API 執行查詢工作時，預設的 SQL 查詢方言。變更這項設定不會影響控制台中的預設方言。這個選項可以是下列其中之一：
  ...
  + `NULL`：使用預設的查詢方言設定，相當於 `'default_legacy_sql'`。
* `enable_global_queries_execution`：這個選項會決定是否可以執行[全域查詢](https://docs.cloud.google.com/bigquery/docs/global-queries?hl=zh-tw)。預設值為 `FALSE`，表示未啟用全域查詢。
...
* `default_max_time_travel_hours`：新資料集的預設時間回溯期 (以小時為單位)。這個時間長度必須介於 48 到 168 之間 (含頭尾)，且必須可被 24 整除。變更預設時間回溯期上限不會影響現有資料集。詳情請參閱「[時間回溯和資料保留](https://docs.cloud.google.com/bigquery/docs/time-travel?hl=zh-tw#time_travel)」。
* `enable_info_schema_storage`：這個選項可存取 [`INFORMATION_SCHEMA.TABLE_STORAGE`](https://docs.cloud.google.com/bigquery/docs/information-schema-table-storage?hl=zh-tw) 和 [`SEARCH_INDEXES`](https://docs.cloud.google.com/bigquery/docs/information-schema-indexes?hl=zh-tw) 檢視畫面及其變體。這個選項預設為停用。如果您首次查詢這些檢視畫面時未將這個選項設為 `TRUE`，查詢會失敗，並提供啟用這個選項的指示。啟用這個選項後，查詢會立即成功，並傳回從該時間點開始產生的資料。檢視畫面中會顯示完整的歷來資料，但可能需要約一天時間。如果您在推出這項設定前使用過這些檢視畫面，這個選項就已啟用。
...
* `disable_on_demand_billing` ([預先發布版](https://docs.cloud.google.com/products?hl=zh-tw#product-launch-stages))：決定是否要為區域中的查詢停用隨選計費。如果為 `true`，所有查詢都必須使用已指派的預留容量，否則會失敗。詳情請參閱[`ALTER PROJECT SET OPTIONS`選項清單](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#project_set_options_list)。
* `preflight_fluid_autoscaling_reservations`：已選擇彈性調度功能的預留項目清單，可按秒計費，且沒有最短時間限制。詳情請參閱「[瞭解預留項目](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw)」。
