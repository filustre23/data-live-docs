...
4. 在「Data source details」(資料來源詳細資料) 部分執行下列操作：
   ...
   * 在「Enable legacy mapping」(啟用舊版對應) 部分，選取「true」(預設)，即可使用[舊版資料類型對應](#data_type_mapping)。選取「false」即可使用更新後的資料類型對應。如果您要進行增量轉移，這個值必須為 **false**。如要進一步瞭解資料類型對應更新，請參閱「[2027 年 3 月 16 日](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#Mar16-mysql)」。
...
* PARAMETERS：已建立移轉設定的 JSON 格式參數。例如：`--params='{"param":"param_value"}'`。以下是 MySQL 轉移作業的參數：
  ...
  + `connector.legacyMapping`：設為 `true` (預設)，即可使用[舊版資料類型對應](#data_type_mapping)。如要使用更新後的資料類型對應，請將這個值設為 `false`。如果您要進行增量轉移，這個值必須是 `false`。如要進一步瞭解資料類型對應更新，請參閱「[2027 年 3 月 16 日](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#Mar16-mysql)」。
