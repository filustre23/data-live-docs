...
您可以使用 MySQL 連接器的 [BigQuery 資料移轉服務](https://docs.cloud.google.com/bigquery/docs/dts-introduction?hl=zh-tw)，將資料從 MySQL 載入 BigQuery。支援託管於地端部署環境、[Cloud SQL](https://docs.cloud.google.com/sql/docs/introduction?hl=zh-tw)，以及其他公有雲供應商 (如 Amazon Web Services (AWS) 和 Microsoft Azure) 的 MySQL 執行個體。您可以使用 BigQuery 資料移轉服務，安排週期性移轉工作，將 MySQL 的最新資料新增至 BigQuery。
...
如果是 MySQL 轉移作業，建議選取只在記錄建立時更新的資料欄，後續更新不會變更該資料欄。例如「`CREATED_AT`」欄。

#### Upsert 寫入模式

新增或更新寫入模式會檢查主鍵，以更新資料列或在目的地資料表中插入新資料列。您可以指定主鍵，讓 MySQL 連接器判斷需要哪些變更，才能讓目的地資料表與來源資料表保持同步。如果在資料移轉期間，目標 BigQuery 資料表中存在指定的主鍵，MySQL 連接器就會使用來源資料表中的新資料更新該資料列。如果資料移轉期間沒有主鍵，MySQL 連接器就會插入新資料列。

選取「新增或更新」模式時，必須選取浮水印欄和主鍵：

* MySQL 連接器必須有記錄欄，才能追蹤來源表格中的變更。
...
如要將 MySQL 資料新增至 BigQuery，請使用下列任一方法設定移轉作業：
...
4. 在「Data source details」(資料來源詳細資料) 部分執行下列操作：
   ...
   * 如要**啟用舊版對應**，請選取「true」 (預設)，使用[舊版資料類型對應](#data_type_mapping)。選取「false」即可使用更新後的資料類型對應。如果您要進行增量轉移，這個值必須為 **false**。如要進一步瞭解資料類型對應更新，請參閱「[2027 年 3 月 16 日](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#Mar16-mysql)」。
...
* PARAMETERS：已建立移轉設定的 JSON 格式參數。例如：`--params='{"param":"param_value"}'`。以下是 MySQL 轉移作業的參數：
  ...
  + `connector.legacyMapping`：設為 `true` (預設值) 即可使用[舊版資料類型對應](#data_type_mapping)。如要使用更新後的資料類型對應，請將這個值設為 `false`。如果您要進行增量轉移，這個值必須是 `false`。如要進一步瞭解資料類型對應更新，請參閱「[2027 年 3 月 16 日](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#Mar16-mysql)」。
...
| `BIT` | `BOOLEAN` |  |
| `TINYINT` | `INTEGER` |  |
| `BOOL`、`BOOLEAN` | `INTEGER` 在 MySQL 資料庫中，`BOOL` 和 `BOOLEAN` 資料類型會以 `TINYINT(1)` 形式儲存，支援 `-128` 到 `127` 範圍內的值。因此，`BOOL` 和 `BOOLEAN` 資料類型在轉移至 BigQuery 時會對應至 `INTEGER`。詳情請參閱[數值資料類型語法](https://dev.mysql.com/doc/refman/8.4/en/numeric-type-syntax.html)。 |  |
| `SMALLINT` | `INTEGER` |  |
| `MEDIUMINT` | `INTEGER` |  |
| `INT`、`INTEGER` | `INTEGER` |  |
| `BIGINT` | `BIGNUMERIC` |  |
