...
* BigQuery 不支援 [Cloud Storage 物件版本管理](https://docs.cloud.google.com/storage/docs/object-versioning?hl=zh-tw)。如果在 Cloud Storage URI 中加入版本編號，載入工作就會失敗。
* 如果載入的檔案有不同的結構定義，您就無法在 Cloud Storage URI 中使用萬用字元。欄位位置的任何差異都視為不同的結構定義。
...
* 為獲得最佳效能，請盡量將資料列群組大小設為至少 16 MiB。
  資料列群組越小，I/O 就會增加，載入和查詢速度也會變慢。
...
`--reference_file_schema_uri` 參數，指向參考檔案的網址。
...
1. 在「來源」部分，從「建立資料表來源」清單中選取「Google Cloud Storage」。
   然後執行下列操作：
   1. 從 Cloud Storage 值區選取檔案，或輸入 [Cloud Storage URI](https://docs.cloud.google.com/bigquery/docs/batch-loading-data?hl=zh-tw#gcs-uri)。您無法在 Google Cloud 控制台中輸入多個 URI，但可以使用[萬用字元](https://docs.cloud.google.com/bigquery/docs/batch-loading-data?hl=zh-tw#load-wildcards)。Cloud Storage 值區的位置必須與要建立、附加或覆寫的表格所在的資料集位置相同。
...
3. `source URIs` 屬性必須是完整的，且必須符合下列格式：`gs://BUCKET/OBJECT`。每個 URI 都可以包含一個「\*」[萬用字元](https://docs.cloud.google.com/bigquery/docs/batch-loading-data?hl=zh-tw#load-wildcards)。
...
| 覆寫資料表 | `--replace`或`--replace=true` | `WRITE_TRUNCATE` | 先清除資料表中所有現有資料，再寫入新的資料。這個動作也會刪除資料表結構定義、資料列層級安全性，並移除所有 Cloud KMS 金鑰。 |
...
2. 在「目的地」部分，指定下列詳細資料：
   ...
   3. 確認「Table type」(資料表類型) 欄位已設為「Native table」(原生資料表)。
3. 在「Schema」(結構定義) 區段中，無需採取任何行動。Parquet 檔案的結構定義為自述式。
**注意：**您可以在附加或覆寫資料表時修改資料表的結構定義。如要進一步瞭解載入作業期間支援的結構定義變更，請參閱「[修改資料表結構定義](https://docs.cloud.google.com/bigquery/docs/managing-table-schemas?hl=zh-tw)」一文。4. 選用：指定**分區和分群設定**。詳情請參閱「[建立分區資料表](https://docs.cloud.google.com/bigquery/docs/creating-partitioned-tables?hl=zh-tw)」和「[建立及使用分群資料表](https://docs.cloud.google.com/bigquery/docs/creating-clustered-tables?hl=zh-tw)」。您無法藉由附加或覆寫的方式，將資料表轉換為分區資料表或分群資料表。 Google Cloud 控制台不支援在載入工作中對分區或分群資料表執行附加或覆寫作業。
5. 按一下「進階選項」，然後執行下列操作：
...
1. 建立指向 Cloud Storage 中來源資料的 `load` 工作。
2. (選擇性操作) 在[工作資源](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/jobs?hl=zh-tw)的 `jobReference` 區段中，於 `location` 屬性指定您的[位置](https://docs.cloud.google.com/bigquery/docs/dataset-locations?hl=zh-tw)。
3. `source URIs` 屬性必須完整且符合下列格式：`gs://BUCKET/OBJECT`。您可以使用以逗號分隔清單的形式包含多個 URI。請注意，系統也支援使用[萬用字元](https://docs.cloud.google.com/bigquery/docs/batch-loading-data?hl=zh-tw#load-wildcards)。
...
Parquet `UINT_8`、`UINT_16`、`UINT_32` 和 `UINT_64` 類型為不帶正負號。將這些類型的值載入 BigQuery 帶正負號的 `INTEGER` 資料欄時，BigQuery 會將這些值視為不帶正負號。如果是 `UINT_64`，如果無正負號值超過 `INTEGER` 的最大值 9,223,372,036,854,775,807，系統就會傳回錯誤。

### Decimal 邏輯類型

`Decimal` 邏輯型別可以轉換為 `NUMERIC`、`BIGNUMERIC` 或 `STRING` 型別。轉換後的型別取決於 `decimal` 邏輯型別的精確度和比例參數，以及指定的小數目標型別。請按照下列方式指定小數目標型別：
...
* 針對[含有外部來源的資料表](https://docs.cloud.google.com/bigquery/external-data-sources?hl=zh-tw)執行查詢：
  使用 [`ExternalDataConfiguration.decimalTargetTypes`](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tables?hl=zh-tw#ExternalDataConfiguration.FIELDS.decimal_target_types) 欄位。
...
`Enum` 邏輯類型可以轉換為 `STRING` 或 `BYTES`。請按照下列方式指定轉換後的目標類型：
...
* 如果是使用 [`bq mk`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_mk) 建立的永久外部資料表：
  請使用 [`--parquet_enum_as_string`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#mk-table) 旗標。

### 清單邏輯類型

您可以為 Parquet `LIST` 邏輯型別啟用結構定義推論功能。BigQuery 會檢查 `LIST` 節點是否為[標準形式](https://github.com/apache/parquet-format/blob/master/LogicalTypes.md#lists)，或是[向後相容規則](https://github.com/apache/parquet-format/blob/master/LogicalTypes.md#backward-compatibility-rules)所述的形式之一：
...
* 如要使用 [`bq mk`](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_mk) 建立永久外部資料表，請使用 [`--parquet_enable_list_inference` 旗標](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#mk-table)。
...
您也可以載入 [GeoParquet](https://geoparquet.org) 檔案。在這種情況下，GeoParquet 中繼資料描述的資料欄預設會解讀為 `GEOGRAPHY` 型別。您也可以提供明確的結構定義，將原始 WKB 資料載入 `BYTES` 資料欄。詳情請參閱「[載入 GeoParquet 檔案](https://docs.cloud.google.com/bigquery/docs/geospatial-data?hl=zh-tw#loading_geoparquet_files)」。

### 資料欄名稱轉換

資料欄名稱可包含英文字母 (a-z、A-Z)、數字 (0-9) 或底線 (\_)，且開頭必須是英文字母或底線。如果使用彈性資料欄名稱，BigQuery 支援以數字做為資料欄名稱開頭。以數字做為資料欄開頭時請務必謹慎，因為使用 BigQuery Storage Read API 或 BigQuery Storage Write API 時，必須特別處理彈性資料欄名稱。如要進一步瞭解彈性資料欄名稱支援功能，請參閱[彈性資料欄名稱](#flexible-column-names)。

資料欄名稱的長度上限為 300 個字元，且不得使用以下任何一個前置字串：
...
* 請勿為資料表及其資料欄使用相同名稱。
* 請避免使用 `_field_` 做為資料欄名稱前置字串。系統保留的前置字元會導致查詢期間自動重新命名。舉例來說，`SELECT _field_ FROM project1.dataset.test` 查詢會傳回名為 `_field_1` 的資料欄。如要查詢這個名稱的資料欄，請使用別名控制輸出內容。
* 為資料表指派其他別名。舉例來說，下列查詢會為資料表 `project1.dataset.test` 指派資料表別名 `t`：
...
資料欄名稱的選擇更加靈活，包括擴大支援非英文語言的字元，以及其他符號。如果彈性資料欄名稱是[加上引號的 ID](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/lexical?hl=zh-tw#quoted_identifiers)，請務必使用倒引號 (`` ` ``) 字元括住。

彈性資料欄名稱支援下列字元：

* 任何語言的任何字母，以 Unicode 規則運算式 [`\p{L}`](https://www.unicode.org/reports/tr44/#General_Category_Values) 表示。
* 任何語言的任何數字字元，以 Unicode 正則運算式 [`\p{N}`](https://www.unicode.org/reports/tr44/#General_Category_Values) 表示。
...
* 任何預期會與另一個字元搭配使用的標記，以 Unicode 規則運算式 [`\p{M}`](https://www.unicode.org/reports/tr44/#General_Category_Values) 表示。例如：重音符號、變音符號或外框。
* 下列特殊字元：
  + 以 Unicode 規則運算式 `\u0026` 表示的半形連接號 (`&`)。
  + 百分比符號 (`%`)，以 Unicode 規則運算式 `\u0025` 表示。
  + 等號 (`=`)，以 Unicode 規則運算式 `\u003D` 表示。
  + 加號 (`+`)，以 Unicode 規則運算式 `\u002B` 表示。
  + 冒號 (`:`)，以 Unicode 規則運算式 `\u003A` 表示。
  + 以 Unicode 規則運算式 `\u0027` 表示的單引號 (`'`)。
  + 小於符號 (`<`)，以 Unicode 正則運算式 `\u003C` 表示。
