...
*容量使用承諾*是指購買 BigQuery 運算容量，但有最短承諾使用期。使用「版本」建立預留項目時，您可以選擇是否購買容量承諾，但購買容量承諾可節省費用。

承諾是地區性資源。在某個區域或多區域購買的使用承諾，無法用於其他區域或多區域。使用承諾無法在區域之間移動，也無法在區域和多區域之間移動。承諾無法在專案之間轉移。
...
如要預留容量一段時間，可以購買[容量使用承諾](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#slot_commitments)。這項功能可提供折扣，協助您節省費用。如要進一步瞭解具體費用，請參閱 [BigQuery 定價](https://cloud.google.com/bigquery/pricing?hl=zh-tw#capacity_compute_analysis_pricing)。
...
承諾是地區性資源。在某個地區或多地區購買的使用承諾無法用於其他地區或多地區。使用承諾無法在區域之間移動，也無法在區域和多區域之間移動。承諾無法在專案之間轉移。
...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `ADMIN_PROJECT_ID`：[管理專案](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#admin-project)的專案 ID，該專案將保留這項承諾的擁有權
...
您隨時可以選擇將承諾轉換為較長期的承諾類型。即使要從舊版方案轉換為版本，也能使用這項功能。
...
* `RENEWAL_PLAN`：[續約](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#renew-commitments)方案
  ...
  只有在 `PLAN_TYPE` 為 `ANNUAL` 時，這項設定才會生效。
* `COMMITMENT_ID`：承諾 ID
...
您可以將承諾使用合約分割為兩份。如果您想[續訂](#renewing-commitments)部分約期，這項功能就非常實用。舉例來說，如果您有 1,000 個運算單元的年度承諾，可以將 300 個運算單元分割為新的承諾，原承諾則保留 700 個運算單元。然後以年費率續訂 700 個名額，並將 300 個名額轉換為三年期方案。承諾的運算單元數量須為 50 的倍數。
...
4. 選取要拆分的約定。
5. 按一下「分割」。
6. 在「Split commitment」(拆分承諾) 頁面中，使用「Configure split」(設定拆分) 滑桿，以 50 個運算單元為單位，選取要拆分到每個部分的運算單元數量。
...
* `ADMIN_PROJECT_ID`：專案 ID
* `LOCATION`：承諾的[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)
* `SLOTS_TO_SPLIT`：要從原始承諾分割到新承諾的配額數量
* `COMMITMENT_ID`：承諾 ID
...
您可以將多個約期合併為一個約期。合併的承諾使用合約必須屬於同一類型 (`ANNUAL` 或 `THREE_YEAR`)。合併後承諾使用合約的結束日期，是原始承諾使用合約中最晚的結束日期。如果任何承諾使用合約的結束日期較早，系統會將這些合約的結束日期延長至較晚的日期，並按比例收取這些時段的費用。
...
續約承諾後，「開始時間」值不會變更。這是指原始承諾的開始時間。「結束時間」是續約承諾到期的時間。舉例來說，如果您在 2022 年 12 月 13 日建立一項年度約期，並在 2023 年 12 月 13 日續約。如果您在 2023 年 12 月 14 日查看約期詳細資料，**開始時間**的值會是 2022 年 12 月 13 日，**結束時間**的值會是 2024 年 12 月 12 日。

系統一律會收取基準運算單元費用。如果容量承諾使用合約到期，您可能需要手動調整預留項目中的基準運算單元數量，以免產生不必要的費用。舉例來說，假設您有 1 年期承諾，其中包含 100 個運算單元，以及 100 個基準運算單元的預留項目。承諾使用合約到期，且沒有續約方案。承諾到期後，您需要以[即付即用費率](https://cloud.google.com/bigquery/pricing?hl=zh-tw#on_demand_pricing)支付 100 個基準運算單元的費用。

## 控管容量使用承諾的建立作業

您可以使用 [IAM 拒絕政策](https://docs.cloud.google.com/iam/docs/deny-overview?hl=zh-tw)，進一步控管可建立容量承諾的使用者。
...
如果建立或完成時段承諾失敗或耗時過長，請考慮暫時使用[隨選價格](https://cloud.google.com/bigquery/pricing?hl=zh-tw#on_demand_pricing)。使用這項解決方案時，您可能需要在未指派給任何預留項目的其他專案中執行重要查詢，或是完全移除專案指派作業。
