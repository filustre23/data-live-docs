...
*容量使用承諾*是指購買 BigQuery 運算容量，但有最短承諾使用期。使用[版本](https://docs.cloud.google.com/bigquery/docs/editions-intro?hl=zh-tw)建立預留項目時，您可以選擇是否購買容量承諾，但這麼做有助於節省成本。

承諾是地區性資源。在某個地區或多地區購買的承諾無法用於其他地區或多地區。使用承諾無法在區域之間移動，也無法在區域和多區域之間移動。承諾無法在專案之間轉移。
...
* `bigquery.capacityCommitments.create` [管理專案](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#admin-project)，該專案會維護承諾使用合約的所有權。
...
4. 在「設定」下方：
   ...
   5. 如果您購買**年約**，請選取**合約到期後要採用的續約方案**：

      1. **每年續訂**。年約到期後，系統會自動續約一年。

      詳情請參閱「[Slot 承諾](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#slot_commitments)」。
   6. 輸入要購買的**運算單元數量**。
   7. 點選「下一步」。
5. 查看預估的購買**費用**。
6. 在「確認並提交」下方：
   ...
   1. 輸入「CONFIRM」確認購買。
   2. 按一下「購買」即可購買運算單元。
7. 如要查看承諾使用設定，請按一下「查看運算單元使用承諾」。容量佈建完成後，所要求的容量使用承諾將會呈現綠色狀態。

   **注意：** 通常很快就能佈建時段，但有時可能需要數小時。如果您有預期需求會增加的重要工作負載，請至少提前一天預留運算單元。
...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `EDITION`：與容量使用承諾相關聯的版本。您只能透過 Enterprise 或 Enterprise Plus 版本建立容量承諾。如要進一步瞭解版本，請參閱「[BigQuery 版本簡介](https://docs.cloud.google.com/bigquery/docs/editions-intro?hl=zh-tw)」。
...
* `bigquery.capacityCommitments.list` [管理專案](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#admin-project)，該專案會維護承諾使用合約的所有權。
...
如要查看管理專案的承諾，請查詢 [`INFORMATION_SCHEMA.CAPACITY_COMMITMENTS_BY_PROJECT` 檢視區塊](https://docs.cloud.google.com/bigquery/docs/information-schema-reservations?hl=zh-tw#schema)。
...
使用 [`bq ls` 指令](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#bq_ls)，並加上 [`--capacity_commitment` 旗標](https://docs.cloud.google.com/bigquery/docs/reference/bq-cli-reference?hl=zh-tw#ls-capacity_commitment-flag)，列出管理專案的承諾。
...
* `bigquery.capacityCommitments.update` [管理專案](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#admin-project)，該專案會維護承諾使用合約的所有權。
...
年約方案有續約計畫，您可以在建立或轉換為年約方案時指定。您可以在承諾期結束前，隨時變更[年約續訂方案](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#renew-commitments)。
...
您隨時可以選擇將承諾轉換為承諾時間較長的類型。即使要從舊版方案轉換為版本，也能使用這項功能。
...
您可以將承諾使用合約拆分為兩份。如果您想[續訂](#renewing-commitments)部分約期，這項功能就非常實用。舉例來說，如果您有 1,000 個運算單元的年度承諾，可以將 300 個運算單元分割為新的承諾，原承諾則保留 700 個運算單元。然後以年費率續訂 700 個名額，並將 300 個名額轉換為三年期方案。承諾的運算單元數量須為 50 的倍數。

分割承諾後，新承諾會沿用原承諾的方案和承諾結束日期。
...
4. 選取要拆分的約定。
5. 按一下「分割」。
6. 在「Split commitment」(拆分承諾) 頁面中，使用「Configure split」(設定拆分) 滑桿，以 50 個運算單元為單位，選取要拆分多少運算單元。
7. 按一下「分割」即可分割承諾。新承諾會列在「運算單元使用承諾」分頁中。
...
* `ADMIN_PROJECT_ID`：專案 ID
* `LOCATION`：承諾的[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)
* `SLOTS_TO_SPLIT`：要從原始約期分割到新約期的時段數量
* `COMMITMENT_ID`：承諾 ID
...
您可以將多個約期合併為一個。合併的承諾使用合約必須屬於同一類型 (`ANNUAL` 或 `THREE_YEAR`)。合併後承諾使用合約的結束日期，為原始承諾使用合約中最晚的結束日期。如果任何承諾使用合約的結束日期較早，系統會將這些合約的結束日期延長至較晚的日期，並按比例收取這些時段的費用。
...
1. [建立新的承諾使用合約](#purchase-commitment)。選擇適當的升級版本。請注意，這項新承諾使用合約的結束日期與現有合約不同。
2. [與支援團隊聯絡](https://docs.cloud.google.com/bigquery/docs/getting-support?hl=zh-tw)，要求取消現有約期。

## 承諾到期

承諾會在期限結束時失效。您無法刪除仍處於有效狀態的承諾。如果續訂方案設為「`NONE`」，系統會自動刪除承諾。否則，系統會視續約方案，以年約或三年約續約。如要將續訂方案變更為 `NONE`，請按照「[續訂約期](#renewing-commitments)」中的步驟操作。

續約承諾後，「開始時間」的值不會變更。這是指原始承諾的開始時間。「結束時間」是續約承諾到期的時間。舉例來說，如果您在 2022 年 12 月 13 日建立一項年度合約，並在 2023 年 12 月 13 日續約。如果您在 2023 年 12 月 14 日查看約期詳細資料，**開始時間**的值會是 2022 年 12 月 13 日，**結束時間**的值會是 2024 年 12 月 12 日。

系統一律會收取基準運算單元費用。如果容量承諾到期，您可能需要手動調整預留項目中的基準運算單元數量，以免產生不必要的費用。舉例來說，假設您有 100 個運算單元的 1 年期承諾，以及 100 個基準運算單元的預訂。承諾使用合約到期，且沒有續約方案。承諾到期後，您需要以[即付即用費率](https://cloud.google.com/bigquery/pricing?hl=zh-tw#on_demand_pricing)支付 100 個基準運算單元的費用。

## 控管容量使用承諾的建立作業

您可以運用 [IAM 拒絕政策](https://docs.cloud.google.com/iam/docs/deny-overview?hl=zh-tw)，進一步控管可建立容量承諾的使用者。
...
請注意，這些政策的優先順序高於 IAM 角色，因此即使使用者擁有 `bigquery.admin` 角色，除非刪除或修改政策，否則也無法建立承諾。

詳情請參閱「[拒絕存取資源](https://docs.cloud.google.com/iam/docs/deny-access?hl=zh-tw)」。

## 排解容量使用承諾問題

使用 BigQuery Reservations 時，如果遇到問題，請參考本節的疑難排解步驟，或許有所助益。
...
4. 選用：如果兩項承諾合約都適用，只要您為兩者購買相同的方案，即可[合併](#merging-commitments)為單一承諾合約。

如果時段承諾失敗或需要很長時間才能完成，請考慮暫時使用[隨選價格](https://cloud.google.com/bigquery/pricing?hl=zh-tw#on_demand_pricing)。使用這項解決方案時，您可能需要在未指派給任何預留項目的其他專案中執行重要查詢，或是完全移除專案指派。
