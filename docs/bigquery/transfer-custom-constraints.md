...
您可以透過自訂機構政策，允許或拒絕對 BigQuery 資料移轉服務移轉作業設定執行特定作業，以符合機構的法規遵循和安全性需求。如果建立或更新移轉作業設定的要求，未通過機構政策設定的自訂限制，要求就會失敗，並向呼叫端傳回錯誤。

## 限制

* 如要在自訂限制條件中指定資料來源，請使用 `resource.dataSourceId` 欄位和資料來源的值。如需 `resource.dataSourceId` 支援的值清單，請呼叫 [`dataSources.list` 方法](https://docs.cloud.google.com/bigquery/docs/reference/datatransfer/rest/v1/projects.dataSources/list?hl=zh-tw)。
...
5. 在「限制條件 ID」方塊中，輸入新自訂限制條件的 ID。自訂限制條件只能包含字母 (包括大寫和小寫) 或數字，例如 `custom.dtsEnableEmailNotification`。這個欄位最多可包含 70 個半形字元，不含前置字元 (`custom.`)，例如 `organizations/123456789/customConstraints/custom`。請勿在限制條件 ID 中輸入 PII 或機密資料，因為錯誤訊息可能會顯示上述資訊。
...
* `methodTypes`：強制執行限制的 REST 方法。可以是 `CREATE`，也可以是 `CREATE` 和 `UPDATE`。如果您使用 `UPDATE` 方法，在違反限制的資源上強制執行限制，除非變更可解決違規問題，否則組織政策會封鎖對該資源的變更。
...
1. 將下列檔案儲存為 `constraint-dts-enable-email.yaml`：
   ...
   這項限制會檢查新轉移設定是否已啟用電子郵件通知。如果未啟用，系統會拒絕作業。
2. 套用限制：
