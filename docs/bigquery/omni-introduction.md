...
許多機構會將資料儲存在多個公有雲中，通常這些資料最後會遭到孤立，因為難以取得所有資料的洞察。您希望使用經濟實惠、速度快的多雲端資料工具分析資料，且不會造成分散式資料治理的額外負擔。使用 BigQuery Omni 統一介面，可減少這些摩擦。

如要對外部資料執行 BigQuery 分析，您必須先[連結至 Amazon S3](https://docs.cloud.google.com/bigquery/docs/omni-aws-create-connection?hl=zh-tw) 或 [Blob 儲存體](https://docs.cloud.google.com/bigquery/docs/omni-azure-create-connection?hl=zh-tw)。如要查詢外部資料，您需要建立參照 Amazon S3 或 Blob 儲存空間資料的 [BigLake 資料表](https://docs.cloud.google.com/bigquery/docs/biglake-intro?hl=zh-tw)。

## BigQuery Omni 工具

您可以使用下列 BigQuery Omni 工具，對外部資料執行 BigQuery 分析：

* [BigQuery Omni 聯結](https://docs.cloud.google.com/bigquery/docs/biglake-intro?hl=zh-tw#cross-cloud_joins)：
  直接從 BigQuery 區域執行查詢，聯結 BigQuery Omni 區域的資料。
* [BigQuery Omni 具體化檢視表](https://docs.cloud.google.com/bigquery/docs/load-data-using-cross-cloud-transfer?hl=zh-tw#materialized_view_replicas)：
  使用[具體化檢視表副本](https://docs.cloud.google.com/bigquery/docs/load-data-using-cross-cloud-transfer?hl=zh-tw#materialized_view_replicas)，
  持續從 BigQuery Omni 區域複製資料。支援資料篩選。
* [使用 `SELECT` 進行 BigQuery Omni 移轉](https://docs.cloud.google.com/bigquery/docs/load-data-using-cross-cloud-transfer?hl=zh-tw)：
  在 BigQuery Omni 區域中，使用 `CREATE TABLE AS SELECT` 或 `INSERT INTO SELECT`
  陳述式執行查詢，然後將結果移至 BigQuery 區域。
* [使用 `LOAD`](https://docs.cloud.google.com/bigquery/docs/load-data-using-cross-cloud-transfer?hl=zh-tw) 進行 BigQuery Omni 移轉：
  使用 [`LOAD DATA` 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/load-statements?hl=zh-tw)，將資料直接從 Amazon Simple Storage Service (Amazon S3) 或 Azure Blob 儲存空間載入至 BigQuery

下表列出各項 BigQuery Omni 工具的主要功能和特色：

|  | BigQuery Omni 聯結 | BigQuery Omni 具體化檢視表 | 使用 BigQuery Omni 傳輸 `SELECT` | 使用 BigQuery Omni 傳輸 `LOAD` |
| --- | --- | --- | --- | --- |
| 建議用途 | 查詢一次性使用的外部資料，您可以與本機資料表聯結，或在兩個不同的 BigQuery Omni 區域之間聯結資料，例如 AWS 和 Azure Blob 儲存體區域之間。如果資料量不大，且快取不是主要需求，請使用 BigQuery Omni 聯結 | 設定重複或排定的查詢，持續以遞增方式轉移外部資料，其中快取是主要需求。舉例來說，維護資訊主頁 | 從 BigQuery Omni 區域查詢外部資料，供一次性使用，並傳送至 BigQuery 區域。這類查詢需要手動控制項 (例如快取和查詢最佳化)，且您使用的複雜查詢不支援 BigQuery Omni 聯結或 BigQuery Omni 具體化檢視區塊 | 使用排定查詢作業來移動原始資料，不需篩選即可直接遷移大型資料集 |
| 支援在遷移資料前進行篩選 | 可以。部分查詢運算子設有上限。詳情請參閱「[BigQuery Omni 聯結限制](https://docs.cloud.google.com/bigquery/docs/biglake-intro?hl=zh-tw#cross-cloud_join_limitations)」。 | 可以。部分查詢運算子設有限制，例如匯總函式和 `UNION` 運算子 | 可以。查詢運算子沒有限制 | 否 |
| 轉移大小限制 | [每次轉移 60 GB](https://docs.cloud.google.com/bigquery/docs/biglake-intro?hl=zh-tw#cross-cloud_join_limitations) (每個傳輸至遠端區域的子查詢都會產生一次轉移) | 不限 | [每次轉移 60 GB](https://docs.cloud.google.com/bigquery/docs/load-data-using-cross-cloud-transfer?hl=zh-tw#limitations_2) (每個傳輸至遠端區域的子查詢都會產生一次轉移) | 不限 |
...
| 資料移轉的運算資源用量 | 使用來源 AWS 或 Azure Blob 儲存體區域中的時段 (預留或隨選) | 未使用 | 使用來源 AWS 或 Azure Blob 儲存體區域中的時段 (預留或隨選) | 未使用 |
...
BigQuery 的架構會將運算與儲存空間分開，因此 BigQuery 可視需要水平擴展，以處理非常大的工作負載。BigQuery Omni 會在其他雲端中執行 BigQuery 查詢引擎，進而擴充這項架構。因此，您不必將資料實際移至 BigQuery 儲存空間。處理程序會在資料所在位置進行。

查詢結果可以透過安全連線傳回 Google Cloud ，例如顯示在 Google Cloud 控制台中。或者，您也可以直接將結果寫入 Amazon S3 值區或 Blob 儲存體。在這種情況下，查詢結果不會跨雲端移動。

BigQuery Omni 會使用標準 AWS IAM 角色或 Azure Active Directory 主體，存取訂閱項目中的資料。您可以將 BigQuery Omni 的讀取或寫入存取權委派給其他使用者，並隨時撤銷存取權。
...
3. BigQuery 資料平面會透過 VPN 連線，接收控制層傳送的查詢。
4. BigQuery 資料平面會從 Amazon S3 值區或 Blob 儲存體讀取資料表資料。
5. BigQuery 資料平面會在資料表資料上執行查詢工作。
   系統會在指定的 AWS 或 Azure 區域處理資料表資料。
6. 查詢結果會透過 VPN 連線，從資料層傳輸至控制層。
7. BigQuery 控制層會接收查詢工作結果，並在查詢工作的回應中向您顯示。這類資料最多會保留 24 小時。
8. 查詢結果會傳回給您。
...
**效能：**由於資料不會跨雲端複製，且查詢會在資料所在的同一區域執行，因此您能更快取得洞察資訊。

**費用。**由於資料不會移動，因此可節省輸出資料移轉費用。由於查詢是在 Google 管理的叢集上執行，因此 AWS 或 Azure 帳戶不會產生與 BigQuery Omni 相關的額外分析費用。系統只會根據 BigQuery 計費模式，向您收取執行查詢的費用。

**安全與資料治理。**您可以在自己的 AWS 或 Azure 訂閱項目中管理資料。您不必將原始資料移出或複製到公有雲。所有運算作業都會在 BigQuery 多租戶服務中進行，該服務與您的資料位於相同區域。

**無伺服器架構。**與 BigQuery 的其他功能一樣，BigQuery Omni 也是無伺服器服務。Google 會部署及管理執行 BigQuery Omni 的叢集。您不需要佈建任何資源或管理任何叢集。

**輕鬆管理。**BigQuery Omni 提供統一的管理介面，可透過 Google Cloud存取。BigQuery Omni 可使用現有的 Google Cloud 帳戶和 BigQuery 專案。您可以在 Google Cloud 控制台中編寫 GoogleSQL 查詢，查詢 AWS 或 Azure 中的資料，並在 Google Cloud 控制台中查看結果。

**BigQuery Omni 移轉。**您可以從 S3 儲存空間和 Blob Storage 將資料載入標準 BigQuery 資料表。詳情請參閱「[將 Amazon S3 資料移轉至 BigQuery](https://docs.cloud.google.com/bigquery/docs/omni-aws-cross-cloud-transfer?hl=zh-tw)」和「[將 Blob 儲存空間資料移轉至 BigQuery](https://docs.cloud.google.com/bigquery/docs/omni-azure-cross-cloud-transfer?hl=zh-tw)」。

## 中繼資料快取功能可提升效能

您可以利用快取中繼資料，提升參照 Amazon S3 資料的 BigLake 資料表查詢效能。如果您要處理大量檔案，或是資料已透過 Apache Hive 分區，這個做法就特別實用。
...
中繼資料包括檔案名稱、分割資訊，以及來自檔案的實體中繼資料，例如列數。你可以選擇是否在資料表上啟用中繼資料快取功能。如果查詢的檔案數量龐大，且包含 Apache Hive 分區篩選器，中繼資料快取功能就能發揮最大效益。
...
啟用中繼資料快取功能後，您可以指定可接受的資料表作業中繼資料過時間隔上限。舉例來說，如果指定間隔為 1 小時，只要資料表的中繼資料在過去 1 小時內重新整理過，針對該資料表執行的作業就會使用快取中繼資料。如果快取中繼資料較舊，作業會改為從 Amazon S3 擷取中繼資料。過時間隔可指定的範圍為 30 分鐘至 7 天。

為 BigLake 或物件資料表啟用中繼資料快取時，BigQuery 會觸發中繼資料產生重新整理工作。你可以選擇自動或手動重新整理快取：

* 如果是自動重新整理，系統會以定義的間隔重新整理快取，通常是 30 到 60 分鐘。如果 Amazon S3 中的檔案是以隨機間隔新增、刪除或修改，自動重新整理快取是個不錯的方法。如要控管重新整理時間，例如在擷取、轉換及載入作業結束時觸發重新整理，請使用手動重新整理。
* 如要手動重新整理，請執行 [`BQ.REFRESH_EXTERNAL_METADATA_CACHE` 系統程序](https://docs.cloud.google.com/bigquery/docs/reference/system-procedures?hl=zh-tw#bqrefresh_external_metadata_cache)，按照符合您需求的排程重新整理中繼資料快取。如果 Amazon S3 中的檔案是以已知間隔新增、刪除或修改 (例如管道的輸出內容)，手動重新整理快取是個好方法。
...
* 如果您自動重新整理資料表的資料快取，並將過時間隔設為 30 分鐘，則如果資料快取重新整理時間較長 (通常為 30 到 60 分鐘)，您對資料表執行的部分作業可能會從 Amazon S3 讀取資料。
...
* 以 Amazon S3 和 Blob Storage 資料為基礎的 BigLake 資料表，無法使用 `OBJECT_PRIVILEGES`、`STREAMING_TIMELINE_BY_*`、`TABLE_SNAPSHOTS`、`TABLE_STORAGE`、`TABLE_CONSTRAINTS`、`KEY_COLUMN_USAGE`、`CONSTRAINT_COLUMN_USAGE` 和 `PARTITIONS`
  [`INFORMATION_SCHEMA` 檢視區塊](https://docs.cloud.google.com/bigquery/docs/information-schema-intro?hl=zh-tw)。
* Blob 儲存體不支援具體化檢視區塊。
* 不支援 JavaScript UDF。
* 系統不支援下列 SQL 陳述式：

  + [BigQuery ML](https://docs.cloud.google.com/bigquery/docs/bqml-introduction?hl=zh-tw) 陳述式。
  + [資料定義語言 (DDL) 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw)，需要 BigQuery 中管理的資料。舉例來說，`CREATE EXTERNAL TABLE`、`CREATE SCHEMA` 或 `CREATE RESERVATION` 均受支援，但 `CREATE TABLE` 不支援。
...
* [排程查詢](https://docs.cloud.google.com/bigquery/docs/scheduling-queries?hl=zh-tw)僅支援 API 或 CLI 方法。查詢的「目的地資料表」選項已停用。僅允許 [`EXPORT DATA`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/export-statements?hl=zh-tw) 查詢。
...
如果查詢結果大於 20 GiB，請考慮將結果匯出至 [Amazon S3](https://docs.cloud.google.com/bigquery/docs/omni-aws-export-results-to-s3?hl=zh-tw) 或 [Blob Storage](https://docs.cloud.google.com/bigquery/docs/omni-azure-export-results-to-azure-storage?hl=zh-tw)。如要瞭解 BigQuery Connection API 的配額，請參閱 [BigQuery Connection API](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#connection_api)。
...
* 瞭解如何[連結至 Amazon S3](https://docs.cloud.google.com/bigquery/docs/omni-aws-create-connection?hl=zh-tw) 和 [Blob Storage](https://docs.cloud.google.com/bigquery/docs/omni-azure-create-connection?hl=zh-tw)。
...
* 瞭解如何使用 [BigQuery Omni 聯結](https://docs.cloud.google.com/bigquery/docs/biglake-intro?hl=zh-tw#cross-cloud_joins)，將 [Amazon S3](https://docs.cloud.google.com/bigquery/docs/query-aws-data?hl=zh-tw) 和 [Blob 儲存體](https://docs.cloud.google.com/bigquery/docs/query-azure-data?hl=zh-tw) BigLake 資料表與 Google Cloud 資料表聯結。
