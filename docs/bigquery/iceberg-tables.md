...
如要存取 Iceberg 受管理資料表的[多陳述式交易](https://docs.cloud.google.com/bigquery/docs/transactions?hl=zh-tw)，請填寫[註冊表單](https://docs.google.com/forms/d/1lQMsrT_jj_bi_aJbcb65dOc8LJTf0Wjb9AZs9EQXkCg?hl=zh-tw)。
...
分區 Iceberg 代管資料表的 BigQuery 資料操縱語言 (DML) 陳述式和查詢，與標準 Iceberg 代管資料表相同。BigQuery 會自動將工作範圍限定在正確的分區，類似於 [Iceberg 隱藏分區](https://iceberg.apache.org/docs/latest/partitioning/#icebergs-hidden-partitioning)。此外，您新增至資料表的任何新資料都會自動分割。
...
* 使用 `tabledata.list` 進行記錄式分頁存取時，不支援 Iceberg 代管資料表。
