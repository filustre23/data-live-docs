...
# STREAMING\_TIMELINE 檢視畫面

`INFORMATION_SCHEMA.STREAMING_TIMELINE` 檢視畫面會顯示目前專案每分鐘的匯總串流統計資料。
...
**注意：** 檢視區塊名稱 `INFORMATION_SCHEMA.STREAMING_TIMELINE` 和 `INFORMATION_SCHEMA.STREAMING_TIMELINE_BY_PROJECT` 是同義詞，可以互換使用。

## 必要權限

如要查詢 `INFORMATION_SCHEMA.STREAMING_TIMELINE` 檢視畫面，您需要專案的 `bigquery.tables.list` Identity and Access Management (IAM) 權限。
...
查詢 `INFORMATION_SCHEMA` 串流檢視區時，查詢結果會包含以串流方式將資料傳入 BigQuery 的歷史和即時資訊。下列檢視表中的每個資料列，都代表串流至特定資料表的統計資料，並以 `start_timestamp` 為起始時間，每隔一分鐘匯總一次。統計資料會依錯誤代碼分組，因此每個時間戳記和表格組合，在每分鐘間隔內遇到的每個錯誤代碼都會有一列。如果要求成功，錯誤代碼會設為 `NULL`。如果特定時間範圍內沒有任何資料串流至資料表，該資料表就不會顯示相應時間戳記的資料列。`INFORMATION_SCHEMA.STREAMING_TIMELINE` 檢視表具有下列結構定義：
...
| 檢視表名稱 | 資源範圍 | 區域範圍 |
| --- | --- | --- |
| `` [PROJECT_ID.]`region-REGION`.INFORMATION_SCHEMA.STREAMING_TIMELINE[_BY_PROJECT] `` | 專案層級 | REGION |
...
* 如要查詢歐盟多區域的資料，請使用 `` `region-eu`.INFORMATION_SCHEMA.STREAMING_TIMELINE_BY_PROJECT ``
...
以下範例會計算過去 30 分鐘內，專案中所有資料表的每分鐘要求失敗次數，並依錯誤代碼細分：
...
以下範例會計算每分鐘成功和失敗的串流要求，並依錯誤代碼類別細分。這項查詢可用於填入資訊主頁。
