...
:   按照「[在 bucket 層級政策中新增主體](https://docs.cloud.google.com/storage/docs/access-control/using-iam-permissions?hl=zh-tw#bucket-add)」一文的說明，將服務帳戶新增為 Cloud Storage bucket 的主體，並授予該主體 Storage 管理員角色。您打算使用這個 Cloud Storage bucket 儲存排定執行的筆記本輸出內容。
...
如要在安排筆記本執行時間時使用筆記本執行階段範本，您需要「筆記本執行階段使用者 (`roles/aiplatform.notebookRuntimeUser`)」角色。
...
建立時間表時，系統會自動部署目前版本的筆記本。如要使用新版筆記本更新排程，請[部署筆記本](#deploy)。
...
建立排程時，系統會自動部署目前版本的筆記本。如要使用新版筆記本更新排程，請[部署筆記本](#deploy)。
...
如果新的 Google 帳戶擁有者從未建立排程，更新憑證以變更筆記本排程擁有者時，也需要手動核准。
...
5. 在「排程筆記本」對話方塊中編輯排程，然後按一下「更新排程」。
...
2. 執行下列其中一項操作：
   ...
   * 在包含所選排程的資料列中，按一下「動作」欄中的「查看動作」more\_vert，然後按一下「刪除」。
