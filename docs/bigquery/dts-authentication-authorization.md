...
BigQuery 資料移轉服務會自動將各種資料來源的資料移轉至 BigQuery。驗證和授權模型會在控制層和資料層這兩個不同階段運作，並適用於兩種使用者：移轉建立者或移轉擁有者。

### 控制層

控制層代表授權程序中的階段，經過驗證的使用者可以在這個階段控制及管理轉移設定和執行作業。控制層中的使用者必須具備適當的 Identity and Access Management (IAM) 權限，才能控制及管理移轉設定和執行作業：
...
資料平面代表使用者無法直接控制的階段。在資料平面中，BigQuery 資料移轉服務可以離線模式運作資料移轉作業，並根據使用者指定的排程自動觸發移轉執行作業。在資料平面中，系統會使用移轉擁有者的憑證存取來源資料，並 (視資料來源而定) 使用移轉擁有者的憑證或 BigQuery 資料移轉服務[服務代理程式](https://docs.cloud.google.com/bigquery/docs/enable-transfer-service?hl=zh-tw#service_agent)，啟動 BigQuery 工作並將資料寫入目的地資料集。
...
* 建立轉移作業後，您就能將擁有權轉移至具備 `bigquery.transfers.update` 和 `bigquery.transfers.get` 權限的新使用者帳戶 (如果資料來源支援服務帳戶，也可以轉移至服務帳戶)。[更新憑證](https://docs.cloud.google.com/bigquery/docs/working-with-transfers?hl=zh-tw#update_credentials)時，您必須登入新帳戶。

## 外部資料來源的讀取權限授權

讀取來源資料所需的權限可能因資料來源而異。舉例來說，如要存取 [Google Ads](https://docs.cloud.google.com/bigquery/docs/google-ads-transfer?hl=zh-tw#required_permissions)，您需要 Google Ads 客戶 ID 的讀取權限。同樣地，如要存取 [Google Play](https://docs.cloud.google.com/bigquery/docs/play-transfer?hl=zh-tw#required_permissions)，您需要在 Google Play 管理中心擁有報表存取權。如要進一步瞭解特定資料來源的權限，請參閱各資料來源的轉移指南。

視轉移擁有者的身分類型而定，您必須使用不同的授權方法來擷取存取權杖，才能存取來源資料。

### 以服務帳戶身分轉移擁有權

如果使用服務帳戶做為移轉擁有者，當您為專案啟用 BigQuery 資料移轉服務 API 時，系統會自動授予必要權限。BigQuery 資料移轉服務會使用[*服務代理程式*](https://docs.cloud.google.com/iam/docs/service-account-types?hl=zh-tw#service-agents)，為使用者提供的服務帳戶 (移轉擁有者) 取得存取權杖。

啟用 BigQuery 資料移轉服務 API 時，系統會為專案建立[服務代理](https://docs.cloud.google.com/bigquery/docs/enable-transfer-service?hl=zh-tw#service_agent)，並授予服務代理 [BigQuery 資料移轉服務代理角色](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw#bigquerydatatransfer.serviceAgent) (`roles/bigquerydatatransfer.serviceAgent`)，其中包含 `iam.serviceAccounts.getAccessToken` 權限。這項權限可讓 BigQuery 資料移轉服務代理模擬移轉擁有者服務帳戶，以擷取存取權杖。
...
第一次為特定資料來源建立移轉作業時，您只需要授權 BigQuery 資料移轉服務。即使使用相同資料來源，第一次為新使用的區域建立移轉作業時，也必須再次授權。但 YouTube 頻道資料移轉作業是例外情況，每次建立 YouTube 頻道資料移轉作業時，都必須手動核准權限。

如果新擁有者先前從未在該區域建立資料來源的轉移作業，更新憑證以變更轉移擁有者時，也需要手動核准。

以下螢幕截圖顯示建立 Google Ads 轉移時的 OAuth 對話方塊介面。對話方塊會顯示要授予的資料來源專屬權限：
...
**警告：** 撤銷存取權後，這個帳戶擁有的所有區域轉移設定，日後都無法再執行轉移作業。

## 啟動 BigQuery 工作的授權

從大多數資料來源遷移資料時 (使用[排程查詢](https://docs.cloud.google.com/bigquery/docs/scheduling-queries?hl=zh-tw)或[資料集副本](https://docs.cloud.google.com/bigquery/docs/managing-datasets?hl=zh-tw#copy-datasets)遷移資料除外)，BigQuery 資料移轉服務會依賴[服務代理程式](https://docs.cloud.google.com/iam/docs/service-account-types?hl=zh-tw#service-agents)，為專案啟動 BigQuery 工作。為專案啟用 BigQuery 資料移轉服務 API 時，系統會自動將必要權限 `bigquery.job.create` 授予[服務代理程式](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw#bigquerydatatransfer.serviceAgent)。詳情請參閱「[啟用 BigQuery 資料移轉服務](https://docs.cloud.google.com/bigquery/docs/enable-transfer-service?hl=zh-tw)」。
...
從大多數資料來源遷移時 (使用[排程查詢](https://docs.cloud.google.com/bigquery/docs/scheduling-queries?hl=zh-tw)或[資料集副本](https://docs.cloud.google.com/bigquery/docs/managing-datasets?hl=zh-tw#copy-datasets)遷移除外)，BigQuery 資料移轉服務會依賴服務代理程式將資料寫入 BigQuery 目的地資料集。建立移轉作業時，BigQuery 資料移轉服務會將必要權限 `roles/bigquery.dataEditor` 授予服務代理程式。您必須具備目的地資料集的 `bigquery.datasets.update` 權限，才能順利授予權限。

使用[排程查詢](https://docs.cloud.google.com/bigquery/docs/scheduling-queries?hl=zh-tw)或[資料集副本](https://docs.cloud.google.com/bigquery/docs/managing-datasets?hl=zh-tw#copy-datasets)遷移資料時，BigQuery 資料移轉服務會使用移轉作業擁有者的憑證執行 BigQuery 工作，並將資料寫入 BigQuery 目標資料集。

**注意：** 授予 BigQuery 資料移轉服務代理程式的 `roles/bigquery.dataEditor` 角色，僅限於移轉設定中使用的目的地資料集。同一個專案下的其他 BigQuery 資料集不受影響。**警告：** 請勿從目的地資料集移除服務代理程式的 `roles/bigquery.dataEditor` 角色。BigQuery 資料移轉服務必須具備 `roles/bigquery.dataEditor` 角色才能運作。

## YouTube 頻道轉移的授權注意事項

在[YouTube 頻道資料移轉](https://docs.cloud.google.com/bigquery/docs/youtube-channel-transfer?hl=zh-tw)中，YouTube 頻道連結器只能移轉與建立資料移轉作業的使用者憑證相關聯的 YouTube 頻道資料。如要移轉不同 YouTube 頻道的資料，您必須為每個頻道建立資料移轉作業，且為每個建立的資料移轉作業使用與該 YouTube 頻道相關聯的使用者憑證。
