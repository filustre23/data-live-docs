...
10. 在「時間表」部分，選擇下列其中一個選項：
    ...
    * **執行一次**：立即執行一次資料剖析檔掃描作業，並在自動刪除時間過後移除掃描作業。這項功能為[預先發布版](https://cloud.google.com/products/?hl=zh-tw#product-launch-stages)。
      ...
      + **設定掃描後結果自動刪除**：自動刪除時間會定義資料剖析掃描在執行後保持有效的時間長度。如果資料剖析掃描未指定自動刪除時間，系統會在 24 小時後自動移除。自動刪除時間範圍從 0 秒 (立即刪除) 到 365 天。
11. 按一下「繼續」。
...
3. 按一下「最近一次的工作結果」分頁標籤。
   ...
   如果至少有一項工作順利完成，**最近一次的工作結果**分頁就會提供最近一次工作的相關資訊。這份報表會列出掃描的資料表欄，以及掃描時發現的欄統計資料。
...
Knowledge Catalog 會儲存最近 300 項工作或過去一年的資料剖析掃描記錄 (以先到者為準)。
