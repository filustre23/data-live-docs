...
過去幾十年來，各機構組織都已精通資料倉儲技術，而且有越來越多機構組織針對大量儲存資料應用敘述性分析，以深入分析其核心業務營運。傳統商業情報 (BI) 專注於查詢、報告和[線上分析處理](https://wikipedia.org/wiki/Online_analytical_processing)，在過去可能是攸關公司成敗的因素，但現已不足以因應需求。

現今機構不只需要敘述性分析瞭解過往事件，也需利用[預測分析](https://wikipedia.org/wiki/Predictive_analytics)；其中經常使用機器學習 (ML) 擷取資料模式，針對未來提出概率陳述。最終目標是開發[描述性分析](https://wikipedia.org/wiki/Prescriptive_analytics)，結合過去的經驗教訓和未來預測，自動引導即時動作。
...
BigQuery 可彈性調整且具備成本效益，可讓使用者存取結構化資料儲存空間、處理和數據分析功能。當資料量急遽增加時，這些特色至關重要，不僅可視需要調整儲存空間和處理資源，還能讓這些資料發揮效益。此外，針對剛著手大數據分析和機器學習，且想擺脫地端部署大數據系統潛在複雜性的機構，BigQuery 提供即付即用的試驗用代管服務。
...
您也可以透過預測性及描述性分析加強使用者體驗。舉例來說，您可以使用 [BigQuery 機器學習](https://docs.cloud.google.com/bigquery/docs/bqml-introduction?hl=zh-tw)、[Gemini Enterprise Agent Platform AutoML 表格](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/start/training-methods?hl=zh-tw)，或 Google 的預先訓練模型進行[圖片分析](https://cloud.google.com/vision/?hl=zh-tw)、[影片分析](https://cloud.google.com/video-intelligence/?hl=zh-tw)、[語音辨識](https://cloud.google.com/speech-to-text/?hl=zh-tw)、[自然語言](https://cloud.google.com/natural-language/?hl=zh-tw)和[翻譯](https://cloud.google.com/translate/?hl=zh-tw)。或者，也可以使用 [Gemini Enterprise Agent Platform](https://cloud.google.com/vertex-ai?hl=zh-tw) 提供自訂訓練模型，處理因應業務需求的各項用途。可能的用途如下：
...
資料治理是在資料生命週期中，從取得、使用到處置的資料管理基本原則措施。您的資料治理計畫需明確概述與資料活動相關的各項政策、程序、責任和控管機制。這份計畫可確保資料的收集、維護、使用和發布方式符合貴機構的資料完整性和安全性需求，也能讓員工探索及運用資料，充分發揮資料的潛在效益。
...
如要進一步瞭解使用 BigQuery 時的資料分析選項，請參閱「[BigQuery 數據分析總覽](https://docs.cloud.google.com/bigquery/docs/query-overview?hl=zh-tw)」一文。本主題將概略介紹報表和分析工具，協助您從資料中取得實用洞察。
