...
1. [建立 Amazon S3 儲存貯體](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html)。
...
3. 如要允許存取暫存 bucket，請使用下列指令，將 `roles/storage.objectViewer` 角色授予 [DTS 服務代理程式](https://docs.cloud.google.com/bigquery/docs/enable-transfer-service?hl=zh-tw#service_agent)：
...
* 要用於轉移作業的暫存值區 URI：
  + 如果是 AWS 代管的 Snowflake 帳戶，則需要[Amazon S3 bucket URI](#preparing-s3-bucket) 和存取憑證。
...
6. 在「Snowflake Credentials」(Snowflake 憑證) 部分，執行下列操作：
   ...
   3. 在「Authentication Mechanism」部分，選取 Snowflake 使用者驗證方法：
      ...
      * 在「Private Key」(私密金鑰)，輸入與[Snowflake 使用者相關聯的公開金鑰](#create-snowflake-user)連結的私密金鑰。
...
* `warehouse`：指定用於執行這項資料移轉的[倉儲](https://docs.snowflake.com/en/user-guide/warehouses-tasks)。
...
* `translation_output_gcs_path`：(選用) 指定 Cloud Storage 資料夾的路徑，該資料夾包含[翻譯引擎的結構定義對應檔案](https://docs.cloud.google.com/bigquery/docs/migration/snowflake-transfer-schema?hl=zh-tw)。你可以將這個欄位留空，讓 Snowflake 連接器自動偵測結構定義。
...
* `staging_s3_uri`：輸入將做為暫存區使用的 [S3 值區的 URI](#preparing-s3-bucket)。只有在 `cloud_provider` 為 `AWS` 時才需要。
...
**注意：** 如果是 Snowflake 移轉作業，CMEK 加密會處理 BigQuery 目的地資料表中的資料加密，以及 Amazon S3 或 Azure Blob Storage 上 Snowflake 移轉程序所用的中繼 Cloud Storage 租戶值區中的資料加密。

## 配額與限制

根據預設，每個資料表之每個載入工作的 BigQuery 載入配額皆為 15 TB。Snowflake 會在內部壓縮資料表資料，因此匯出的資料表大小會大於 Snowflake 回報的資料表大小。

如要縮短大型資料表的載入時間，並移除 15 TB 的 BigQuery 載入限制，請為保留項目指派作業指定 [`PIPELINE` 工作類型](https://docs.cloud.google.com/bigquery/docs/migration/snowflake-transfer?hl=zh-tw#quotas_and_limits)。
...
* 您可以透過下列方式提升資料表卸載速度：
  + 增加 Snowflake 虛擬倉庫的大小，特別是在轉移大型 Snowflake 表格 (1 TiB 以上) 時。
...
* 以 Upsert 模式進行增量轉移時，請考慮在主鍵資料欄上進行叢集處理和分區，以提升轉移效能。
