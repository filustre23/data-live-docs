...
設定適當的權限，即可建立、編輯或查看記事本。

所有具備 [Dataform 管理員角色](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#dataform.admin) (`roles/dataform.admin`) 的使用者，都擁有專案中所有筆記本的存取權。
...
如要取得建立及執行筆記本所需的權限，請要求管理員授予您專案的「BigQuery Studio 使用者」 (`roles/bigquery.studioUser`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
...
**注意：** 在專案中獲派「程式碼建立者」角色的使用者，可以使用 Dataform API 或 Dataform 指令列介面 (CLI)，列出該專案中的程式碼資產名稱。
...
**注意：** 建立 Notebook 時，BigQuery 會授予您該 Notebook 的「Dataform 管理員」角色 (`roles/dataform.admin`)。在 Google Cloud 專案中獲派 Dataform 管理員角色的所有使用者，都擁有專案中建立的所有筆記本的擁有者存取權。如要覆寫這項行為，請參閱[在建立資源時授予特定角色](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#grant-specific-role)。
...
* [BigQuery 讀取工作階段使用者](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw#bigquery.readSessionUser) (`roles/bigquery.readSessionUser`)
* [筆記本執行階段使用者](https://docs.cloud.google.com/vertex-ai/docs/general/access-control?hl=zh-tw#aiplatform.notebookRuntimeUser) (`roles/aiplatform.notebookRuntimeUser`)
* [程式碼檢視器](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#dataform.codeViewer) (`roles/dataform.codeViewer`)
...
BigQuery 的 Google Cloud 控制台筆記本庫是中心樞紐，可供您探索及使用預先建構的筆記本範本。
...
1. 前往 Google Cloud 控制台的「BigQuery」頁面。
   ...
   [前往「BigQuery」](https://console.cloud.google.com/bigquery?hl=zh-tw)
2. 如要開啟筆記本庫，請在編輯器面板的分頁列中，按一下「SQL 查詢」add\_box旁的箭頭 arrow\_drop\_down，然後依序點選「筆記本」**> 所有範本**。
3. 在筆記本範本庫中選取範本。例如，您可以選取「開始使用 BigQuery DataFrames」。

   系統會開啟新筆記本，其中包含的儲存格會顯示針對 `bigquery-public-data.ml_datasets.penguins` 公開資料集的範例查詢。
...
5. 在資料表旁邊，依序點選 more\_vert「動作」和「在 Python 筆記本中開啟」>。

   系統會開啟新筆記本，其中包含針對所選表格顯示範例查詢的儲存格。
...
3. **在「查詢結果」窗格中，依序點選「開啟方式」>「Notebook」**。
   ...
   系統會開啟新筆記本，其中包含的儲存格含有可傳回查詢 SQL 和查詢結果的程式碼。
...
3. 在「Explorer」窗格中展開專案，然後執行下列其中一個步驟：
   ...
   * 依序點選「筆記本」旁的 more\_vert「查看動作」>「上傳至筆記本」。
   * 在 Google Cloud 專案名稱旁，依序點選 more\_vert「查看動作」「上傳至專案」「筆記本」。
4. 在「上傳筆記本」對話方塊的「筆記本」欄位中，按一下「瀏覽」，然後選取要上傳的筆記本。
5. 選用：在「筆記本名稱」欄位中編輯筆記本名稱。
6. 在「Region」(區域) 欄位中，選取要上傳筆記本的區域。
7. 按一下「上傳」。

你可以透過「Explorer」窗格存取筆記本。

## 連線到執行階段

請參閱下列各節，瞭解如何將筆記本連結至 [Gemini Enterprise Agent Platform 執行階段](https://docs.cloud.google.com/colab/docs/create-runtime?hl=zh-tw)。執行階段是運算資源，可執行筆記本中的程式碼。執行階段必須與筆記本位於相同區域。

如要進一步瞭解執行階段，請參閱[執行階段和執行階段範本](https://docs.cloud.google.com/colab/docs/runtimes?hl=zh-tw)。

**注意：** 如果您使用 [VPC Service Controls](https://docs.cloud.google.com/vpc-service-controls/docs/overview?hl=zh-tw)，請務必先[設定私人 Google 存取權搭配 VPC Service Controls](https://docs.cloud.google.com/vpc-service-controls/docs/private-connectivity?hl=zh-tw)，再連線至執行階段。否則，服務會傳回錯誤 `Failed to connect to Runtime Network
...
如要使用預設執行階段以外的執行階段，請先在 Agent Platform 中[建立該額外執行階段](https://docs.cloud.google.com/gemini-enterprise-agent-platform/notebooks/colab/create-runtime?hl=zh-tw)。
...
3. 在「Explorer」窗格中展開專案，然後按一下「Notebooks」。
4. 按一下筆記本名稱即可開啟。
5. 在筆記本中，按一下「連線」旁的下拉式選單，然後按一下「連線到執行階段」。arrow\_drop\_down
6. 按一下「連線至現有的執行階段」。
...
5. 在筆記本中，按一下「連線」旁的下拉式選單，然後按一下「連線到執行階段」。arrow\_drop\_down
6. 按一下「建立新的執行階段」。
7. 在「執行階段範本」中，選取要使用的[Agent Platform 執行階段範本](https://docs.cloud.google.com/colab/docs/create-runtime-template?hl=zh-tw)。
...
* **SQL 儲存格**：使用 [SQL 儲存格](https://docs.cloud.google.com/colab/docs/sql-cells?hl=zh-tw)執行 GoogleSQL 查詢。查詢輸出內容會自動儲存為 DataFrame，名稱與儲存格標題相同。您可以在單一 SQL 儲存格中執行多個 SQL 陳述式，但只有最後一個陳述式的結果會儲存至 DataFrame。
  ...
  您可以在運算式中參照 Python 變數，或將 BigQuery DataFrames 當做查詢中的資料表，方法是將變數名稱放在大括號 (`{ }`) 中：
...
3. 在「Explorer」窗格中展開專案，然後按一下「Notebooks」。
4. 按一下要停用儲存輸出的筆記本。
5. 如要展開選單列，請依序點選「keyboard\_arrow\_down」keyboard\_arrow\_down「切換標題顯示設定」。
6. 依序點選「編輯」**>「筆記本設定」**。
...
* 瞭解如何[在 BigQuery Studio Git 存放區掛接中存取筆記本檔案](https://docs.cloud.google.com/bigquery/docs/git-repositories?hl=zh-tw#notebook-mount)。
