...
將 Agent Platform 使用者角色授予連線的服務帳戶。

如果您打算在建立遠端模型時，將端點指定為網址 (例如 `endpoint = 'https://us-central1-aiplatform.googleapis.com/v1/projects/myproject/locations/us-central1/publishers/google/models/gemini-2.5-pro'`)，請在網址指定的專案中授予這個角色。
...
* `SAFETY_SETTINGS`：`ARRAY<STRUCT<STRING AS category, STRING AS
  threshold>>` 值，可設定內容安全門檻來篩選回應。結構體中的第一個元素會指定危害類別，第二個元素則會指定對應的封鎖門檻。模型會篩除違反這些設定的內容。每個類別只能指定一次。舉例來說，您無法同時指定 `STRUCT('HARM_CATEGORY_DANGEROUS_CONTENT' AS category,
