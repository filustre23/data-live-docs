...
如要取得建立資料集、資料表、複製資料、查詢資料及建立 BI Engine 預留空間所需的權限，請要求管理員授予您專案的下列 IAM 角色：
...
5. 在「建立資料集」頁面中，執行下列操作：
   ...
   * 在本教學課程中，您可以選取「啟用資料表到期時間」，然後指定資料表到期前天數。
...
8. 按一下「複製」。
9. **選用步驟：複製作業完成後，請展開「**`PROJECT_NAME`」>「biengine\_tutorial」**，然後依序點選「311\_service\_requests\_copy」>「預覽」**，確認表格內容。將 **`PROJECT_NAME`** 替換為本教學課程的 Google Cloud 專案名稱。
