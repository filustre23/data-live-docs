...
* `EXPIRATION_TIME`：從目前時間到有效期限的時間差 (以秒為單位)。
* `PROJECT_ID`：包含快照的專案 ID。
* `DATASET_NAME`：包含快照的資料集名稱。
* `SNAPSHOT_NAME`：快照名稱。
...
6. 在顯示的「新增主體」窗格中，輸入要授予表格快照存取權的[主體](https://docs.cloud.google.com/iam/docs/principals-overview?hl=zh-tw) ID。
