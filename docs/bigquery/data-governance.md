...
Lakehouse 執行階段目錄可讓您使用多個資料處理引擎，透過單一結構定義查詢單一資料副本，不必重複資料。可用的資料處理引擎包括 BigQuery、Apache Spark、Apache Flink 和 Apache Hive。資料可儲存在 BigQuery 儲存空間資料表、BigQuery 中的代管 Apache Iceberg 資料表，或 BigLake 外部資料表等位置。
