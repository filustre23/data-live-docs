...
本文說明如何使用 Google Cloud 主控台建立 BigQuery ML 模型。
...
3. 啟用 BigQuery 和 BigQuery Connection API。
   ...
   **啟用 API 時所需的角色**
   ...
   [啟用 API](https://console.cloud.google.com/apis/enableflow?apiid=bigquery.googleapis.com%2Cbigqueryconnection.googleapis.com&hl=zh-tw)
...
* 下列遠端模型需要[Cloud 資源連結](https://docs.cloud.google.com/bigquery/docs/create-cloud-resource-connection?hl=zh-tw)：
  ...
  + [透過 Gemini Enterprise Agent Platform 和合作夥伴模型使用遠端模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model?hl=zh-tw)
  ...
  + [Gemini Enterprise Agent Platform 中的自訂模型與遠端模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-https?hl=zh-tw)
...
* 迴歸：
  ...
  + [提升樹迴歸](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-boosted-tree?hl=zh-tw)
...
* 分群：[K-means](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-kmeans?hl=zh-tw)
...
2. 點選左側窗格中的 explore「Explorer」。
   ...
   如果沒有看到左側窗格，請按一下 last\_page「Expand left pane」(展開左側窗格)，開啟窗格。
3. 在「Explorer」窗格中，按一下「Datasets」，然後點選您建立的資料集。
...
10. 按一下「繼續」。
11. 在「模型選項」頁面中，選取模型類型。可選取的模型類型會因您選擇的模擬目標而異。
12. 在「訓練資料」部分，執行下列任一操作：

    * 選取「資料表/檢視畫面」，從資料表或檢視畫面取得訓練資料，然後選取專案、資料集，以及檢視畫面或資料表名稱。
    * 選取「查詢」，從已儲存的查詢取得訓練資料，然後選取已儲存的查詢。
13. 在「選取的輸入標籤資料欄」中，從表格、檢視畫面或查詢中選擇要用做模型輸入內容的資料欄。
14. 如有「必要選項」部分，請指定要求的資料欄資訊：
...
16. 選用：如有「超參數調整」專區，您可以指定 **NUM\_TRIALS** 選項，為模型啟用 [超參數調整](/bigquery/docs/hyperparameter-tuning-tutorial)。可供超參數調整的引數會因您建立的模型類型而異。
17. 按一下「建立模型」。
...
* [Agent Platform 模型或合作夥伴模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model?hl=zh-tw)
...
10. 在「遠端連線」部分中，執行下列任一操作：
    ...
    * 如果沒有設定預設連線，或缺少適當的角色，請選取「Cloud resource connection」(Cloud 資源連結)。
      ...
      3. 在「連線」部分，選取要用於遠端模型的連線，或是選取「建立新連線」來建立新連線。
         ...
         **重要事項：** 建立新連線時，請務必先授予連線服務帳戶適當的角色，再繼續操作。如要進一步瞭解要授予哪些角色，請參閱您要建立的遠端模型類型的參考文件。
11. 在「必要選項」部分，執行下列其中一項操作：
    ...
    * 如果是開放模型以外的遠端模型，請複製並貼上要使用的端點。這是部署至 Agent Platform 的模型共用公開端點，格式為 `https://location-aiplatform.googleapis.com/v1/projects/project/locations/location/endpoints/endpoint_id`。詳情請參閱 [`ENDPOINT`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-open?hl=zh-tw#endpoint) 的說明。
12. 按一下「建立模型」。
13. 模型建立完成後，按一下「前往模型」即可查看模型詳細資料。

## 透過自訂模型建立遠端模型

使用這個程序，透過[部署至 Gemini Enterprise Agent Platform 的自訂模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-https?hl=zh-tw)建立遠端模型。
...
9. 在「Model options」(模型選項) 頁面的「Remote connection」(遠端連線) 部分，執行下列其中一項操作：
   ...
   * 如果沒有設定預設連線，或缺少適當的角色，請選取「Cloud resource connection」(Cloud 資源連結)。
     ...
     3. 在「連線」部分，選取要用於遠端模型的連線，或是選取「建立新連線」來建立新連線。
        ...
        **重要事項：** 建立新連線時，請務必先授予連線服務帳戶適當的角色，再繼續操作。如要進一步瞭解要授予哪些角色，請參閱您要建立的遠端模型類型的參考文件。
10. 在「Required options」(必要選項) 部分，指定要使用的端點。這是部署至 Agent Platform 的模型所共用的公開端點，格式為 `https://location-aiplatform.googleapis.com/v1/projects/project/locations/location/endpoints/endpoint_id`。詳情請參閱 [`ENDPOINT`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-https?hl=zh-tw#endpoint)。
11. 按一下「建立模型」。
...
10. 在「遠端連線」部分中，執行下列任一操作：
    ...
    * 如果沒有設定預設連線，或缺少適當的角色，請選取「Cloud resource connection」(Cloud 資源連結)。
      ...
      3. 在「連線」部分，選取要用於遠端模型的連線，或是選取「建立新連線」來建立新連線。
         ...
         **重要事項：** 建立新連線時，請務必先授予連線服務帳戶適當的角色，再繼續操作。如要進一步瞭解要授予哪些角色，請參閱您要建立的遠端模型類型的參考文件。
11. 在「Required options」(必要選項) 部分，選取要使用的 Cloud AI 服務類型。
12. 在「選用」部分，指定使用 `CLOUD_AI_DOCUMENT_V1` 服務時的
    [文件處理器](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-service?hl=zh-tw#document_processor)
    資訊。如果您使用 `CLOUD_AI_SPEECH_TO_TEXT_V2` 服務，可以選擇指定[語音辨識器](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model-service?hl=zh-tw#speech_recognizer)資訊。
13. 按一下「建立模型」。
