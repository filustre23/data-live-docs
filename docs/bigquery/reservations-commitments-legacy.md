...
**附註：** 只有許可清單中的客戶可以使用舊版預留項目，包括存取固定費率帳單或特定約期。如要確認您是否能使用這些舊版功能，請與管理員聯絡。固定費率計費模式會定義運算資源的計費方式，但固定費率預留項目和承諾使用方案的功能與 Enterprise 版的運算單元相同。
...
7. 如要查看承諾使用合約，請按一下「查看運算單元使用承諾」。容量佈建完成後，所要求的容量使用承諾將會呈現綠色狀態。
   ...
   **注意：** 通常很快就能佈建位置，但有時可能需要數小時。如果重要工作負載的預期需求量會增加，請至少提前一天預留運算單元。

首次購買容量時，系統會建立 `default` 預訂。
...
1. 刪除待處理的承諾使用合約。
2. 購買運算單元數量較少的新承諾使用合約。視容量而定，較小的使用承諾可能會立即生效。
