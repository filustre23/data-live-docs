...
承諾是地區性資源。在某個地區或多地區購買的承諾無法用於其他地區或多地區。使用承諾無法在區域之間移動，也無法在區域與多區域之間移動。
...
承諾是地區性資源。在某個地區或多地區購買的承諾無法用於其他地區或多地區。使用承諾無法在區域之間移動，也無法在區域和多區域之間移動。
...
* `RENEWAL_PLAN`：[續約](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#renew-commitments)方案
  ...
  只有在 `PLAN_TYPE` 為 `ANNUAL` 時，這項設定才會生效。如果 `PLAN_TYPE` 為 `MONTHLY`，請省略這個旗標。
* `COMMITMENT_ID`：承諾 ID
...
您可以將承諾使用合約拆分為兩份。如果您想[續訂](#renewing-commitments)部分約期，這項功能就非常實用。舉例來說，如果您有 1,000 個運算單元的年約，可以將 300 個運算單元分割到新的承諾使用合約，原承諾使用合約則保留 700 個運算單元。這樣一來，您就能以年費率續訂 700 個運算單元，並在結束日期後將 300 個運算單元轉換為彈性運算單元。
...
您可以將多個約期合併為一個。合併的承諾使用合約必須屬於相同類型 (`FLEX`、`MONTHLY`、`ANNUAL` 或 `THREE_YEAR`)。合併後承諾使用合約的結束日期，是原始承諾使用合約中最晚的結束日期。如果任何承諾使用合約的結束日期較早，系統會將這些合約的結束日期延長至較晚的日期，並按比例收取這些時段的費用。
...
如要刪除容量承諾，您需要下列 Identity and Access Management (IAM) 權限：

* `bigquery.capacityCommitments.delete` [管理專案](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#admin-project)，該專案會維護承諾使用合約的所有權。
