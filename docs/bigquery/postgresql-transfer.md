...
* 資料轉移期間，PostgreSQL 連接器會識別已建立索引和已分割的鍵欄，以便平行批次轉移資料。因此，我們建議您在資料表中指定主鍵欄或使用索引欄，以提升資料轉移的效能並降低錯誤率。請考量下列事項：
...
PostgreSQL 連接器支援傳輸層安全 (TLS) 設定，可加密傳輸至 BigQuery 的資料。PostgreSQL 連接器支援下列 TLS 設定：
...
**注意：** 如要要求提供有關增量轉移的意見回饋或支援，請傳送電子郵件至 [dts-preview-support@google.com](mailto:dts-preview-support@google.com)。
您可以設定*完整*資料轉移，在每次資料轉移時，轉移 PostgreSQL 資料集的所有資料。
...
4. 在「Data source details」(資料來源詳細資料) 部分執行下列操作：
   ...
   * 如要**啟用舊版對應**，請選取「true」 (預設)，使用[舊版資料類型對應](#data_type_mapping)。選取「false」即可使用更新後的資料類型對應。如果您要進行增量轉移，這個值必須為 **false**。如要進一步瞭解資料類型對應更新，請參閱「[2027 年 3 月 16 日](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#Mar16-postgresql)」。
     資料庫伺服器。
...
* PARAMETERS：已建立移轉設定的 JSON 格式參數。例如：`--params='{"param":"param_value"}'`。以下是 PostgreSQL 轉移作業的參數：
  ...
  + `primaryKeys`：將資料表中的資料欄指定為主鍵。
    如要進行增量轉移，這個欄位為必填。
  + `connector.legacyMapping`：設為 `true` (預設值) 即可使用[舊版資料類型對應](#data_type_mapping)。如要使用更新後的資料類型對應，請將這個值設為 `false`。如果您要進行增量轉移，這個值必須是 `false`。如要進一步瞭解資料類型對應更新，請參閱「[2027 年 3 月 16 日](https://docs.cloud.google.com/bigquery/docs/transfer-changes?hl=zh-tw#Mar16-postgresql)」。
