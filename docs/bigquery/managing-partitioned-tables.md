...
如要深入瞭解 BigQuery 中的 IAM 角色，請參閱[存取權控管](https://docs.cloud.google.com/bigquery/access-control?hl=zh-tw)。
...
建立分區資料表時，您可以要求對該資料表的所有查詢，都必須包含根據分區資料欄篩選的述詞篩選器 (`WHERE` 子句)。這項設定可以提升效能並降低成本，因為 BigQuery 可以使用篩選器，修剪不符合述詞的分區。這項規定也適用於參照分區資料表的檢視表和具體化檢視表查詢。
...
如果分區資料表設有「需要分區篩選器」，則對該資料表的每項查詢都必須至少包含一個述詞，且該述詞只會參照分區欄。如果查詢沒有這類述詞，就會傳回下列錯誤：
...
**注意：** 所需權限與[複製資料表](https://docs.cloud.google.com/bigquery/docs/managing-tables?hl=zh-tw#copy-table)相同。
