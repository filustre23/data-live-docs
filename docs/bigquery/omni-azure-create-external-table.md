...
使用 [`CREATE SCHEMA` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#create_schema_statement)。以下範例會在 `azure-eastus2` 區域建立資料集：
...
5. 在「Create table」(建立資料表) 頁面的「Source」(來源) 區段中，執行下列操作：
   ...
   3. 在「File format」(檔案格式) 中，選取 Azure 的資料格式。支援的格式包括 **AVRO**、**CSV**、**DELTA\_LAKE**、**ICEBERG**、**JSONL**、**ORC** 和 **PARQUET**。
6. 在「目的地」部分，執行下列操作：

   1. 在「Dataset」(資料集) 部分，選擇適當的資料集。
   2. 在「Table」(資料表) 欄位中，輸入資料表名稱。
   3. 確認「Table type」(資料表類型) 已設為 [External table] (外部資料表)。
   4. 在「Connection ID」(連線 ID) 專區中，從下拉式選單選擇適當的連線 ID。如需連線相關資訊，請參閱「[連線至 Blob 儲存空間](https://docs.cloud.google.com/bigquery/docs/omni-azure-create-connection?hl=zh-tw)」。
...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `AZURE_LOCATION`： Google Cloud中的 Azure 位置，例如 `azure-eastus2`
...
舉例來說，下列指令會建立新的 BigLake 資料表 `my_dataset.my_table`，可查詢儲存在路徑 `azure://account_name.blob.core.windows.net/container/path` 的 Blob 儲存體資料，且在位置 `azure-eastus2` 中有讀取連線：
...
**注意：**如要覆寫預設專案，請使用 `--project_id=PROJECT_ID` 參數。將 `PROJECT_ID` 替換為您的專案 ID。 Google Cloud
...
您可以為 Blob Storage 中的 Hive 分區資料建立 BigLake 資料表。建立外部分區資料表後，您就無法變更分區鍵。如要變更分割區鍵，您必須重新建立資料表。
...
5. 在「來源」部分，指定下列詳細資料：
   ...
   1. 在「Create table from」(使用下列資料建立資料表) 部分，選取下列任一選項：
...
下列範例會在 Blob 儲存空間中的分區資料上建立 BigLake 資料表。指定結構定義。
...
Delta Lake 是開放原始碼資料表格式，支援 PB 級資料表。Delta Lake 資料表可做為臨時和永久資料表查詢，並支援做為 [BigLake](https://docs.cloud.google.com/bigquery/docs/biglake-intro?hl=zh-tw) 資料表。
...
TABLE`](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#create_external_table_statement) 陳述式和 Delta Lake 格式建立外部資料表：
...
### 透過 Delta Lake 進行 BigQuery Omni 轉移
...
如要為現有服務範圍新增輸出政策，請使用 [`gcloud access-context-manager perimeters update` 指令](https://docs.cloud.google.com/sdk/gcloud/reference/access-context-manager/perimeters/update?hl=zh-tw)。
舉例來說，下列指令會將 `egress.yaml` 檔案中定義的輸出政策，新增至名為 `omniPerimeter` 的現有 service perimeter：
...
BigQuery 管理員可以建立網路規則，授予 BigQuery Omni 存取 Blob 儲存體資源的權限。這項功能可確保只有授權的 BigQuery Omni VPC 能與 Blob 儲存體互動，進一步提升資料安全性。
