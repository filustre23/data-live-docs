...
2. 在查詢編輯器中輸入下列陳述式：
   ...
   * `AZURE_LOCATION`：Azure 位置，例如 Google Cloud中的 `azure-eastus2`
...
舉例來說，下列指令會建立新的 BigLake 資料表 `my_dataset.my_table`，可查詢儲存在路徑 `azure://account_name.blob.core.windows.net/container/path` 的 Blob 儲存體資料，並在位置 `azure-eastus2` 中建立讀取連線：
...
* table\_name：資料表名稱。
* connection\_name：連線名稱，連線必須識別 [Amazon S3](https://docs.cloud.google.com/bigquery/docs/omni-aws-create-connection?hl=zh-tw) 或 [Blob Storage](https://docs.cloud.google.com/bigquery/docs/omni-azure-create-connection?hl=zh-tw) 來源。
...
BigQuery 管理員可以建立網路規則，授予 BigQuery Omni 存取 Blob 儲存體資源的權限。這可確保只有授權的 BigQuery Omni VPC 才能與 Blob 儲存體互動，進而提升資料安全性。
