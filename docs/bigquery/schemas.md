...
資料欄名稱的命名方式更靈活，包括擴大支援英文以外語言的字元，以及其他符號。如果彈性資料欄名稱是[加上引號的 ID](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/lexical?hl=zh-tw#quoted_identifiers)，請務必使用倒引號 (`` ` ``) 字元括住。
...
* [外部資料表](https://docs.cloud.google.com/bigquery/docs/external-tables?hl=zh-tw)不支援彈性資料欄名稱。
...
| [布林](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-types?hl=zh-tw#boolean_type) | `BOOL` | TRUE 或 FALSE (不分大小寫) |
...
* `"ROUND_HALF_AWAY_FROM_ZERO"`：這個模式 (預設) 會將中間值四捨五入至遠離零的值。
...
使用 [`CREATE TABLE` 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#create_table_statement)。使用 [column](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#column_name_and_column_schema) 選項指定結構定義。下列範例會建立名為 `newtable` 的新資料表，其中包含整數、字串和布林類型資料欄 x、y 和 z：
...
* `path_to_source`：本機或 Cloud Storage 上的 CSV 或 JSON 資料檔案位置。
* `schema`：內嵌結構定義。
...
您也可以使用 JSON 結構定義檔指定結構定義，而不使用內嵌結構定義。JSON 結構定義檔是由包含以下項目的 JSON 陣列所組成：
...
* 選用：資料欄的[政策標記](https://docs.cloud.google.com/data-catalog/docs/policy-tags?hl=zh-tw)，用於欄位層級的存取權控管
* 選用：`STRING` 或 `BYTES` 類型的資料欄值長度上限
* 選用：[精確度](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-types?hl=zh-tw#decimal_types)，適用於 `NUMERIC` 或 `BIGNUMERIC` 類型
...
* `dataset`：包含您要載入資料的資料表所屬的資料集。
* `table`：您要載入資料的資料表名稱。
* `path_to_data_file`：本機或 Cloud Storage 上的 CSV 或 JSON 資料檔案位置。
