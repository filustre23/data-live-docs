...
BigQuery sharing (舊稱 Analytics Hub) 是一個資料交換平台，可讓您透過完善的安全性和隱私權架構，跨機構大規模共用資料和洞察資訊。您可以使用 BigQuery sharing 探索並存取不同資料供應商收錄的資料庫。這個資料庫也包含 Google 提供的資料集。

舉例來說，您可以透過共用功能，使用第三方和 Google 資料集，擴增您的 Analytics 和 ML 計畫。

Analytics Hub Identity and Access Management (IAM) 角色可讓您執行下列共用工作：

* 身為 Analytics Hub 發布者，您可以與合作夥伴網路或自家機構即時分享資料。[項目](#listings)可讓您分享資料，不必複製共用資料，並在 [Google Cloud Marketplace](https://docs.cloud.google.com/bigquery/docs/analytics-hub-cloud-marketplace?hl=zh-tw) 或透過自有管道營利。您可以建立分析就緒資料來源的目錄，並設定精細的權限，將資料提供給合適的目標對象。您也可以管理訂閱項目，並查看項目的使用指標。
* Analytics Hub 訂閱者可以探索所需資料、將共用資料與現有資料合併，以及使用 [BigQuery 內建功能](https://docs.cloud.google.com/bigquery/docs/introduction?hl=zh-tw#explore-bigquery)。訂閱房源後，系統會在 Google Cloud 專案中建立[連結資料集](#linked_datasets)或連結的 Pub/Sub 訂閱項目。您可以使用[訂閱資源](https://docs.cloud.google.com/bigquery/docs/reference/analytics-hub/rest/v1/projects.locations.subscriptions?hl=zh-tw)管理訂閱項目，該資源會儲存訂閱者的相關資訊，並代表發布者與訂閱者之間的連結。
* Analytics Hub 檢視者可以瀏覽資料交換和項目，以及您有權在 BigQuery sharing 中查看的項目。如果還沒有商家資訊的訂閱權限，可以向發布者要求授予共用資料的存取權。您可以在 BigQuery sharing 和 Cloud Marketplace 上，探索整合 Cloud Marketplace 的商業項目。
...
共用資料集是 BigQuery 資料集，也是 BigQuery sharing 中的資料共用單位。BigQuery 架構將運算和儲存空間分開，讓資料發布者能與任意數量的訂閱者共用資料集，不必複製多份資料。發布者可以在專案中建立或使用現有的 BigQuery 資料集，並加入下列支援的物件，然後傳送給訂閱者：
...
* [資料表快照](https://docs.cloud.google.com/bigquery/docs/table-snapshots-intro?hl=zh-tw)
...
* 請勿在 VPC Service Controls 範圍內的專案中發布共用資料。如果專案中的共用資料集位於 VPC Service Controls 範圍內，您需要為交換庫專案 (代管的項目) 和所有訂閱者專案設定適當的[輸入和輸出規則](https://docs.cloud.google.com/vpc-service-controls/docs/ingress-egress-rules?hl=zh-tw)，才能順利訂閱發布者的項目。
...
共用主題是 [Pub/Sub 主題](https://docs.cloud.google.com/pubsub/docs/create-topic?hl=zh-tw)，也是 [BigQuery 中串流資料共用的單位](https://docs.cloud.google.com/bigquery/docs/analytics-hub-stream-sharing?hl=zh-tw)。發布者可以在專案中建立或使用現有的 Pub/Sub 主題，並將其發布給訂閱者。

#### 資料交換庫

資料交換是容器，可讓您透過自助服務共用資料。其中包含參照共用資源的項目。發布者和管理員可以在交易平台和商家資訊層級授予訂閱者存取權。這有助於避免明確授予基礎共用資源的存取權。您可以瀏覽資料交換，探索可存取的資料，並訂閱共用資源。[建立資料交換庫](https://docs.cloud.google.com/bigquery/docs/analytics-hub-manage-exchanges?hl=zh-tw#create-exchange)時，您可以指派主要聯絡人的聯絡電子郵件地址。使用者可以透過這個電子郵件地址，向資料交換擁有者提出問題或疑慮。

資料交換可以是下列其中一種類型：

* **私人資料交換。**根據預設，資料交換是私人的，只有能存取該交換的使用者或群組，才能查看或訂閱其刊登項目。
* **公開資料交換庫。**資料交換庫預設為私人性質，只有可存取該交換庫的使用者或群組，才能查看或訂閱其中的項目。不過，您可以選擇將資料交換庫設為公開。公開資料交換庫中的項目可供[Google Cloud 使用者 (`allAuthenticatedUsers`)](https://docs.cloud.google.com/iam/docs/principals-overview?hl=zh-tw#all-authenticated-users)[探索](https://docs.cloud.google.com/bigquery/docs/analytics-hub-view-subscribe-listings?hl=zh-tw#discover-listings)和[訂閱](https://docs.cloud.google.com/bigquery/docs/analytics-hub-view-subscribe-listings?hl=zh-tw#subscribe-listings)。如要進一步瞭解公開資料交換庫，請參閱「[將資料交換庫設為公開](https://docs.cloud.google.com/bigquery/docs/analytics-hub-manage-exchanges?hl=zh-tw#make-data-exchange-public)」。
...
使用者可透過主要聯絡人電子郵件地址，向資料交換庫擁有者詢問或回報相關問題。供應商名稱和聯絡人是最初提供資料給資料交換庫的機構。這項資訊為選填。發布者名稱和聯絡人是發布資料，供 BigQuery 共用使用的機構。這項資訊為選填。詳情請參閱「[管理資料交換庫](https://docs.cloud.google.com/bigquery/docs/analytics-hub-manage-listings?hl=zh-tw)」。

根據為商家資訊設定的 IAM 政策，以及包含商家資訊的資料交換類型，商家資訊可分為下列兩種：

* **公開目錄**：公開目錄會與所有[Google Cloud 使用者 (`allAuthenticatedUsers`)](https://docs.cloud.google.com/iam/docs/principals-overview?hl=zh-tw#all-authenticated-users) 分享。公開資料交易所中的目錄就是公開目錄。這些目錄可以是*免費公開資源*或*商業資源*的參照。如果目錄是商業資源，訂閱者可以直接向資料供應商要求存取目錄，也可以瀏覽及購買[整合 Google Cloud Marketplace 的商業目錄](https://docs.cloud.google.com/bigquery/docs/analytics-hub-cloud-marketplace?hl=zh-tw)。
* **私人項目**：私人項目會直接與個人或群組共用。舉例來說，私人項目可以參照您與貴機構其他內部團隊共用的行銷指標資料集。
...
訂閱 BigQuery 共用項目時，系統會建立連結的資源，將訂閱者連結至基礎共用資源。
...
訂閱者連結資料集後，即可存取共用資料集的資料表和檢視畫面，不必再進行任何身分與存取權管理授權。
...
訂閱共用主題的產品資訊時，系統會在訂閱端專案中建立連結的 Pub/Sub 訂閱項目。系統不會建立共用主題或訊息資料的副本。連結的 [Pub/Sub 訂閱項目](https://docs.cloud.google.com/pubsub/docs/subscription-overview?hl=zh-tw)訂閱者可以存取發布至共用主題的訊息。訂閱者存取共用主題的訊息資料時，不需要任何額外的 IAM 授權。發布者可以直接在 Pub/Sub 中管理訂閱項目，也可以透過 BigQuery sharing 訂閱項目管理功能進行管理。如要進一步瞭解連結的 Pub/Sub 訂閱項目，請參閱「[透過 Pub/Sub 串流分享](https://docs.cloud.google.com/bigquery/docs/analytics-hub-stream-sharing?hl=zh-tw)」。

## 資料輸出選項 (僅限 BigQuery 共用資料集)

發布商可透過資料輸出選項，限制訂閱者從連結的 BigQuery 資料集匯出資料。

發布商可以針對目錄、查詢結果或兩者啟用資料輸出限制。如果資料輸出受到限制，則適用下列限制：
...
* 共用主題最多可有 10,000 個 Pub/Sub 訂閱項目。這項限制包括連結的 Pub/Sub 訂閱項目，以及在 BigQuery sharing 以外建立的 Pub/Sub 訂閱項目 (例如直接從 Pub/Sub 建立)。
...
* 您無法在連結資料集內的個別資料表上設定 [IAM 角色](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw)或 [IAM 政策](https://docs.cloud.google.com/config-connector/docs/reference/resource-docs/iam/iampolicy?hl=zh-tw)，請改為在連結資料集層級套用。
* 您無法在連結資料集中的資料表上附加 [IAM 標記](https://docs.cloud.google.com/bigquery/docs/tags?hl=zh-tw)，請改為在連結資料集層級套用。
* 2023 年 7 月 25 日前建立的連結資料集，不會由[訂閱資源](https://docs.cloud.google.com/bigquery/docs/analytics-hub-manage-subscriptions?hl=zh-tw)回填資料。只有 2023 年 7 月 25 日後建立的訂閱項目，才能搭配 API 方法使用。
* 如果您是發布商，則適用下列 BigQuery 互通性限制：
  ...
  + [查詢計畫](https://docs.cloud.google.com/bigquery/docs/query-plan-explanation?hl=zh-tw)會顯示共用檢視區塊查詢和常式查詢，包括專案 ID，以及授權檢視區塊中涉及的其他資料集。請勿在共用檢視區塊或常式查詢中加入任何您認為機密的內容，例如加密金鑰。
  + 系統會在[Data Catalog](https://docs.cloud.google.com/data-catalog/docs/concepts/overview?hl=zh-tw) (已淘汰) 和[Knowledge Catalog](https://docs.cloud.google.com/dataplex/docs/catalog-overview?hl=zh-tw)中為共用資料集建立索引。共用資料集的更新 (例如新增資料表或檢視區塊) 會立即提供給訂閱者。不過，在某些情況下，例如共用資料集有超過 100 位訂閱者或資料表時，這些服務可能需要最多 18 小時才能為更新建立索引。由於索引建立作業會延遲，訂閱者無法立即在 Google Cloud 控制台中搜尋這些更新的資源。
...
* 如果您是訂閱者，則適用下列 BigQuery 互通性限制：
  ...
  + 如果查詢的連結資料集和 `JOIN` 陳述式大於 1 TB (實體儲存空間)，查詢可能會失敗。如要解決這個問題，請[與支援團隊聯絡](https://docs.cloud.google.com/bigquery/docs/getting-support?hl=zh-tw)。
  ...
  + 多個地區的商家資訊有以下限制：
  + 只有共用資料集和連結的資料集副本支援多個區域的項目。共用 Pub/Sub 主題和訂閱項目不支援多個區域的項目。
  + 資料無塵室不支援多個區域的房源資訊。
...
* 使用指標有以下限制：
  ...
  + [外部資料表](https://docs.cloud.google.com/bigquery/docs/external-tables?hl=zh-tw)
    ...
    可能含有不準確的資料。
  + 用量指標僅支援 [BigQuery 作業](https://docs.cloud.google.com/bigquery/docs/managing-jobs?hl=zh-tw)的用量。下列資源不支援耗用量：
...
假設您是零售商，而貴機構在名為「預測」的 Google Cloud 專案中，有即時需求預測資料。您想與供應鏈系統中的數百家供應商分享這項需求預測資料。以下各節說明如何透過 BigQuery 分享功能與供應商分享資料。

### 管理員

身為預測專案的擁有者，您必須先啟用 API，然後將[Analytics Hub 管理員角色](https://docs.cloud.google.com/bigquery/docs/analytics-hub-grant-roles?hl=zh-tw#ah-admin-role) (`roles/analyticshub.admin`) 指派給專案中管理資料交換的使用者。具有 Analytics Hub 管理員角色的使用者稱為 *BigQuery sharing 管理員*。

BigQuery sharing 管理員可以執行下列工作：

* 在機構的預測專案中建立、更新、刪除及共用資料交換。
...
* 授予由所有供應商組成的 Google 群組 [Analytics Hub 訂閱者角色](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw#analyticshub.subscriber) (`roles/analyticshub.subscriber`)，即可管理 *BigQuery 共享訂閱者*。如要讓供應商只能查看可用的交易平台和刊登項目，請授予他們 [Analytics Hub 檢視者角色](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw#analyticshub.viewer) (`roles/analyticshub.viewer`)。這些供應商無法訂閱刊登項目。
...
資料供應商可以[追蹤共用資料集的用量指標](https://docs.cloud.google.com/bigquery/docs/analytics-hub-monitor-listings?hl=zh-tw#use-analytics-hub)，包括下列詳細資料：

* 針對共用資料集執行的工作。
* 訂閱者專案和機構組織的共用資料集用量詳細資料。
* 工作處理的資料列和位元組數量。

詳情請參閱「[管理房源](https://docs.cloud.google.com/bigquery/docs/analytics-hub-manage-listings?hl=zh-tw)」。

### 訂閱人數

訂閱者可以瀏覽資料交換庫中可存取的項目，也可以建立連結的資料集，訂閱這些項目並將資料集新增至專案。供應商接著就能對這些連結的資料集執行查詢，並即時擷取結果。
...
如果是 BigQuery 資料集，發布者需支付資料儲存費用，而訂閱者則需根據以量計價或容量計價模式，支付對共用資料執行的查詢費用。如要瞭解定價資訊，請參閱 [BigQuery 定價](https://cloud.google.com/bigquery/pricing?hl=zh-tw)。

如果是 Pub/Sub，主題發布者須支付寫入共用主題的位元組總數 (發布總處理量)，以及網路輸出 (如適用)。訂閱者需支付的費用，取決於從連結的訂閱項目讀取的位元組總數 (訂閱輸送量)，以及網路輸出量 (如適用)。詳情請參閱「[Pub/Sub 定價](https://cloud.google.com/pubsub/pricing?hl=zh-tw#pubsub)」。
...
* [HIPAA](https://cloud.google.com/security/compliance/hipaa?hl=zh-tw)
* [HITRUST](https://cloud.google.com/security/compliance/hitrust?hl=zh-tw)

## VPC Service Controls

您可以設定輸入和輸出規則，允許發布者和訂閱者存取設有 VPC Service Controls 範圍的專案資料。詳情請參閱「[共用 VPC Service Controls 規則](https://docs.cloud.google.com/bigquery/docs/analytics-hub-vpc-sc-rules?hl=zh-tw)」。
