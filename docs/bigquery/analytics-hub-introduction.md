...
* Analytics Hub 發布者可以與合作夥伴網路或自家機構即時分享資料。[資訊](#listings)可讓您分享資料，不必複製共用資料，並在 [Google Cloud Marketplace](https://docs.cloud.google.com/bigquery/docs/analytics-hub-cloud-marketplace?hl=zh-tw) 或透過自有管道營利。您可以建立可供分析的資料來源目錄，並設定精細的權限，將資料交付給正確的目標對象。你也可以管理訂閱項目，並查看清單的使用指標。
...
* 如果您是訂閱者，則適用下列 BigQuery 互通性限制：
  ...
  + 多個地區的商家資訊有以下限制：
  + 只有共用資料集和連結的資料集副本支援多個區域的項目。共用 Pub/Sub 主題和訂閱項目不支援多個區域的刊登資訊。
  + 資料無塵室不支援多個區域的房源資訊。
...
本節提供範例，說明如何使用 BigQuery sharing 功能，與合作夥伴協作及透過資料營利。

### 合作夥伴協作

假設您是零售商，且貴機構在名為「預測」的 Google Cloud 專案中，有即時需求預測資料。您想與供應鏈系統中的數百家供應商分享這項需求預測資料。以下各節說明如何透過 BigQuery 共用功能，與供應商共用資料。
...
您是 `MarketDataSource` 專案的擁有者，因此必須先啟用 Analytics Hub API 和 Cloud Marketplace API。然後將[Analytics Hub 管理員角色](https://docs.cloud.google.com/bigquery/docs/analytics-hub-grant-roles?hl=zh-tw#ah-admin-role) (`roles/analyticshub.admin`) 指派給管理商業交換庫的團隊。
...
訂閱者可以在 BigQuery sharing 或 Cloud Marketplace 中直接瀏覽項目。訂閱者購買訂閱項目後，即可訂閱項目並在自己的Google Cloud 專案中建立連結的資料集。訂閱者接著可以對歷史股票資料執行分析查詢，並與自有交易演算法合併，不必擷取、複製或管理過時的檔案。
