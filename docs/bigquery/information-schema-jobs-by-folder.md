...
| `query_info.performance_insights` | `RECORD` | 工作的[效能洞察](https://cloud.google.com/bigquery/docs/reference/rest/v2/Job?hl=zh-tw#PerformanceInsights)。 |
...
如果專案遷移至機構 (無論是從沒有機構或從其他機構遷移)，您都無法透過 `INFORMATION_SCHEMA.JOBS_BY_FOLDER` 檢視遷移日期前的作業資訊，因為該檢視畫面只會保留遷移日期後的資料。
