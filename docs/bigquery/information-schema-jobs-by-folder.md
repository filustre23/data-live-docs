...
| `end_time` | `TIMESTAMP` | 這項工作的結束時間，以自記錄週期後的毫秒數為單位。這個欄位代表工作進入 `DONE` 狀態的時間。 |
...
如果專案遷移至機構 (無論是從沒有機構或從其他機構遷移)，則無法透過 `INFORMATION_SCHEMA.JOBS_BY_FOLDER` 檢視遷移日期前的作業資訊，因為該檢視畫面只會保留遷移日期後的資料。
...
**注意：** 查詢 `INFORMATION_SCHEMA.JOBS_BY_FOLDER` 以找出查詢工作的摘要費用時，應排除 `SCRIPT` 陳述式類型，否則部分值可能會重複計算。`SCRIPT` 列包含所有子項作業的摘要值，這些作業是做為這項作業的一部分執行。
