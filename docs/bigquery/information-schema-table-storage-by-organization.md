...
# TABLE\_STORAGE\_BY\_ORGANIZATION 檢視畫面

`INFORMATION_SCHEMA.TABLE_STORAGE_BY_ORGANIZATION` 檢視畫面會為與目前專案相關聯的整個機構，提供每個資料表或具體化檢視畫面的一列資料。

這個表格中的資料並非即時更新，可能會有幾秒到幾分鐘的延遲。如果儲存空間變更是由於分割區或資料表到期，或是修改資料集時間旅行視窗所致，`INFORMATION_SCHEMA.TABLE_STORAGE` 檢視畫面最多可能需要一天才會反映變更。如果資料集包含超過 1,000 個資料表，且您刪除了該資料集，則只有在刪除資料集的[時空旅行時間範圍](https://docs.cloud.google.com/bigquery/docs/time-travel?hl=zh-tw#time_travel)結束後，這個檢視畫面才會反映變更。
...
如要查詢 `INFORMATION_SCHEMA.TABLE_STORAGE_BY_ORGANIZATION` 檢視畫面，您必須具備下列組織的 Identity and Access Management (IAM) 權限：
...
只有已定義[Google Cloud機構](https://docs.cloud.google.com/resource-manager/docs/cloud-platform-resource-hierarchy?hl=zh-tw#organizations)的使用者才能使用這個結構定義檢視畫面。

如要進一步瞭解 BigQuery 權限，請參閱「[使用 IAM 控管存取權](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw)」。

## 結構定義

`INFORMATION_SCHEMA.TABLE_STORAGE_BY_ORGANIZATION` 檢視表具有下列結構定義：
...
| 檢視表名稱 | 資源範圍 | 區域範圍 |
| --- | --- | --- |
| `` [`PROJECT_ID`.]`region-REGION`.INFORMATION_SCHEMA.TABLE_STORAGE_BY_ORGANIZATION `` | 包含指定專案的機構 | `REGION` |
