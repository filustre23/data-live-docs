...
如要取得指定配置設定所需的權限，請要求系統管理員在專案或機構中授予您「[BigQuery 管理員](https://docs.cloud.google.com/iam/docs/roles-permissions/bigquery?hl=zh-tw#bigquery.admin) 」(`roles/bigquery.admin`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
...
* 專用編碼會透過查詢計畫傳播，因此系統可以在資料編碼時處理更多資料。
...
如要估算進階執行階段的影響，可以使用下列 SQL 查詢，找出預估執行時間改善幅度最大的專案查詢：
...
**注意：** 您只能比較 2026 年 1 月 30 日當天或之後建立的查詢，因為自該日起，`INFORMATION_SCHEMA.JOBS` 檢視畫面會持續顯示進階執行階段最佳化指標 (`enhanced_vectorization` 和 `short_query_optimization`)。先前的查詢會限制查詢時間範圍，確保這一點。
