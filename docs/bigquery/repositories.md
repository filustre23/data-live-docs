...
* 建立及管理私人存放區，包括在私人存放區中對工作區和檔案執行的所有動作：
...
**重要事項：** 如要建立連結至遠端 Git 存放區的 BigQuery 存放區，但該存放區未列入 `dataform.restrictGitRemotes` 政策的允許清單，請先將遠端 Git 存放區新增至政策的 `allowedValues` 清單，然後建立新的 BigQuery 存放區並連結至遠端存放區。詳情請參閱「[限制遠端存放區](https://docs.cloud.google.com/dataform/docs/restrict-git-remotes?hl=zh-tw)」一文。
