...
**重要事項：** 如要享有更簡化的資料夾式版本管控體驗，建議使用 [BigQuery Studio Git 存放區](https://docs.cloud.google.com/bigquery/docs/git-repositories?hl=zh-tw)
[(搶先版)](https://cloud.google.com/products/?hl=zh-tw#product-launch-stages)。
透過這項整合功能，您可以在 BigQuery Studio 檔案瀏覽器中直接使用 Git 分支，並一次複製多個存放區。BigQuery Studio Git 存放區功能也讓使用者不必管理個別工作區。
...
具備存放區程式碼編輯器角色的主體可以編輯存放區中的所有工作區。
...
6. 在「Region」(區域) 下拉式清單中，選取 BigQuery 區域，用來儲存存放區及其內容。選取離您所在位置最近的 BigQuery 區域。
...
您可以透過 HTTPS 或 SSH 連線至遠端存放區。如果遠端存放區未對公開網際網路開放 (例如位於防火牆後方)，將 BigQuery 存放區連線至遠端存放區可能會失敗。下表列出支援的 Git 供應商，以及這些供應商存放區可用的連線方法：
...
1. 在 Git 供應商中執行下列操作：
   ...
   1. 在 Bitbucket 中[建立私密安全殼層金鑰](https://support.atlassian.com/bitbucket-cloud/docs/configure-ssh-and-two-step-verification/)。
   ...
   2. 如果您沒有現有的 SSH 金鑰，或想使用新金鑰，請[建立私密 SSH 金鑰](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent)。
   3. [將 GitHub 公開安全殼層金鑰上傳](https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account)至 GitHub 存放區。
...
3. [授予 Dataform 服務代理密鑰存取權](https://docs.cloud.google.com/secret-manager/docs/manage-access-to-secrets?hl=zh-tw)。

   Dataform 服務代理程式 ID 的格式如下：
...
9. 在編輯器中，選取「設定」分頁標籤。
10. 按一下「Connect with Git」(透過 Git 連線)。
...
12. 在「遠端 Git 存放區網址」欄位中，輸入遠端 Git 存放區的網址，結尾為 `.git`。
    ...
    * 絕對網址：`ssh://git@{host_name}[:{port}]/{repository_path}`，`port` 為選填。
...
如要透過 HTTPS 連線至遠端存放區，請使用個人存取權杖建立 Secret Manager Secret，並與 Dataform 服務代理程式共用該 Secret。

BigQuery 接著會使用存取權權杖登入 Git 供應商，代表使用者提交變更。BigQuery 會使用使用者的 Google Cloud 電子郵件地址進行這些提交作業，因此您可以判斷每個提交作業是由誰完成。

**警告：** 所有對自訂服務帳戶具有[act-as 權限](https://docs.cloud.google.com/dataform/docs/strict-act-as-mode?hl=zh-tw)的 BigQuery 使用者，都會共用私有 HTTPS 權杖。建議您使用 Git 供應商建立機器使用者，並限制其存取您打算搭配 BigQuery 使用的遠端 Git 存放區。只有專案擁有者和具有[程式碼擁有者角色](https://docs.cloud.google.com/dataform/docs/access-control?hl=zh-tw#dataform.codeOwner) (`roles/dataform.codeOwner`) 的 BigQuery 使用者，才能使用 HTTPS 權杖連線至存放區。 Google Cloud BigQuery 使用者無法看到 HTTPS 權杖本身。
...
1. 在 Git 供應商中執行下列操作：
   ...
   1. 在 GitHub 中，建立[精細的個人存取權杖](https://github.blog/2022-10-18-introducing-fine-grained-personal-access-tokens-for-github/)或[傳統個人存取權杖](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/creating-a-personal-access-token#about-personal-access-tokens)。
...
* 瞭解如何[使用 BigQuery Studio Git 存放區管理程式碼](https://docs.cloud.google.com/bigquery/docs/git-repositories?hl=zh-tw)。
