...
整合後，您就能直接在 BigQuery Studio 檔案瀏覽器中使用 Git 分支，並一次複製多個存放區。BigQuery Studio Git 存放區功能也讓使用者不必管理個別工作區。
...
您可以透過 HTTPS 或 SSH 連線至遠端存放區。如果遠端存放區未對公開網際網路開放 (例如位於防火牆後方)，將 BigQuery 存放區連結至遠端存放區可能會失敗。下表列出支援的 Git 供應商，以及其存放區可用的連線方法：
...
2. 在 Secret Manager 中[建立密鑰](https://docs.cloud.google.com/secret-manager/docs/creating-and-accessing-secrets?hl=zh-tw#create)，然後將私密安全殼層金鑰貼為密鑰值。您的私密 SSH 金鑰應儲存在類似 `~/.ssh/id_ed25519` 的檔案中。為密鑰命名，方便日後搜尋。
