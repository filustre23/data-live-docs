...
您可以查詢 `INFORMATION_SCHEMA.ORGANIZATION_OPTIONS_CHANGES` 檢視區塊，擷取機構中 BigQuery 設定變更的即時中繼資料。這個檢視畫面會反映 2024 年 1 月 31 日後所做的機構層級和專案層級設定變更。

## 所需權限

如要取得擷取設定變更所需的權限，請要求管理員授予您機構的「[BigQuery 管理員](https://docs.cloud.google.com/iam/docs/roles-permissions/bigquery?hl=zh-tw#bigquery.admin) 」(`roles/bigquery.admin`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
...
查詢 `INFORMATION_SCHEMA.ORGANIZATION_OPTIONS_CHANGES` 檢視表時，查詢結果會針對機構中的每項設定變更，各列出一個相對應的資料列。
...
這個檢視畫面會顯示正在執行的工作階段，以及過去 180 天內完成的工作階段記錄。
