...
資料管理員可以在資料表上設定資料列層級的存取權政策，或建立獨立的資料表來存放機密資料，藉此限制使用者查看該資料的權限。將資料儲存在獨立的資料表中，可隔離資料並移除查看資料表列數的權限。
...
* [授權檢視表、資料列層級安全防護機制和個別資料表比較](https://docs.cloud.google.com/bigquery/docs/row-level-security-intro?hl=zh-tw#comparison_of_authorized_views_row-level_security_and_separate_tables)
...
然後視需要授予主體權限，存取包含檢視表群組的資料集，或資料集中的個別檢視表。可存取其他資料集的資料集稱為「已授權資料集」。授權其他資料集存取其資料的資料集稱為「共用資料集」。
...
* 刪除授權檢視區後，最多可能需要 24 小時，系統才會從檢視區清單中移除該授權檢視區。在這段期間，您無法存取該授權檢視區，但刪除的授權檢視區可能會顯示在檢視區清單中，並計入[授權檢視區上限](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#dataset_limits)。如果新授權檢視區會超出上限，系統可能會禁止建立該檢視區。
* 您無法在次要副本中建立授權檢視表。詳情請參閱[資料集複寫中的資源行為](https://docs.cloud.google.com/bigquery/docs/data-replication?hl=zh-tw#resource-behavior)。
...
舉例來說，在下圖中，專案 R 中的呼叫端會存取專案 A 中的授權檢視區塊，而該檢視區塊的來源資料表位於專案 B。在這種情況下，VPC Service Controls 輸入和輸出規則必須允許專案 R 中的呼叫端使用 BigQuery 服務存取專案 A 和專案 B。
...
* 如要建立檢視表，您必須具備資料集的 `bigquery.tables.create` IAM 權限。`roles/bigquery.dataEditor` 預先定義的 IAM 角色包含建立檢視表所需的權限。
...
* 查詢工作執行的專案 (帳單或執行專案) 的 `roles/bigquery.jobUser` (或 `roles/bigquery.user`) IAM 角色。這個角色會授予 `bigquery.jobs.create` 權限，這是對檢視區塊執行查詢工作時的必要權限。

  無論檢視畫面代管於何處，您只需要在要執行工作的專案中，具備 `roles/bigquery.jobUser` IAM 角色。
* 授權檢視表所屬資料集的 `roles/bigquery.dataViewer` IAM 角色。這個角色會授予 `bigquery.tables.getData` 權限，這是查詢檢視表時的必要權限。

  如果使用者從其他專案查詢授權 view，則不需要檢視畫面所在專案的 `roles/bigquery.jobUser` 角色。他們需要檢視畫面所在資料集的 `roles/bigquery.dataViewer` 角色，以及查詢執行所在專案的 `roles/bigquery.jobUser` 角色。
...
如要建立授權檢視表，請選擇下列任一選項。如需授權、共用及刪除授權檢視表的完整步驟，請參閱「[建立授權檢視表](https://docs.cloud.google.com/bigquery/docs/create-authorized-views?hl=zh-tw)」教學課程。
...
4. 在「Save view」(儲存檢視表) 對話方塊中，執行下列操作：
   ...
   2. 在「資料集」中，輸入要儲存檢視區塊的資料集。這必須是與來源查詢所用資料集不同的資料集。
...
呼叫 [`tables.delete`](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tables/delete?hl=zh-tw) 方法，並使用 `projectID`、`datasetID` 和 `tableID` 屬性，移除資料集的授權 view。詳情請參閱「[資料表](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tables?hl=zh-tw)」。
