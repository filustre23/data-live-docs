...
然後視需要授予主體權限，存取包含檢視畫面群組的資料集，或資料集中的個別檢視畫面。可存取其他資料集的資料集稱為「授權資料集」。授權其他資料集存取資料的資料集稱為「共用資料集」。
...
* 您無法在次要副本中建立授權檢視。詳情請參閱「[資料集複製作業中的資源行為](https://docs.cloud.google.com/bigquery/docs/data-replication?hl=zh-tw#resource-behavior)」。
...
舉例來說，在下圖中，專案 R 中的呼叫端會存取專案 A 中的授權 view，而該 view 的來源資料表位於專案 B 中。在這種情況下，VPC Service Controls 輸入和輸出規則必須允許專案 R 中的呼叫端，使用 BigQuery 服務存取專案 A 和專案 B。

#### VPC Service Controls 存取權的必要權限

主體不需要來源資料專案的 Identity and Access Management 權限，但除了允許存取包含檢視區塊的專案外，輸入規則也必須允許存取包含來源資料表的來源專案中的 BigQuery 服務。
...
* 如要瞭解如何設定 BigQuery 的輸入和輸出規則，請參閱「[VPC Service Controls 支援的產品：BigQuery](https://docs.cloud.google.com/vpc-service-controls/docs/supported-products?hl=zh-tw#table_bigquery)」。

舉例來說，如果您在 `PROJECT_A` (檢視區塊) 中建立檢視區塊，存取 `PROJECT_B` (來源) 中的資料，則需要為 `PROJECT_B` 設定一組連入規則，並為 `PROJECT_A` 設定另一組連入規則：
...
下列 YAML 檔案顯示 `PROJECT_B` (來源) 的輸出規則集。您還需要一組 `PROJECT_A` (檢視) 的輸出規則：
...
**注意：** 除非您是資料擁有者，否則無法變更授權 view 的 SQL 查詢。
...
**注意：** 如要在授權資料集中建立或更新授權 view，不需要 `bigquery.datasets.update` 權限。
...
* 包含授權 view 的資料集的 `roles/bigquery.dataViewer` IAM 角色。這個角色會授予 `bigquery.tables.getData` 權限，這是查詢檢視區塊的必要權限。
  ...
  如果使用者從其他專案查詢授權 view，則不需要檢視區塊所在專案的 `roles/bigquery.jobUser` 角色。他們需要具備內含檢視表的資料集 `roles/bigquery.dataViewer` 角色，以及查詢執行的專案 `roles/bigquery.jobUser` 角色。
