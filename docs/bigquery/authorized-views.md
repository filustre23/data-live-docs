...
* [授權檢視表、資料列層級安全防護機制和獨立資料表比較](https://docs.cloud.google.com/bigquery/docs/row-level-security-intro?hl=zh-tw#comparison_of_authorized_views_row-level_security_and_separate_tables)
* [資料列層級安全性簡介](https://docs.cloud.google.com/bigquery/docs/row-level-security-intro?hl=zh-tw)
* [列層級安全性的用途示例](https://docs.cloud.google.com/bigquery/docs/row-level-security-intro?hl=zh-tw#example_use_cases)
* [資料欄層級存取控管機制簡介](https://docs.cloud.google.com/bigquery/docs/column-level-security-intro?hl=zh-tw)

### 共用資料集中的所有檢視畫面

如要授予檢視表集合資料集的存取權，不必逐一授權每個檢視表，可以將檢視表歸入資料集，然後授予包含檢視表的資料集，存取包含資料的資料集。

然後視需要授予主體權限，存取包含檢視群組的資料集，或資料集中的個別檢視區塊。可存取其他資料集的資料集稱為「授權資料集」。授權其他資料集存取資料的資料集稱為「共用資料集」。
...
* 在其他資料集中建立授權檢視表或授權具體化檢視表時，來源資料集和授權檢視表資料集必須位於相同的區域[位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。
* 刪除授權 view 後，系統最多可能需要 24 小時，才會從檢視表清單中移除該授權 view。這段期間您無法存取已授權的檢視表，但已刪除的已授權檢視表可能會出現在檢視表清單中，並計入[已授權檢視表限制](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#dataset_limits)。如果新的授權 view 會超出這項限制，系統就會禁止建立額外的授權 view。
* 您無法在次要副本中建立授權檢視區塊。
...
在 VPC Service Controls 範圍內使用授權檢視表時，輸入和輸出規則必須允許主體存取內含檢視表的專案，以及內含檢視表存取資料的來源資料的任何其他專案。

舉例來說，在下圖中，專案 R 中的呼叫端會存取專案 A 中的授權檢視區塊，而專案 A 的來源資料表位於專案 B。在這種情況下，VPC Service Controls 輸入和輸出規則必須允許專案 R 中的呼叫者，使用 BigQuery 服務存取專案 A 和專案 B。

#### VPC Service Controls 存取權的必要權限

主體不需要來源資料專案的 Identity and Access Management 權限，但除了允許存取包含檢視區塊的專案之外，輸入規則也必須允許存取包含來源資料表的來源專案中的 BigQuery 服務。

如要瞭解必要角色和權限，請參閱「[專案和資料集的使用者權限](https://docs.cloud.google.com/bigquery/docs/authorized-views?hl=zh-tw#user_permissions_on_the_project_and_dataset_for_the_view)」。

#### 輸入和輸出規則

下列 VPC Service Controls 輸入和輸出規則範例，可讓 BigQuery API 存取來源和檢視專案。

* 如要瞭解如何設定輸入和輸出規則，請參閱「[設定輸入和輸出政策](https://docs.cloud.google.com/vpc-service-controls/docs/configuring-ingress-egress-policies?hl=zh-tw)」。
* 如要瞭解 BigQuery 的輸入和輸出規則設定限制，請參閱「[VPC Service Controls 支援的產品：BigQuery](https://docs.cloud.google.com/vpc-service-controls/docs/supported-products?hl=zh-tw#table_bigquery)」。

舉例來說，如果您在 `PROJECT_A` (檢視畫面) 中建立檢視畫面，存取 `PROJECT_B` (來源) 中的資料，則需要為 `PROJECT_B` 設定一組連入規則，並為 `PROJECT_A` 設定另一組連入規則：
...
下列 YAML 檔案顯示 `PROJECT_B` (來源) 的輸出規則集。您還需要一組 `PROJECT_A` (檢視畫面) 的輸出規則：
...
* 如要建立資料集，您需要專案的 `bigquery.datasets.create` IAM 權限。
* 如要建立檢視區塊，您必須具備資料集的 `bigquery.tables.create` IAM 權限。`roles/bigquery.dataEditor` 預先定義的 IAM 角色包含建立檢視區塊所需的權限。
* 如要建立可查詢您無權存取資料表的檢視區塊，您必須具備該檢視區塊所查詢資料表的 `bigquery.tables.getData` 權限。

如要進一步瞭解 BigQuery 中的 IAM 角色和權限，請參閱[預先定義的角色和權限](https://docs.cloud.google.com/bigquery/docs/access-control?hl=zh-tw)。

#### 含有來源資料的資料集權限

如要授權檢視表，請授予檢視表來源資料所屬資料集的讀取權限。

如要執行這項授權，您必須具備來源資料所屬資料集的下列權限：

**注意：** 如要在授權資料集中建立或更新授權檢視表，您不需要 `bigquery.datasets.update` 權限。

* `bigquery.datasets.update`
* `datasets.getIamPolicy`
* `datasets.setIamPolicy`

如要瞭解更新資料集所需的額外權限，請參閱「[必要權限](https://docs.cloud.google.com/bigquery/docs/updating-datasets?hl=zh-tw#required_permissions)」。

#### 專案和資料集的檢視權限

如要與使用者或群組共用授權檢視區塊，您必須授予使用者或群組下列 IAM 權限：

* 查詢工作執行的專案 (帳單或執行專案) 的 `roles/bigquery.jobUser` (或 `roles/bigquery.user`) IAM 角色。這個角色會授予 `bigquery.jobs.create` 權限，這是對檢視區執行查詢工作時的必要權限。

  無論檢視畫面代管於何處，您只需要在要執行工作的專案中，具備 `roles/bigquery.jobUser` IAM 角色。
* 包含授權 view 的資料集的 `roles/bigquery.dataViewer` IAM 角色。這個角色會授予 `bigquery.tables.getData` 權限，這是查詢檢視區塊的必要權限。

  如果使用者從其他專案查詢授權檢視區塊，則不需要在代管檢視區塊的專案中具備 `roles/bigquery.jobUser` 角色。他們需要具備內含檢視表的資料集 `roles/bigquery.dataViewer` 角色，以及查詢執行的專案 `roles/bigquery.jobUser` 角色。
...
如要建立授權 view，請選擇下列任一選項。如要瞭解授權、共用及刪除授權檢視表的完整步驟，請參閱「[建立授權檢視表](https://docs.cloud.google.com/bigquery/docs/create-authorized-views?hl=zh-tw)」教學課程。
...
4. 在「Save view」(儲存檢視表) 對話方塊中，執行下列操作：
   ...
   4. 按一下 [儲存]。
5. 授予可使用授權檢視畫面的使用者[必要權限](#user_permissions_on_the_project_and_dataset_for_the_view)。
...
授權檢視區塊後，您可以對資料集、資料表或檢視區塊完成下列工作，維持存取權：
...
**注意：** 如果移除 VPC Service Controls 範圍內檢視區塊的授權，查詢可能會開始失敗，並顯示 VPC Service Controls 錯誤。詳情請參閱「[已撤銷的授權和 VPC Service Controls](#revoked-authorization-vpcsc)」。移除檢視區後，請等待 24 小時再重複使用檢視區名稱，或使用不重複的名稱。如要進一步瞭解如何移除檢視區塊，請參閱「[配額與限制](#quotas_and_limits)」一文。
...
### 授權遭撤銷和 VPC Service Controls

當授權 view 存取 VPC Service Controls perimeter 內的來源資料集資料時，查詢 view 會略過對基礎資料表執行的標準 VPC Service Controls 檢查。

如果從來源資料集撤銷或移除檢視授權，BigQuery 會改用標準授權檢查，並直接查詢來源專案中的基本資料表。如果這些基礎資料表跨越 VPC Service Controls 範圍界線，且沒有允許直接存取資料表的明確輸出規則，VPC Service Controls 就會封鎖查詢，並傳回輸入或輸出違規錯誤。

還原來源資料集的檢視授權後，BigQuery 會再次略過直接基礎資料表檢查，VPC Service Controls 錯誤也會隨之解決。

## 配額與限制

* 授權檢視表會受到資料集限制。詳情請參閱「[資料集限制](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#dataset_limits)」。
* 移除授權檢視後，系統最多可能需要 24 小時，才會移除所有對該檢視的參照。為避免發生錯誤，請等待 24 小時再重新使用已移除的資料檢視名稱，或為資料檢視建立專屬名稱。
...
資料欄層級安全性對檢視區塊的影響，與檢視區塊是否為授權檢視區塊無關。
...
*連結的資料集*是唯讀 BigQuery 資料集，做為共用資料集的指標或參照。訂閱共用*項目*會在專案中建立連結的資料集，但不會複製資料集，因此訂閱者可以讀取資料，但無法在其中新增或更新物件。

[不支援](https://docs.cloud.google.com/bigquery/docs/analytics-hub-introduction?hl=zh-tw#limitations)參照連結資料集中資料表的具體化檢視表。
...
* 如要建立支援其他類型存取權控管的具體化檢視表，請參閱「[建立具體化檢視表](https://docs.cloud.google.com/bigquery/docs/materialized-views-create?hl=zh-tw#access_control)」。
