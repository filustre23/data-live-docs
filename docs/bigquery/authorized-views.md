...
在 VPC Service Controls 服務範圍內使用已授權檢視表時，允許主體存取內含檢視表的專案的輸入規則，也必須允許存取內含檢視表存取資料的來源資料的任何專案。主體不需要來源資料專案的 Identity and Access Management 權限，但除了包含檢視區塊的專案外，輸入規則也必須允許存取資料來源專案中的 BigQuery。
