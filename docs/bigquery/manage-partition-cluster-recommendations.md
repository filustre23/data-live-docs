...
如要取得存取分區和分群建議所需的權限，請要求管理員授予您「BigQuery 分區分群建議工具檢視者」 (`roles/recommender.bigqueryPartitionClusterViewer`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
