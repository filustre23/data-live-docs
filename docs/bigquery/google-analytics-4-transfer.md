...
您可以在[建立 Google Analytics 轉移作業](https://docs.cloud.google.com/bigquery/docs/google-analytics-4-transfer?hl=zh-tw#set-up-ga4-transfer)時指定自訂報表。
...
* PARAMETERS：已建立資料移轉設定的 JSON 格式參數，例如 `--params='{"param":"param_value"}'`。如果是 Google Analytics 4 轉移，則必須提供 `property_id`
  參數。
