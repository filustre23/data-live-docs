...
如要取得管理組織政策所需的權限，請要求管理員授予您組織資源的[組織政策管理員](https://docs.cloud.google.com/iam/docs/roles-permissions/orgpolicy?hl=zh-tw#orgpolicy.policyAdmin)  (`roles/orgpolicy.policyAdmin`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
