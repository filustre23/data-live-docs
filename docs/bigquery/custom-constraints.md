...
* 如果因資料集以外資源的自訂限制而遭拒存取，[`PolicyViolationInfo`](https://docs.cloud.google.com/dotnet/docs/reference/Google.Cloud.Audit/latest/Google.Cloud.Audit.PolicyViolationInfo?hl=zh-tw)不會發布到 [BigQuery 稽核記錄](https://docs.cloud.google.com/bigquery/docs/reference/auditlogs?hl=zh-tw)。錯誤訊息會提供拒絕作業的 `constraintId`。
