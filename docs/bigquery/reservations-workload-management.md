...
如果測試效能的區域有閒置運算單元，預留項目可能會以比預期少的運算單元達到效能。這是因為預留項目使用閒置運算單元來滿足工作負載的運算單元需求。如果其他預訂項目的工作負載變更導致閒置運算單元減少，工作負載效能可能會降低。因此，對於生產層級、有時間限制的工作負載，請確保預留項目在運算單元容量方面可自給自足。這樣一來，閒置時段的變化就不會直接影響工作成效。
...
在以下範例中，有三個預留項目和 1,200 個閒置時段。如果沒有預留項目群組，每個預留項目會平均分配到 400 個閒置運算單元。如果將預留項目 1 和 2 分組，閒置時段分配情形就會改變。閒置運算單元會先平均分配給預留項目群組和預留項目 3 (兩者各 600 個閒置運算單元)。接著，預留項目群組的 600 個閒置運算單元會平均分配給預留項目 1 和 2。
...
如要使用購買的運算單元，請建立指派項目，將專案指派給特定預留項目。請確認專案已[指派給預留項目](https://docs.cloud.google.com/bigquery/docs/reservations-assignments?hl=zh-tw)。
...
如果專案中沒有指派 `ML_EXTERNAL` 工作類型的保留項目，查詢工作就會採用以量計價模式。`QUERY` 工作類型指派作業只能用於非外部模型或矩陣因式分解模型的 BigQuery ML 模型。詳情請參閱「[預訂指派](https://docs.cloud.google.com/bigquery/docs/reservations-assignments?hl=zh-tw#assign-ml-workload)」說明文件。
