...
管理員也可以根據指派的預留項目，設定運算單元處理量。舉例來說，如果您有生產層級、對時間敏感的工作負載，請建立預留項目，並提供足夠的基準運算單元。基準運算單元一律可用，可確保預留項目一律有足夠的處理量。不過，如果您使用自動調整規模的預留項目，預留項目中的容量不一定會預留。使用[自動調度資源預留項目](https://docs.cloud.google.com/bigquery/docs/slots-autoscaling-intro?hl=zh-tw)時，系統會根據需求自動擴充及縮減容量。此外，[閒置運算單元](https://docs.cloud.google.com/bigquery/docs/slots?hl=zh-tw#idle_slots)可供多個預留項目共用。

## 指派的預留項目

如要在預留項目中使用已分配的時段，您必須將其*指派*給一或多個專案、資料夾或機構。專案中的工作執行時，會使用指派保留項目的運算單元。資源可以從[Google Cloud 資源階層](https://docs.cloud.google.com/resource-manager/docs/cloud-platform-resource-hierarchy?hl=zh-tw)中的父項繼承指派項目。如果專案未指派給保留項目，則會沿用上層資料夾或機構的指派作業 (如有)。
...
* `BACKGROUND`：如果您選擇[使用自己的預留空間](https://docs.cloud.google.com/bigquery/docs/search-index?hl=zh-tw#use_your_own_reservation)，透過 Datastream 的背景套用作業將來源資料庫複製到 BigQuery，請使用這個預留空間。如果沒有更具體的預留項目適用於 `BACKGROUND_CHANGE_DATA_CAPTURE`、`BACKGROUND_COLUMN_METADATA_INDEX` 和 `BACKGROUND_SEARCH_INDEX_REFRESH` 所述的工作類型，系統也會將這個預留項目做為備援。Standard 版不提供 `BACKGROUND` 預訂功能。
* `CONTINUOUS`：請將這項預留量用於[持續查詢](https://docs.cloud.google.com/bigquery/docs/continuous-queries-introduction?hl=zh-tw)工作。
...
您可以在一個區域使用以量計價模式，並在另一個區域使用隨選模式。根據預設，所有專案都會採用以量計價模式。在某個區域內，您可以將專案、資料夾或機構指派給預留項目，使其選擇使用以容量為準的計費模式。舉例來說，如果您在美國多區域購買了運算單元承諾使用，並將您的機構指派給預設的預留項目，您的機構將會在美國多區域採用容量計費模式，而在所有其他區域則仍將採用以量計價模式。
...
採用以量計價的專案會使用與已承諾容量不同的容量。這些專案不會影響您已承諾容量的可用性。如果您查詢串流緩衝區中的資料，隨選工作不會針對從串流緩衝區處理的位元組收費，但預留工作會使用運算單元處理這些位元組。
...
最佳做法是為預留資源建立專屬專案。這個專案稱為「管理」專案，因為它集中管理您的約期費用和管理作業。為這個專案取個好記易懂的名稱，例如「`bq-COMPANY_NAME-admin`」。然後建立一或多個獨立專案，用於存放 BigQuery 工作。
...
最佳做法是限制管理專案的數量。這有助於簡化帳單管理和時段分配作業。建議您盡可能為貴機構的所有預訂項目使用一個管理專案。複雜的機構可能需要額外的管理專案，才能符合管理或帳單需求。
...
如果測試效能的區域有閒置運算單元，預訂作業可能會以比預期少的運算單元達到效能。這是因為預留項目使用閒置運算單元來滿足工作負載的運算單元需求。如果其他預訂項目的工作負載變更導致閒置運算單元可用性降低，工作負載效能可能會降低。因此，對於生產層級、有時間限制的工作負載，請確保預留項目在運算單元容量方面可自給自足。這樣一來，閒置時段的變化就不會直接影響工作成效。
...
*運算單元使用承諾*是指在指定時間範圍內購買運算單元。您可以購買運算單元，以 50 個運算單元為遞增量，最多可購買[區域運算單元配額](https://docs.cloud.google.com/bigquery/quotas?hl=zh-tw#reservations)。容量承諾為選用項目，但可為穩定的工作負載節省成本。運算單元承諾使用合約可用於支付預留項目的基準運算單元費用。接著，系統會將未使用的運算單元容量，以閒置運算單元的形式分享給其他預留項目。運算單元承諾使用不適用於自動調度資源運算單元。為確保您能以折扣費率使用已承諾的運算單元，請確認您承諾的運算單元足以涵蓋基準運算單元。你可以建立的約定數量沒有上限。從您成功購買使用承諾的時刻起，系統就會開始收費。如要查看目前定價資訊，請參閱[容量承諾定價](https://cloud.google.com/bigquery/pricing?hl=zh-tw#capacity_compute_analysis_pricing)。
...
建議您為預留項目指派非零的基準，以獲得最可預測的效能和初始容量。雖然您可以設定預留項目，將基準運算單元設為零，並設定最大容量，打算使用自動調度資源功能，但這種做法的成效完全取決於自動調度資源功能是否已正確啟用，以及是否主動取得運算單元。如果這類零基準預留項目的自動調度資源功能無法有效運作，系統就會改為完全依賴可用的閒置運算單元容量，這無法保證效能，且可能導致查詢速度難以預測或降低。
...
在下列範例中，有三個預留項目和 1,200 個閒置時段。如果沒有預留項目群組，每個預留項目會平均分配到 400 個閒置運算單元。如果將預留項目 1 和 2 分組，閒置時段分配情形就會改變。閒置運算單元會先平均分配給預留項目群組和預留項目 3 (兩者各 600 個閒置運算單元)。接著，預留項目群組的 600 個閒置運算單元會平均分配給預留項目 1 和 2。
...
* 群組內預留項目的總大小不得超過 3 萬個運算單元。包括自動調度資源限制。
...
使用共用運算單元集區不會產生費用。如要確保效能穩定且可預測，建議購買 `PIPELINE` 預留項目。
...
[[["容易理解","easyToUnderstand","thumb-up"],["確實解決了我的問題","solvedMyProblem","thumb-up"],["其他","otherUp","thumb-up"]],[["難以理解","hardToUnderstand","thumb-down"],["資訊或程式碼範例有誤","incorrectInformationOrSampleCode","thumb-down"],["缺少我需要的資訊/範例","missingTheInformationSamplesINeed","thumb-down"],["翻譯問題","translationIssue","thumb-down"],["其他","otherDown","thumb-down"]],["上次更新時間：2026-05-18 (世界標準時間)。"],[],[]]