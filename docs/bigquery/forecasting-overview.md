...
預測是一種分析歷來資料，然後據以預測未來趨勢的技術。舉例來說，您可以分析多個門市的銷售資料，以便預測這些門市未來的銷售業績。在 BigQuery ML 中，您會對[時間序列](https://en.wikipedia.org/wiki/Time_series)資料執行預測。
...
* 使用 [`ARIMA_PLUS_XREG` 模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-multivariate-time-series?hl=zh-tw)搭配 [`ML.FORECAST` 函式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-forecast?hl=zh-tw)。如要預測多個變數的未來值，請使用這種方法。這種做法需要您建立及管理模型。
...
| 最佳用途 | * 您想完全掌控模型，包括自訂模型。 * 您需要模型輸出內容的可解釋性。 | * 您希望盡量簡化設定，不必先建立模型就能進行預測。 |

## 建議的知識

即使沒有太多機器學習知識，您也能使用 BigQuery ML 陳述式和函式的預設設定，建立及使用預測模型。不過，具備機器學習開發和預測模型的基本知識，有助於您同時改善資料和模型，進而獲得更出色的結果。建議您使用下列資源，熟悉機器學習技術和程序：
