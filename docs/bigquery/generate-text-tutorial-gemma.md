...
部署至 Agent Platform 的開放模型會依機器時數收費。也就是說，端點完全設定完成後就會開始計費，直到您解除部署為止。如要進一步瞭解 Agent Platform 計價方式，請參閱[這個頁面](https://cloud.google.com/vertex-ai/pricing?hl=zh-tw#prediction-prices)。
