...
透過 BigQuery 資料集複製功能，您可以在兩個不同的區域或多區域之間，設定資料集的自動複製作業。

## 總覽

在 BigQuery 中建立資料集時，您會選取資料儲存的區域或多區域。*區域*是地理區域內的一組資料中心，*多區域*則是包含兩個以上地理區域的大型地理區域。您的資料會儲存在其中一個區域，且不會在多區域內複製。如要進一步瞭解區域和多區域，請參閱 [BigQuery 位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw)。

BigQuery 一律會將資料副本儲存在資料集位置內的兩個不同Google Cloud 可用區。「區域」是地區內資源的部署範圍。 Google Cloud 在所有區域中，區域間的複製作業都會使用同步雙重寫入。選取多區域位置不會提供跨區域複製或區域備援功能，因此如果發生區域服務中斷，資料集可用性不會提高。資料會儲存在地理位置內的單一地區。

如要進一步確保異地備援，您可以複製任何資料集。BigQuery 會在您指定的另一個區域建立資料集的次要副本，然後在兩個區域之間非同步複製這個副本，總共會有四個可用區副本。
...
主要副本可寫入，次要副本則為唯讀。系統會以非同步方式將寫入主要備用資源的資料複製到次要備用資源。在每個區域內，資料會備援儲存在兩個可用區。網路流量絕不會離開 Google Cloud 網路。
...
* **儲存空間。**次要區域的儲存空間位元組會以次要區域的獨立副本計費。[長期儲存空間](https://cloud.google.com/bigquery/pricing?hl=zh-tw#storage)中的資料表和分區不會在次要副本中重設為動態儲存空間。
...
新增資料集副本前，您必須先在 BigQuery 中建立要複製的初始資料集 (如果尚未建立)。新增副本的位置會設為您新增副本時指定的位置。新增副本的位置必須與初始資料集的位置不同。這表示資料集中的資料會持續在資料集建立位置和副本位置之間複製。對於需要共置的副本 (例如檢視區塊、具體化檢視區塊或非 BigLake 外部資料表)，如果新增副本的位置與來源資料的位置不同或不相容，可能會導致工作錯誤。
...
* 透過 [BigQuery Storage Write API](https://docs.cloud.google.com/bigquery/docs/write-api?hl=zh-tw) 或 [`tabledata.insertAll`](https://docs.cloud.google.com/bigquery/docs/reference/rest/v2/tabledata/insertAll?hl=zh-tw) 方法寫入主要副本的串流資料會盡量複製到次要副本，但複製延遲時間可能較長。
* 從 [Datastream](https://cloud.google.com/datastream-for-bigquery?hl=zh-tw) 或 [BigQuery 變更資料擷取作業](https://docs.cloud.google.com/bigquery/docs/change-data-capture?hl=zh-tw)寫入主要副本的串流 upsert 作業，會盡量複製到次要副本，但可能會有較高的複製延遲。複製完成後，次要副本中的 upsert 作業會根據表格設定的 [`max_staleness`](https://docs.cloud.google.com/bigquery/docs/change-data-capture?hl=zh-tw#manage_table_staleness) 值，合併到次要副本的表格基準線。
* 系統不支援對啟用[細微 DML](https://docs.cloud.google.com/bigquery/docs/data-manipulation-language?hl=zh-tw#fine-grained_dml) 的資料表進行跨區域複製。雖然系統會禁止建立已包含這類資料表的複製資料集，但不會禁止在現有複製資料集內的資料表上啟用細微 DML。不過，這麼做可能會導致啟用細微 DML 的資料表在次要區域中產生不正確的查詢結果。
* 您可以使用 Google Cloud 控制台
  或 SQL [資料定義語言 (DDL) 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw)管理複製和切換作業。
* 每個區域或多區域的每個資料集只能有一個副本。您無法在同一個目的地區域中，為同一個資料集建立兩個次要副本。
* 副本中的資源會受到「[資源行為](#resource-behavior)」一節所述的限制。
* [政策標記](https://docs.cloud.google.com/bigquery/docs/managing-policy-tags-across-locations?hl=zh-tw)和相關聯的資料政策不會複製到次要副本。如果使用者沒有專案、資料夾或機構層級的 `roles/datacatalog.categoryFineGrainedReader` 角色，即使升級複本，凡是參照原始區域以外區域中含有政策標記的資料欄，查詢都會失敗。
* 次要副本建立完成後，才能在次要副本中使用[時空旅行](https://docs.cloud.google.com/bigquery/docs/time-travel?hl=zh-tw)功能。
* 如要在資料集上啟用跨區域複製功能，目的地區域大小上限 (以邏輯位元組為單位) 預設為：`us` 和 `eu`
  [多區域](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#multi-regions)為 10 PB，其他[區域](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#regions)為 500 TB。這些上限可設定。如需更多資訊，請與[Google Cloud 支援團隊](https://cloud.google.com/support-hub?hl=zh-tw)聯絡。
* 這項配額適用於邏輯資源。
...
* 如果未設定 `replica_kms_key` 值，則無法在次要區域查詢套用[客戶自行管理的加密金鑰](https://docs.cloud.google.com/bigquery/docs/customer-managed-encryption?hl=zh-tw) (CMEK) 的資料表。
* 不支援 BigLake 資料表。
* 您無法複製外部或同盟資料集。
* 不支援 [BigQuery Omni 位置](https://docs.cloud.google.com/bigquery/docs/omni-introduction?hl=zh-tw#locations)。
* 如要設定[災難復原](https://docs.cloud.google.com/bigquery/docs/managed-disaster-recovery?hl=zh-tw)的資料複製功能，請勿使用下列區域配對：
  + `us-central1` - `us` 多區域
  + `us-west1` - `us` 多區域
  + `eu-west1` - `eu` 多區域
  + `eu-west4` - `eu` 多區域
* 無法複製例行程序層級的存取權控管，但可以複製例行程序的資料集層級存取權控管。
* 搜尋索引的運作方式如下：
  + 只有搜尋索引中繼資料會複製到次要區域，索引資料本身不會複製。
  + 如果切換至副本，系統會從先前的主要區域刪除索引，並在升級的區域中重新產生索引。
  + 如果在 8 小時內來回切換，索引產生作業就會延遲 8 小時。
* 如果您升級次要副本，[排定查詢](https://docs.cloud.google.com/bigquery/docs/scheduling-queries?hl=zh-tw)不會自動重新導向至新的主要位置。這些資源仍會繫結至建立時指定的區域。
  + 如果原始主要副本在升級後變成唯讀次要副本，嘗試將資料寫入資料集的排程查詢就會失敗。
  + 如果原始主要區域無法使用，即使排程查詢只執行讀取作業，也會失敗。
  + 如要確保排定查詢繼續對升級的主要副本執行，您必須在新主要位置手動重新建立查詢。
* 您無法複製隱藏的資料集。
...
| --- | --- | --- |
| 已授權的資料集 | 已授權的資料集 | 您無法在次要副本中建立授權資料集，必須在主要副本中建立，然後從次要區域查詢。 |
| 獲得授權的處理常式 | 獲得授權的處理常式 | 您無法在次要副本中建立授權常式。授權常式必須在主要副本中建立，然後才能從次要區域查詢。 |
| 授權 view | 授權 view | 您無法在次要副本中建立授權 view。授權 view 必須在主要副本中建立，然後才能從次要區域查詢。 |
...
| 外部資料表 | 外部資料表 | 系統只會複製外部資料表定義。如果 Cloud Storage bucket 與副本不在相同位置，查詢就會失敗。 |
| 邏輯檢視畫面 | 邏輯檢視畫面 | 如果邏輯檢視區塊參照的資料集或資源與邏輯檢視區塊不在相同位置，查詢時就會失敗。 |
| 代管資料表 | 代管資料表 | 沒有差別。 |
| materialized view | materialized view | 如果參照的資料表與具體化檢視區不在同一地區，查詢就會失敗。複製的具體化檢視區可能會出現高於檢視區[過時程度上限](https://docs.cloud.google.com/bigquery/docs/materialized-views-create?hl=zh-tw#max_staleness)的情況。 |
| 型號 | 型號 | 儲存為代管資料表。 |
| 遠端功能 | 遠端功能 | 連線是區域資源。如果參照的資料集或資源 (連線) 與遠端函式不在相同位置，遠端函式在執行時就會失敗。 |
| 處理常式 | 使用者定義函式 (UDF) 或預存程序 | 如果常式參照的資料集或資源與常式不在相同位置，執行時就會失敗。凡是參照連線的常式 (例如遠端函式)，都無法在來源區域以外運作。 |
| 資料列存取政策 | 資料列存取政策 | 沒有差別。 |
| 搜尋索引 | 搜尋索引 | 系統只會複製索引中繼資料，索引資料只會存在於主要區域。 |
...
跨區域複製功能不適合在整個區域服務中斷時，做為災難復原計畫使用。如果主要副本所在區域全面服務中斷，您就無法升級次要副本。由於次要備用資源為唯讀，您無法在次要備用資源上執行任何寫入工作，也無法升級次要區域，直到主要備用資源的區域還原為止。如要進一步瞭解如何準備災難復原，請參閱「[受管理災難復原](https://docs.cloud.google.com/bigquery/docs/managed-disaster-recovery?hl=zh-tw)」。
...
| 主要副本 | 次要副本 | 區域 1 | 在區域 1 中執行的所有工作都會失敗。唯讀工作會繼續在次要備用資源所在的區域 2 中執行。在成功與區域 1 同步前，區域 2 的內容會過時。 |
...
5. 在「建立資料集副本」窗格中，執行下列操作：
   ...
   3. 選用步驟：如要使用客戶自行管理的加密金鑰 (CMEK)，請展開「Advanced options」(進階選項) 部分，然後選取「Customer-managed encryption key (CMEK)」(客戶管理的加密金鑰 (CMEK)) 選項。
6. 按一下「建立備用資源」。
...
您可以為位於區域或多區域的任何資料集新增副本，前提是該區域或多區域尚未複製該資料集。新增副本後，初始複製作業需要一段時間才能完成。資料複製期間，您仍可執行參照主要副本的查詢，查詢處理容量不會減少。您無法在多區域的地理位置內複製資料。

以下範例會在 `us-central1` 區域中建立名為 `my_dataset` 的資料集，然後在 `us-east4` 區域中新增副本：
...
如要確認次要副本是否已成功建立，可以查詢 [`INFORMATION_SCHEMA.SCHEMATA_REPLICAS`](https://docs.cloud.google.com/bigquery/docs/information-schema-schemata-replicas?hl=zh-tw) 檢視中的 `creation_complete` 欄。

建立次要副本後，您可以[明確設定查詢位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#specify_locations)為次要區域，藉此查詢次要副本。如果未明確設定位置，BigQuery 會使用資料集主要副本的區域。

### 升級次要副本

如果主要區域處於連線狀態，您可以升級次要副本。升級作業會將次要副本切換為可寫入的主要副本。如果次要副本已趕上主要副本，這項作業會在幾秒內完成。如果次要副本尚未趕上，升級作業必須等到次要副本趕上後才會完成。如果主要副本所在的區域發生服務中斷，次要副本就無法升級為主要副本。

注意事項：

* 升級程序進行期間，所有寫入資料表的作業都會傳回錯誤。升級程序開始後，舊的主要副本會立即變成無法寫入。
...
* 您必須在查詢設定中，將工作地點明確設為次要區域。詳情請參閱「[BigQuery 指定位置](https://docs.cloud.google.com/bigquery/docs/locations?hl=zh-tw#specify_locations)」一文。
...
3. 點按「Details」(詳細資料) 分頁標籤。
4. 在「副本」部分中，找到要移除的副本，然後依序點選「更多動作」more\_vert「刪除」。
...
如要刪除整個資料集，請先捨棄所有次要副本。如果您刪除整個資料集 (例如使用 [`DROP
...
* **網路輸出位元組**：從主要區域複製到次要區域的資料量 (以位元組為單位)，會計入費用。這項指標可協助您監控頻寬配額用量。
...
您可以透過跨區域資料集複製功能，將資料集從一個區域遷移至另一個區域。以下範例說明如何使用跨區域複寫功能，將現有的 `my_migration` 資料集從 `US` 多區域遷移至 `EU` 多區域。
...
您的資料集位於 `EU` 多地區，且沒有 `my_migration` 資料集的副本。您已成功將資料集遷移至 `EU` 多地區。如要查看遷移的完整資源清單，請參閱「[資源行為](#resource-behavior)」。

## 由客戶管理的加密金鑰 (CMEK)

建立次要副本時，[客戶代管的 Cloud Key Management Service 金鑰](https://docs.cloud.google.com/bigquery/docs/customer-managed-encryption?hl=zh-tw)不會自動複製。如要維護複製資料集的加密狀態，您必須為新增的副本設定 `replica_kms_key` 位置。您可以使用 [`ALTER SCHEMA ADD REPLICA` DDL 陳述式](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language?hl=zh-tw#alter_schema_add_replica_statement)設定 `replica_kms_key`。

使用 CMEK 複製資料集時，系統會按照下列情況運作：
...
* 如果您在 `default_kms_key` 或 `replica_kms_key` 上使用 [Cloud KMS 金鑰輪替](https://docs.cloud.google.com/bigquery/docs/customer-managed-encryption?hl=zh-tw#key_rotation)，金鑰輪替後，您仍可查詢複製的資料集。
  ...
  + 如果刪除金鑰輪替前在主要副本資料表上設定的金鑰版本，只要資料表仍使用金鑰輪替前設定的金鑰版本，就無法查詢，直到金鑰版本更新為止。如要更新金鑰版本，舊金鑰版本必須處於啟用狀態 (未停用或刪除)。
...
以下範例會在 `us-west1` 區域中建立副本，並設定 `replica_kms_key` 值。如要使用 CMEK 金鑰，請授予 [BigQuery 服務帳戶權限](https://docs.cloud.google.com/bigquery/docs/customer-managed-encryption?hl=zh-tw#grant_permission)，允許加密及解密。
...
* 建立資料集副本後，您就無法更新來源資料集的「`default_kms_key`」值。
...
指派給資料欄的資料政策是區域資源，因此資料政策和附加的資料表必須位於相同區域。建立次要副本時，系統會自動複製資料政策。如果資料政策附加至複製資料集中的任何資料表，BigQuery 會在次要區域建立或更新資料政策和對應的 IAM 政策。

### 可變動性

複製的資料政策在次要區域中為唯讀，您無法在次要區域[更新資料政策](https://docs.cloud.google.com/bigquery/docs/column-data-masking?hl=zh-tw#update-data-policies-on-column)。附加至主要區域中資料表的原始資料政策仍可變更。只有在資料政策未附加至任何次要資料表時，才能變更該政策。如果附加至次要區域中的資料表，政策就會變成不可變更。如果資料政策不可變更，BigQuery 會拒絕任何更新或設定 IAM 政策的操作。
...
在主要區域中，將資料政策附加至資料欄或從資料欄卸離/刪除，都屬於資料表結構定義變更。所有次要區域的資料表結構定義都會更新，以反映變更。不過，如果從主要區域的所有資料表欄中分離資料政策，資料政策就會在次要區域中成為孤立政策，您必須手動清理。不建議在次要區域手動附加複製的資料政策，因為這可能會導致複雜的情況，例如無法新增或移除 FGAC 授權。

如果刪除已卸離的資料政策，背景工作會從資料表結構定義中移除該政策。發生這種情況時，資料表結構定義變更會傳播至目的地區域。與卸離政策一樣，您必須手動刪除次要區域中的資料政策。

### 捨棄副本

如果捨棄副本，系統不會自動刪除附加的資料政策。與卸離政策相同，您必須在次要區域手動刪除資料政策。
