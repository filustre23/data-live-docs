...
* **Cloud Storage bucket**。如需找出 Cloud Storage bucket 的步驟，請參閱「[事前準備](https://docs.cloud.google.com/bigquery/docs/play-transfer?hl=zh-tw#before_you_begin)」一節。Cloud Storage 值區的開頭是 `pubsite_prod_rev`。例如：`pubsite_prod_rev_01234567890987654321`。
...
* 如要瞭解如何使用移轉作業，包括取得移轉設定、列出移轉設定以及查看移轉設定的執行記錄，請參閱[使用移轉功](https://docs.cloud.google.com/bigquery/docs/working-with-transfers?hl=zh-tw)