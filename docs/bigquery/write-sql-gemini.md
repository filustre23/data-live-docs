...
未經您明確許可，Gemini for Google Cloud 不會使用提示詞或回覆內容來訓練模型。如要進一步瞭解 Google 如何使用您的資料，請參閱「[Gemini for Google Cloud 如何使用您的資料](https://docs.cloud.google.com/gemini/docs/discover/data-governance?hl=zh-tw)」一文。

**注意：** 如要選擇共用資料，以利改良 [搶先版](https://cloud.google.com/products/?hl=zh-tw#product-launch-stages)中的 Gemini in BigQuery 功能，請參閱「[協助改良建議](https://docs.cloud.google.com/bigquery/docs/write-sql-gemini?hl=zh-tw#help_improve_suggestions_2)」一文。

Gemini in BigQuery 僅支援英文提示。

本文適用於使用 SQL 查詢和 [BigQuery 中的 Colab Enterprise 筆記本](https://docs.cloud.google.com/bigquery/docs/notebooks-introduction?hl=zh-tw)的資料分析師、資料科學家和資料開發人員。本程式碼研究室假設您瞭解如何在 BigQuery Studio 環境中查詢資料，或如何使用 Python 筆記本分析 BigQuery 資料。
...
7. 在功能清單中，確認已選取下列功能：
   ...
   * **SQL 查詢專用 Gemini** 清單：
     ...
     + **自動產生**：您可以在 BigQuery 查詢編輯器中，使用自然語言註解提示 Gemini in BigQuery 生成 SQL 查詢。
...
如要取得使用 Gemini 輔助撰寫查詢所需的權限，請要求管理員授予專案的「Gemini for Google Cloud 使用者」 (`roles/cloudaicompanion.user`) IAM 角色。如要進一步瞭解如何授予角色，請參閱「[管理專案、資料夾和組織的存取權](https://docs.cloud.google.com/iam/docs/granting-changing-revoking-access?hl=zh-tw)」。
...
使用 SQL 生成工具，就能以自然語言生成與最近查看或查詢的資料表相關的 SQL 查詢。您也可以使用這項工具修改現有查詢，以及手動指定要產生 SQL 的資料表。
...
3. 在「透過 Gemini 生成 SQL 程式碼」對話方塊中，您可以選擇：
   ...
   * 輸入自然語言提示詞，內容是您最近查看或查詢的資料表。舉例來說，如果您最近查看了 [`bigquery-public-data.austin_bikeshare.bikeshare_trips` 資料表](https://console.cloud.google.com/bigquery?ws=%211m5%211m4%214m3%211sbigquery-public-data%212saustin_bikeshare%213sbikeshare_trips&hl=zh-tw)，可以輸入以下內容：
...
4. 點按「生成」。
   ...
   **注意：** 每次輸入相同提示時，Gemini 版 BigQuery 可能會建議不同的語法。
5. 查看生成的 SQL 查詢，然後進行下列任一操作：
   ...
   * 如要修正建議的 SQL 查詢，請在「修正」欄位中輸入修正內容，然後點選 send「修正」。舉例來說，輸入 `limit to 1000` 即可限制查詢結果數量。如要比較查詢的變更，請勾選「顯示差異」核取方塊。
...
1. 反白選取有錯的文字。
2. 依序按一下 auto\_awesome「Refine」(修正) 和 astrophotography\_mode「Fix it」(修正問題)。
...
1. 前往 Google Cloud 控制台的「BigQuery Studio」頁面。
   ...
   [前往 BigQuery Studio](https://console.cloud.google.com/bigquery?hl=zh-tw)
2. 在查詢編輯器的分頁列中，點選「SQL 查詢」add\_box旁的下拉式箭頭 arrow\_drop\_down，然後點選「Notebook」。

   系統會開啟新筆記本，其中包含的儲存格會顯示針對 `bigquery-public-data.ml_datasets.penguins` 公開資料集的範例查詢。
3. 如要插入新的程式碼儲存格，請點按工具列中的「程式碼」add。新的程式碼儲存格會顯示「開始使用 AI 編寫或生成程式碼」訊息。
4. 在新的程式碼儲存格中，按一下「生成」。
...
* bigframes.pandas：提供與 pandas 相容的 API，用於數據分析。
...
2. 在查詢編輯器的分頁列中，點選「SQL 查詢」add\_box旁的下拉式箭頭 arrow\_drop\_down，然後點選「Notebook」。
   ...
   系統會開啟新筆記本。
3. 如要插入新的程式碼儲存格，請點按工具列中的「程式碼」add。
...
9. 在新儲存格中呼叫 `peek()` 方法 (例如 `result.peek()`)，然後按一下「執行儲存格」play\_circle。畫面上會顯示多列資料。

## 補全 Python 程式碼

Python 程式碼補全功能會嘗試根據查詢編輯器中的內容，提供符合情境的適當建議。在您輸入內容時，Gemini in BigQuery 會根據目前程式碼的情境，建議合理的後續步驟，或協助您反覆調整程式碼。
...
3. 在編輯器中開始輸入 Python 程式碼。例如 `%%bigquery`。Gemini in BigQuery 會在您輸入時，建議內嵌程式碼。
4. 如要接受建議，請按下 `Tab` 鍵。
...
3. 在「Explorer」窗格中展開專案，然後按一下「Notebooks」。
4. 按一下要開啟的筆記本。
5. 在筆記本的程式碼儲存格中，輸入含有錯誤的程式碼，然後執行儲存格。舉例來說，您可能會輸入 `print(1`，但缺少右括號。
...
7. 如要修正所有其他類型的錯誤，請按照下列步驟操作：
   ...
   3. 如要接受建議的變更，請按一下「library\_add」library\_add「新增程式碼儲存格」。
...
您可以要求 Gemini Code Assist 在筆記本中生成 PySpark 程式碼。Gemini Code Assist 會擷取並使用相關的 BigQuery 和 Dataproc Metastore 資料表及其結構定義，生成程式碼回覆。Gemini Code Assist 具備結構定義知識，因此不會產生錯覺，還會建議彙整鍵和資料欄類型。
