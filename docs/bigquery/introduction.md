...
BigQuery 提供一致的方式來處理結構化和非結構化資料，並支援 Apache Iceberg、Delta 和 Apache Hudi 等開放資料表格式。BigQuery 串流支援持續擷取和分析資料，而 BigQuery 可擴充的分散式分析引擎，則可在幾秒內查詢 TB 級資料，幾分鐘內查詢 PB 級資料。

BigQuery 提供內建管理功能，可讓您探索及管理資料，並管理中繼資料和資料品質。透過語意搜尋和資料歷程等功能，您可以尋找並驗證用於分析的相關資料。您可以跨機構分享資料和 AI 資產，並享有存取權控管的優點。這些功能由 Knowledge Catalog 提供技術支援，這是 Google Cloud中資料和 AI 資產的整合式智慧治理解決方案。

BigQuery 的架構包含兩個部分：擷取、儲存及最佳化資料的儲存層，以及提供分析功能的運算層。Google 的 Petabit 級網路可讓運算和儲存層相互通訊，因此這兩層能有效率地獨立運作。

舊版資料庫通常必須在讀取/寫入作業和分析作業之間共用資源。這可能會導致資源衝突，並在資料寫入或讀取儲存空間時，減緩查詢速度。如果資料庫管理工作 (例如指派或撤銷權限) 需要資源，共用資源集區的負擔可能會進一步加重。BigQuery 將運算和儲存層分開，讓各層動態分配資源，不會影響其他層的效能或可用性。

這項分離原則可讓 BigQuery 加速創新，因為儲存空間和運算資源的改善項目可以獨立部署，不會造成系統停機或對效能產生負面影響。此外，這也是提供全代管無伺服器資料倉儲的必要條件，BigQuery 工程團隊會負責更新和維護作業。因此您不必佈建或手動擴充資源，可以專心提供價值，不必處理傳統資料庫管理工作。
...
* [公開資料集](https://docs.cloud.google.com/bigquery/public-data?hl=zh-tw)：透過探索公開資料集計畫提供的大型真實資料，體驗 BigQuery 的效能。
...
BigQuery 會使用欄式儲存格式儲存資料，這種格式經過最佳化，適合用於分析查詢。BigQuery 會以表格、列和欄的形式呈現資料，並完整支援資料庫交易語意 ([ACID](https://en.wikipedia.org/wiki/ACID))。BigQuery 儲存空間會自動複製到多個位置，以提供高可用性。
...
BigQuery 可集中管理資料和運算資源，而[身分與存取權管理 (IAM)](https://docs.cloud.google.com/iam/docs?hl=zh-tw) 則可透過整個 Google Cloud使用的存取權模式，協助您保護這些資源。
[Google Cloud 安全性最佳做法](https://cloud.google.com/security/best-practices?hl=zh-tw)
提供穩固但彈性的方法，可納入周邊安全防護，或更複雜且精細的[縱深防禦方法](https://cloud.google.com/security/overview/whitepaper?hl=zh-tw#technology_with_security_at_its_core)。

* [資料安全與管理簡介](https://docs.cloud.google.com/bigquery/docs/data-governance?hl=zh-tw)可協助您瞭解資料管理，以及保護 BigQuery 資源可能需要的控管機制。
* 「工作」是指 BigQuery 代表您執行的動作，包括載入、匯出、查詢或複製資料。
* [預訂](https://docs.cloud.google.com/bigquery/docs/reservations-intro?hl=zh-tw)可讓您在以量計價和以運算量為基礎的計價模式之間切換。
...
* [Google BigQuery: The Definitive Guide: Data Warehousing, Analytics, and Machine Learning at Scale](https://www.google.com/books/edition/Google_BigQuery_The_Definitive_Guide/-Jq4DwAAQBAJ?hl=zh-tw) (Google BigQuery 決定版指南：大規模資料倉儲、分析和機器學習) 一書由 Valliappa Lakshmanan 和 Jordan Tigani 撰寫，說明 BigQuery 的運作方式，並提供服務使用方式的完整逐步操作指南。
...
* **運用資料洞察探索及瞭解你的資料**。資料洞察功能會使用從資料表的中繼資料產生的深入分析查詢，以自動化且直覺的方式發掘模式並執行統計分析。這項功能特別有助於解決早期資料探索的冷啟動問題。詳情請參閱[在 BigQuery 中產生資料洞察](https://docs.cloud.google.com/bigquery/docs/data-insights?hl=zh-tw)。
* **透過對話式數據分析功能與資料對話**。對話式數據分析可讓您以自然語言與資料對話。
  您可以建立資料代理，並設定自訂中繼資料、指令、詞彙表字詞和通過驗證的查詢，建立商業邏輯。您也可以直接與資料來源對話，包括資料表、檢視區塊、屬性圖和 Lakehouse 資料表。詳情請參閱「[對話式數據分析總覽](https://docs.cloud.google.com/bigquery/docs/conversational-analytics?hl=zh-tw)」。
...
* **取得 SQL 和 Python 資料分析輔助**。您可以使用 Gemini in BigQuery，以 SQL 或 Python 生成或建議程式碼，並說明現有的 SQL 查詢。您也可以使用自然語言查詢開始進行資料分析。如要瞭解如何生成、完成及摘要程式碼，請參閱下列說明文件：  
  + SQL 程式碼輔助
  ...
  + Python 程式碼輔助
    ...
    - [產生 BigQuery DataFrame Python 程式碼](https://docs.cloud.google.com/bigquery/docs/write-sql-gemini?hl=zh-tw#dataframe)
      ([預先發布版](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))
* **準備要分析的資料**。BigQuery 的資料準備功能會根據情境提供 AI 生成的轉換建議，協助您清理資料以供分析。詳情請參閱「[使用 Gemini 準備資料](https://docs.cloud.google.com/bigquery/docs/data-prep-get-suggestions?hl=zh-tw)」。
* **使用翻譯規則自訂 SQL 翻譯**。([預先發布版](https://cloud.google.com/products?hl=zh-tw#product-launch-stages))
  建立 Gemini 強化翻譯規則，在使用[互動式 SQL 翻譯器](https://docs.cloud.google.com/bigquery/docs/interactive-sql-translator?hl=zh-tw)時自訂 SQL 翻譯。
...
* 使用工具分析及以視覺化的方式呈現 BigQuery 資料，
  包括：[Looker](https://docs.cloud.google.com/bigquery/docs/looker?hl=zh-tw)、
  [數據分析](https://docs.cloud.google.com/bigquery/docs/visualize-looker-studio?hl=zh-tw)和
  [Google 試算表](https://docs.cloud.google.com/bigquery/docs/connected-sheets?hl=zh-tw)。
...
* [盡可能提高查詢效能](https://docs.cloud.google.com/bigquery/docs/best-practices-performance-overview?hl=zh-tw)，方法如下：
  ...
  + [分區資料表](https://docs.cloud.google.com/bigquery/docs/partitioned-tables?hl=zh-tw)：根據時間或整數範圍修剪大型資料表。
...
* [管理費用](https://docs.cloud.google.com/bigquery/docs/controlling-costs?hl=zh-tw)：使用[預留項目](https://docs.cloud.google.com/bigquery/docs/reservations-intro?hl=zh-tw)，平衡隨選和以容量為準的計價方式。
...
* 如要瞭解 BigQuery 安全性總覽，請參閱「[資料安全與管理總覽](https://docs.cloud.google.com/bigquery/docs/data-governance?hl=zh-tw)」。
