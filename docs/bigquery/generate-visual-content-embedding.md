...
如果您打算在建立遠端模型時，使用模型名稱 (例如 `endpoint = 'gemini-2.0-flash'`) 指定端點，請在您打算建立遠端模型的專案中授予這些角色。
...
2. 使用 SQL 編輯器建立[遠端模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model?hl=zh-tw)：
   ...
   * `ENDPOINT`：要使用的[嵌入模型](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model?hl=zh-tw#endpoint)，例如 `gemini-embedding-2-preview` 或 `multimodalembedding@001`。
     ...
     模型必須位於您要建立遠端模型的位置。`gemini-embedding-2-preview` 模型僅支援 `us-central1` 和 `US` 區域。詳情請參閱[位置](https://docs.cloud.google.com/bigquery/docs/reference/standard-sql/bigqueryml-syntax-create-remote-model?hl=zh-tw#locations)。
