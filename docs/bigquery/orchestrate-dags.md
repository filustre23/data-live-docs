...
本文說明如何透過在 Google Cloud 控制台的 BigQuery「排程」頁面，從 Managed Airflow 3[排定有向無環圖 (DAG) 的排程](https://docs.cloud.google.com/composer/docs/composer-3/composer-overview?hl=zh-tw#about-airflow)，有效率地協調資料管道。這項功能可協助您自動執行及監控資料流程，為 BigQuery 數據分析提供支援。您可以手動觸發 DAG，並集中管理 DAG，包括查看執行記錄和記錄檔。
...
如要進一步瞭解 Airflow 的核心概念，例如 Airflow DAG、DAG 執行、工作或運算子，請參閱 Airflow 說明文件的「[核心概念](https://airflow.apache.org/docs/apache-airflow/stable/core-concepts/index.html)」頁面。
...
2. 執行下列任一操作：
   ...
   * 在包含所選 DAG 的資料列中，按一下「Actions」(動作) 欄中的 more\_vert「View actions」(查看動作)，然後按一下「Trigger DAG」(觸發 DAG)。
...
5. 如要查看 DAG 的視覺化圖表和工作依附元件，請選取「圖表」分頁標籤。
...
2. 執行下列任一操作：
   ...
   * 在包含所選 DAG 的資料列中，點選「動作」欄中的 more\_vert「查看動作」，然後點選「暫停 DAG」。
