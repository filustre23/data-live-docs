...
* **20% 折扣**：承諾使用 3 年即可享有此折扣。在合約期限內，您會以每小時的承諾支出金額，支付 BigQuery CUD 3 年期價格 (計費模式 ID 4D8D-49A7-C5B1)。
...
**注意**：本文討論部分依支出計算的承諾使用折扣 (CUD)，這些折扣已自動遷移至新的計費模式，採用折扣而非抵免額。 Google Cloud 控制台的「帳單總覽」頁面會顯示通知，指出遷移日期。如要進一步瞭解這些改善措施、受影響的 CUD，以及您需要採取的行動，請參閱「[依支出計算的 CUD](https://docs.cloud.google.com/docs/cuds-multiprice?hl=zh-tw)」。
...
系統會自動將以支出為準的 CUD 套用至區域內的 BigQuery 執行個體總用量，讓您享有低廉且可預測的費用，不必手動變更或更新任何項目。這項彈性機制有助於您提高承諾用量，節省時間和金錢。BigQuery 支出型 CUD 適用於所有 BigQuery 隨用隨付方案的使用量。BigQuery 支出型 CUD 適用於所有支援的運算容量類型，包括：

* BigQuery 版本
* Managed Service for Apache Airflow (第 3 代) (舊稱 *Cloud Composer 3*)
* Knowledge Catalog
* BigQuery 服務
* Managed Service for Apache Spark (舊稱 *Dataproc 無伺服器*)

如需適用 SKU 的完整清單，請參閱「[可使用 BigQuery CUD 的 SKU](https://cloud.google.com/skus/sku-groups/bigquery-cud-eligible-skus?hl=zh-tw)」。
...
* BigQuery 支出型 CUD 不適用於儲存空間或隨選方案。
* BigQuery 依用量計費的 CUD 適用於特定區域的所有即付即用運算單元 SKU。
...
請前往 [BigQuery 定價](https://cloud.google.com/bigquery/pricing?hl=zh-tw)頁面，瞭解三年期方案的每小時費用，例如每運算單元小時 $0.036 美元 (可享 8 折優惠)。該金額就是承諾價格，您承諾在 `us-central1` 的所有專案中，三年內每小時在 BigQuery 花費的金額。如果超量，系統會依即付即用費率計費。
