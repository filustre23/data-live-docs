...
可解釋 AI 可協助您瞭解預測機器學習模型為分類和迴歸工作產生的結果，方法是定義資料列中各項特徵對預測結果的影響程度。這類資訊通常稱為「功能歸因」。有了這項資訊，您就能確認模型的運作情形是否符合預期、找出偏誤，以及探索改良模型和訓練資料的方式。

BigQuery ML 和 Gemini Enterprise Agent Platform 都有可解釋 AI 服務，可提供特徵說明。您可以在 BigQuery ML 中執行可解釋性功能，也可以在 Agent Platform 中[註冊模型](https://docs.cloud.google.com/bigquery/docs/managing-models-vertex?hl=zh-tw#register_models)，然後在該處執行可解釋性功能。
...
## Agent Platform 的可解釋 AI

在 Agent Platform 中，可解釋 AI 適用於下列可匯出的監督式學習模型子集：
...
### 在 Model Registry 中啟用可解釋 AI

在 Model Registry 中註冊 BigQuery ML 模型後，如果模型類型支援可解釋 AI，您可以在將模型部署至端點時啟用可解釋 AI。註冊 BigQuery ML 模型時，系統會自動填入所有相關聯的中繼資料。

**注意：** 可解釋 AI 會產生少許額外費用。詳情請參閱 [Gemini Enterprise Agent Platform 計價方式](https://cloud.google.com/vertex-ai/pricing?hl=zh-tw)。
...
7. 選取機型，例如 `n1-standard-2`。
8. 在「模型設定」下方的記錄部分，勾選核取方塊，啟用可解釋性選項。
9. 依序點選「完成」和「繼續」，將設定部署至端點。

如要瞭解如何對 Model Registry 中的模型使用 XAI，請參閱「[使用已部署的模型取得線上說明](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/tabular-data/classification-regression/get-online-predictions?hl=zh-tw#online-explanation)」。如要進一步瞭解 Gemini Enterprise Agent Platform 中的 XAI，請參閱「[取得說明](https://docs.cloud.google.com/gemini-enterprise-agent-platform/machine-learning/explainable-ai/getting-explanations?hl=zh-tw)」。

## 後續步驟

* 瞭解如何[在 Gemini Enterprise Agent Platform 中管理 BigQuery ML 模型](https://docs.cloud.google.com/bigquery/docs/managing-models-vertex?hl=zh-tw)。
* 如要進一步瞭解支援可解釋性的模型所適用的 SQL 陳述式和函式，請參閱[機器學習模型端對端使用者歷程](https://docs.cloud.google.com/bigquery/docs/e2e-journey?hl=zh-tw)。
