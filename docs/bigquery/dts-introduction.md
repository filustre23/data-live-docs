...
**提示：** 你也可以使用「管道與連線」頁面，透過[簡化工作流程](https://docs.cloud.google.com/bigquery/docs/pipeline-connection-page?hl=zh-tw)建立轉移作業。
...
**附註：** 訂閱 [BigQuery DTS 公告群組](https://groups.google.com/g/bigquery-dts-announcements?hl=zh-tw)，即可接收與 BigQuery 資料移轉服務相關的公告。
...
BigQuery 資料移轉服務支援從儲存資料的任何區域，移轉資料至目的地資料集所在的任何位置。
...
只有在專案、資料夾或機構已指派給具有下列任一[工作類型](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#assignments)的預留位置時，BigQuery 資料移轉服務觸發的工作才會使用預留位置配額：
