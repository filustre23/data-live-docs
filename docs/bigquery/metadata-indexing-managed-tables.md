...
根據預設，BigQuery 中的中繼資料索引作業是免費的背景作業，您不必執行任何操作。不過，索引新鮮度取決於可用的免費資源，且沒有效能服務等級目標 (SLO)。如果索引新鮮度對您的用途至關重要，建議設定[`BACKGROUND`預留](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#assignments)，在背景最佳化作業之間共用資源。
...
1. 前往 Google Cloud 控制台的「容量管理」頁面。
   ...
   [前往「容量管理」頁面](https://console.cloud.google.com/bigquery/admin/reservations?hl=zh-tw)
