...
根據預設，BigQuery 中的中繼資料索引作業是免費的背景作業，您不必進行任何操作。不過，索引新鮮度取決於可用的免費資源，且沒有效能服務等級目標 (SLO)。如果索引新鮮度對您的用途至關重要，建議設定[`BACKGROUND`預留](https://docs.cloud.google.com/bigquery/docs/reservations-workload-management?hl=zh-tw#assignments)，在背景最佳化作業之間共用資源。
...
1. 前往 Google Cloud 控制台的「Workload management」(工作負載管理) 頁面。

   [前往「工作負載管理」](https://console.cloud.google.com/bigquery/admin/reservations?hl=zh-tw)
...
2. 前往「整合」頁面。
   ...
   [前往「整合」頁面](https://console.cloud.google.com/monitoring/integrations?hl=zh-tw)
3. 找到 **BigQuery** 整合功能，然後按一下「查看詳細資料」。
4. 在「快訊」分頁中，選取「Slot Usage - Background Metadata Cache Slot Usage Too High」(使用量 - 背景中繼資料快取插槽使用量過高)。
...
BigQuery 會使用分區和分群功能自動修剪資料，而中繼資料索引功能則可根據未用於分區或分群的資料欄修剪資料，進一步最佳化查詢。如果篩選資料欄與資料表的分區或叢集資料欄之間存在排序依附元件，修剪作業的成效就會提高。在這些情況下，即使篩選條件是根據並非資料表叢集設定明確部分的資料欄，您也可能會發現掃描的位元組數少於預期。如果資料表僅經過分區處理，但未經過叢集處理，則修剪作業對隨選查詢的效用有限，您可能不會看到掃描的位元組減少。
