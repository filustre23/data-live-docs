...
If the connection [requires personal credentials](/docs/connect-to-data/data-connections/oauth-data-connections#notebook-view) for authoring and [**Sharing results**](/docs/connect-to-data/data-connections/oauth-data-connections#sharing-results) is **Keep private to each user**, the resulting Thread cannot be shared with other users in the workspace — but the notification will still deliver to the Task's configured destination.
