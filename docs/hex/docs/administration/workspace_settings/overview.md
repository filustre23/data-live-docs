...
Type or paste the email (or a comma-separated list of emails) you wish to add to your workspace. Then select a workspace role from the drop-down menu and click **Add users**. The invited user(s) will receive an invite email from **[[email protected]](/cdn-cgi/l/email-protection#80eeeff4e9e6f9c0e8e5f8aef4e5e3e8)** prompting them to join your workspace. You will see the invited user(s) appear in the **Users** list.
